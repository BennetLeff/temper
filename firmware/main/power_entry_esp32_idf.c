...
enum {
    ...
    /* Candidate logic pulse. Minimum and maximum at the installed GPIO14
     * node still require a target capture with the selected latch. */
    PE_IDF_COOKER_RESET_PULSE_US = 1000,
};
...
static bool idf_pulse_cooker_reset_open_drain(void *context) {
    (void)context;
    const gpio_num_t pin = (gpio_num_t)PE_ESP_GPIO_COOKER_RESET_REQUEST;
    gpio_config_t config = {
        .pin_bit_mask = 1ULL << PE_ESP_GPIO_COOKER_RESET_REQUEST,
        .mode = GPIO_MODE_INPUT_OUTPUT_OD,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    /* GPIO14 is an input at boot. Preload released before enabling its
     * output path so no push-pull high or accidental low edge is produced. */
    if (gpio_set_level(pin, 1) != ESP_OK) return false;
    if (gpio_config(&config) != ESP_OK || gpio_get_level(pin) != 1 ||
        gpio_set_level(pin, 0) != ESP_OK) {
        (void)gpio_set_level(pin, 1);
        (void)gpio_set_direction(pin, GPIO_MODE_INPUT);
        return false;
    }
    esp_rom_delay_us(PE_IDF_COOKER_RESET_PULSE_US);
    if (gpio_set_level(pin, 1) != ESP_OK || gpio_get_level(pin) != 1) {
        /* Best-effort release if the GPIO API fails after the sinking edge.
         * STOP and the shared reset-node interlock remain asserted on fault. */
        (void)gpio_set_direction(pin, GPIO_MODE_INPUT);
        return false;
    }
    return true;
...
bool pe_esp32_idf_boot(pe_esp32_adapter_t *adapter) {
    ...
    pe_esp32_adapter_ops_t ops = {
        ...
        .pulse_cooker_reset_open_drain = idf_pulse_cooker_reset_open_drain,
