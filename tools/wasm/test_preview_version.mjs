...
test("privileged workflows contain only immutable Action refs", () => {
  ...
  const publishJobStart = publisher.indexOf("  publish-preview:");
  const publishJobEnd = publisher.indexOf("  terminalize-source-failure:");
  assert.ok(publishJobStart >= 0 && publishJobEnd > publishJobStart);
  const publishJob = publisher.slice(publishJobStart, publishJobEnd);
  assert.match(publishJob, /environment: cloudflare-wasm-preview/);
  const publishStepStart = publishJob.indexOf("- name: Validate, upload, sweep, compare, and preserve diagnostics");
  const publishStepEnd = publishJob.indexOf("- name: Re-read PR head before authoritative completion");
  assert.ok(publishStepStart >= 0 && publishStepEnd > publishStepStart);
  const publishStep = publishJob.slice(publishStepStart, publishStepEnd);
  assert.match(publishStep, /^\s+CLOUDFLARE_API_TOKEN: \$\{\{ secrets\.CF_TOKEN \}\}$/m);
  assert.match(publishStep, /if \[ -z "\$\{CLOUDFLARE_API_TOKEN:-\}" \]/);
  assert.match(publishStep, /grep -R -Fq "\$\{CLOUDFLARE_API_TOKEN\}" \/tmp\/preview-diagnostics/);
  assert.doesNotMatch(publisher, /CLOUDFLARE_WASM_PREVIEW_API_TOKEN/);
  const nightly = readFileSync(new URL("../../.github/workflows/wasm-tier-nightly.yml", import.meta.url), "utf8");
  const nightlyUploadJobStart = nightly.indexOf("  worker-dispatch-r19:");
  const nightlyUploadJobEnd = nightly.indexOf("  report-nightly-failure:");
  assert.ok(nightlyUploadJobStart >= 0 && nightlyUploadJobEnd > nightlyUploadJobStart);
  const nightlyUploadJob = nightly.slice(nightlyUploadJobStart, nightlyUploadJobEnd);
  assert.match(nightlyUploadJob, /environment: cloudflare-wasm-preview/);
  const nightlyUploadStepStart = nightlyUploadJob.indexOf("- name: Upload and sweep the candidate as an immutable non-deployed version");
  const nightlyUploadStepEnd = nightlyUploadJob.indexOf("- name: Deployed-corpus staleness check");
  assert.ok(nightlyUploadStepStart >= 0 && nightlyUploadStepEnd > nightlyUploadStepStart);
  const nightlyUploadStep = nightlyUploadJob.slice(nightlyUploadStepStart, nightlyUploadStepEnd);
  assert.match(nightlyUploadStep, /^\s+CLOUDFLARE_API_TOKEN: \$\{\{ secrets\.CF_TOKEN \}\}$/m);
  assert.match(nightlyUploadStep, /if \[ -z "\$\{CLOUDFLARE_API_TOKEN:-\}" \]/);
  assert.match(nightlyUploadStep, /grep -R -Fq "\$\{CLOUDFLARE_API_TOKEN\}" "\$\{diag\}"/);
  assert.doesNotMatch(nightly, /CLOUDFLARE_WASM_PREVIEW_API_TOKEN/);
  const producer = readFileSync(new URL("../../.github/workflows/wasm-tier-preview-verdict.yml", import.meta.url), "utf8");
  assert.doesNotMatch(producer, /secrets\./);
