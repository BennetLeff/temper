...
# Validate the entire setup and report any issues
bd-validate-setup() {
    local errors=0
    ...
    if _bd_hooks_installed 2>/dev/null; then
    ...
    # Check 4: Worktree root
    echo "✓ Worktree root: ${BD_WORKTREE_ROOT:-\$HOME/worktrees}"

    # Check 5: Multi-agent mode
    if [[ "$_BD_MULTIAGENT_ENABLED" == "true" ]]; then
        echo "✓ Multi-agent coordination: enabled"
    else
        echo "○ Multi-agent coordination: not loaded"
    fi
    ...
    if [[ $errors -eq 0 ]]; then
        ...
        echo ""
        echo "To fix, run: ./bd-setup.sh --quiet"
        return 1
    fi
}
...
bd-status() {
    ...
    local project
    project=$(_bd_get_project_name)
    local worktree_root="${BD_WORKTREE_ROOT:-$HOME/worktrees}/$project"
...
# Generate shell config block for installation
bd-generate-config() {
    local shell="${1:-bash}"
...
# Load worktree helpers (auto-sources multiagent utilities)
source "\$(cd "\$(dirname "\${BASH_SOURCE[0]}")" && pwd)/tools/bd-worktree-helpers.sh"
BASH
    fi
}
