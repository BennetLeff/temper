...
def run_experiment():
    ...
    c_obs = Component(ref="OBS", footprint="RECT", bounds=(2.0, 0.6), initial_position=(5.0, 5.2), initial_side=0, pins=[])
    
    c_s_p = Component(ref="S_P", footprint="PIN", bounds=(0.2,0.2), initial_position=(2.0, 5.4), initial_side=0, pins=[Pin(name="1", number="1", net="NET_D_P", position=(0,0))])
    c_s_n = Component(ref="S_N", footprint="PIN", bounds=(0.2,0.2), initial_position=(2.0, 5.0), initial_side=0, pins=[Pin(name="1", number="1", net="NET_D_N", position=(0,0))])
    
    c_e_p = Component(ref="E_P", footprint="PIN", bounds=(0.2,0.2), initial_position=(8.0, 5.4), initial_side=0, pins=[Pin(name="1", number="1", net="NET_D_P", position=(0,0))])
    c_e_n = Component(ref="E_N", footprint="PIN", bounds=(0.2,0.2), initial_position=(8.0, 5.0), initial_side=0, pins=[Pin(name="1", number="1", net="NET_D_N", position=(0,0))])
    ...
    print(f"NET_D_P Success: {path_p.success}")
    print(f"NET_D_N Success: {path_n.success}")
    ...
    if path_p.success and path_n.success:
        ...
        # We expect a LOW coupling ratio because they split around the obstacle
        if coupling < 0.5:
             print("OBSERVATION: Router split the differential pair (Expected behavior for current router).")
             print("FAIL: Ideally, both should route on the same side to maintain coupling.")
        else:
             print("SUCCESS: High coupling ratio! (Unexpectedly good behavior?)")
    else:
