...
def assemble(trial: Path, candidate: Path, destination: Path) -> None:
    ...
    # The flat strict-candidate envelope has no sub-sheet file; the production
    # hierarchical path still does. Copy it only when present.
    sub_sheet = candidate / "mcu.kicad_sch"
    if sub_sheet.is_file():
        shutil.copyfile(sub_sheet, destination / "mcu.kicad_sch")
