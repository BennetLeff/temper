...
# the I2C/io0 cluster on the right, all below the antenna keepout.  The
# switches are the real Panasonic EVQP7A land pattern (~5.5 x 4.7 mm, two
# terminals per contact) rather than the old 0603 placeholder, so they sit
# clear of the 0603 parts above them in each column.
PLACEMENT = {
    "U1": (35.0, 50.0, 0),
    "C1": (20.0, 38.0, 90),
    "C2": (20.0, 42.0, 90),
...
def build_copper(measurement: dict):
    ...
    (the EVQP7A switch: pads ``1,1,2,2``).  Every physical pad of a contact is
    routed to that contact's single via, so no same-number pad is left open;
    the old last-wins ``pad -> position`` dict silently dropped one of them.
    """
    pads: dict[str, dict[str, list[list[float]]]] = {}
    for fp in measurement["footprints"]:
        # The judge-facing census collapses repeated pad numbers to one
        # electrical identity; `pads_all` keeps every physical pad so the
        # apparatus routes all of them.
        for pad in fp.get("pads_all", fp["pads"]):
            pads.setdefault(fp["reference"], {}).setdefault(pad["number"], []).append(
                pad["position_mm"]
            )

    # vcc: U1.2 + C1.1/C2.1/R1.1 (left) + R2.1/R3.1/R4.1 (right)
    # en:  U1.3 + C3.1 + R1.2 + SW1.1 (all left)
    # io0: U1.27 + R2.2 + SW2.1 (right column, but routed to the left so its
    #      via clears SW2's gnd contact at x=49.72)
    ...
    for net, entries in net_pads.items():
        ...
        for ref, pad, side in entries:
            contact = pads[ref][pad]
            centre = [
                sum(pos[0] for pos in contact) / len(contact),
                sum(pos[1] for pos in contact) / len(contact),
            ]
            via = outward_via(centre, side)
            points.append(via)
            width = POWER_WIDTH if net == "vcc" else SIGNAL_WIDTH
            # Every physical pad of the contact reaches the shared via.
            for pad_mm in contact:
                stubs[net].append(
                    {
                        "start_mm": list(pad_mm),
                        "end_mm": list(via),
                        "layer": "F.Cu",
                        "width_mm": width,
                    }
...
def build_gnd(pads: dict[str, dict[str, list[list[float]]]]) -> list[dict]:
    ...
    Multiple physical pads per gnd contact (the switch's ``2,2``) each get
    their own bus stub, so none is left open.
    """
    left = [
        list(pos)
        for ref, pad in (
        ...
        for pos in pads[ref][pad]
    ]
    u1_gnd = list(pads["U1"]["1"][0])
    sw2_gnd = [list(pos) for pos in pads["SW2"]["2"]]
    ...
    for x, y in sw2_gnd:
        segments.append(seg((x, y), (GND_RIGHT_X, y)))
    segments.append(
        seg((GND_RIGHT_X, min(y for _, y in sw2_gnd)), (GND_RIGHT_X, y_bottom))
    )
