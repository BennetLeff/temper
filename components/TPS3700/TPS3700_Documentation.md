...
`INA+` monitors post-ferrite `RTD_AVDD` through a 616 kΩ / 100 kΩ, 0.1%
divider. OUTA is pulled up to `SAFETY_3V3`; it is low at RTD_AVDD undervoltage,
which prevents the post-rail open-drain NAND from clearing `RTD_HW_FAULT`.
...
hysteretic supervisor; the computed complete threshold range is 2.766–2.870 V.
