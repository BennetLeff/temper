...
**Scoring weights:**
- Without routing: 50% placement + 50% DRC
- With routing: 40% placement + 40% DRC + 20% routing

### Routing vs Placement Metrics

- **Placement metrics** (HPWL, overlaps, clearances) are computed directly from component positions without running a router
- **Routing metrics** (completion, wirelength, vias) come from actual routing verification and prove the placement is routable
- Routing metrics provide ground truth but are slower to compute
=======
| Metric ID | Description | Target | Source | Command |
|-----------|-------------|--------|--------|---------|
| `routing_completion_pct` | Percentage of nets successfully routed | ==100% | FreeRouting | `scripts/placement_quality_report.py --pcb {pcb} --route` |
| `routing_wirelength_mm` | Total routed wirelength | project-specific | FreeRouting | `scripts/placement_quality_report.py --pcb {pcb} --route` |
| `routing_via_count` | Total number of vias in routing | <=100 | FreeRouting | `scripts/placement_quality_report.py --pcb {pcb} --route` |
| `routing_quality_score` | Composite placement + routing score | >=80 | Quality Script | `scripts/placement_quality_report.py --pcb {pcb} --json` |
