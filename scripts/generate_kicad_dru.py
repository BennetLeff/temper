...
def generate_dru() -> str:
    ...
    lines.append(f"   (constraint clearance (min {DEFAULT_ROUTING_CLEARANCE_MM}mm))")
    lines.append(")")
    lines.append("")
...
# ---------------------------------------------------------------------------
# PER-NET-PAIR CLEARANCE EXPORT (added 2026-08-12 -- see
# docs/evidence/2026-08-12-router-safety-clearances.md).
#
# The .kicad_dru above is what kicad-cli enforces, and therefore what this
# board's clearance violations are counted against. The router's A* search
# needs the SAME numbers if its routing decision is to stop producing those
# violations in the first place -- but it cannot read a KiCad rule file, and
# re-deriving the figures by hand inside the router would be a second,
# drifting copy of a safety-critical table.
#
# So this module exports the rules it just emitted, resolved into the flat
# per-net-class-PAIR matrix the router's occupancy model can consume. The
# resolution EVALUATES the emitted rules under KiCad's own last-matching-
# rule-wins precedence (the same `_matching_rules` machinery the shadowing
# guard uses), rather than re-reading the constants above -- so the exported
# matrix is a measurement of the emitted file and cannot disagree with what
# kicad-cli will check.
#
# WORLD: Track <-> Track, different footprint references. That is the pairing
# the ROUTER decides. docs/evidence/2026-08-12-dru-rule-precedence.md measured
# 1,053 of 1,291 distinct violating pairs as bare track<->track naming no
# component, and stripping all copper takes the board to its 48-violation
# placement floor. Same-footprint rules (RULE 1/1a/5/7) are excluded by the
# differing references, which is correct: two tracks are never "in the same
# footprint", so the router must not borrow a footprint-internal relaxation.
# ---------------------------------------------------------------------------

# The class KiCad reports for a net carrying no net-class assignment. 69 of
# this board's 110 nets are in exactly that position, and every complement-
# shaped safety rule above ("6mm to everything except itself") therefore
# applies to them -- which is precisely the population the router had been
# routing at the flat 0.2mm default.
...
def derive_pair_clearance_matrix(
    content: str, constraint: str = "clearance"
) -> dict[tuple[str, str], float]:
    """Resolve the emitted rules into ``{(class_a, class_b): required_mm}``.

    ``content`` is an emitted .kicad_dru (post-``order_rules_by_strictness``).
    The returned mapping is symmetric and total over every net class the file
    names, plus :data:`UNASSIGNED_NETCLASS`.
    """
    rules = [r for r in _parse_rules(content) if r.constraints]
    pair_rules = [r for r in rules if any(t in r.constraints for t in PAIR_CONSTRAINT_TYPES)]
    predicates = [_compile_condition(r.condition) for r in pair_rules]
    ...
    for class_a, class_b in itertools.product(classes, repeat=2):
        world = {
            "a_NetClass": class_a,
            "b_NetClass": class_b,
            "a_Type": "Track",
        ...
        applicable = [
            r for r in _matching_rules(pair_rules, predicates, world) if constraint in r.constraints
        ]
        if applicable:
            # Last matching rule wins -- KiCad's own precedence, which
            # order_rules_by_strictness() has already made equivalent to
            # strictest-wins for this file.
            matrix[(class_a, class_b)] = applicable[-1].constraints[constraint]
    return matrix


def render_pair_clearance_yaml(content: str) -> str:
    """Render :func:`derive_pair_clearance_matrix` as the router's config."""
    matrix = derive_pair_clearance_matrix(content)
    classes = sorted({a for a, _ in matrix})
    lines = [
        "# GENERATED by scripts/generate_kicad_dru.py -- DO NOT EDIT BY HAND.",
        "#",
        "# Per-net-class-PAIR clearance, in mm, for a Track<->Track pairing on",
        "# different footprints. Derived by evaluating pcb/temper.kicad_dru's own",
        "# rules under KiCad's last-matching-rule-wins precedence, so these are the",
        "# figures kicad-cli enforces -- not an independent restatement of them.",
        "#",
        "# Consumed by temper_placer.router_v6.pair_clearance, which turns it into",
        "# the per-clearance-profile occupancy grids the A* search decides against.",
        ...
        "world: track_to_track_different_reference",
        "",
        "classes:",
    ]
    ...
    lines.append("")
    lines.append("pairs:")
    for (class_a, class_b), value in sorted(matrix.items()):
        if class_a <= class_b:
            lines.append(f"  {class_a}|{class_b}: {value}")
    lines.append("")
    return "\n".join(lines)


def render_pair_creepage_yaml(content: str) -> str:
    """Render :func:`derive_pair_clearance_matrix` (constraint
    ``"creepage"``) as the A* obstacle map's per-class-pair creepage table.

    Same machinery as :func:`render_pair_clearance_yaml`; the constraint
    filter is the only difference. The world stays Track<->Track: the
    creepage rules' conditions reference only NetClass, so the resolved
    figure is identical for Pad, Via and Zone items of the same two classes
    (see the PAIR_CREEPAGE_MATRIX_PATH comment for the full argument).
    """
    matrix = derive_pair_clearance_matrix(content, "creepage")
    # The classes list is the FULL universe the matrix was derived over --
    # not the matrix keys, which would silently drop a class whose every
    # pair resolves to 0.0 (no creepage rule matches it, e.g. GateDriveHV,
    # which every reinforced rule excludes from its B-side). A dropped
    # class would then be misread by the loader as UNASSIGNED and wrongly
    # charged the Default-class creepage figure.
    classes = sorted(
        {kicad_class_name(key) for key in TEMPER_NET_CLASSES} | {UNASSIGNED_NETCLASS}
    )
    lines = [
        "# GENERATED by scripts/generate_kicad_dru.py -- DO NOT EDIT BY HAND.",
        "#",
        "# Per-net-class-PAIR CREEPAGE, in mm, between any two copper items of",
        "# different nets (Track<->Track, Pad<->Track, Track<->Via, Pad<->Pad,",
        "# ...). Derived by evaluating pcb/temper.kicad_dru's own creepage",
        "# rules under KiCad's last-matching-rule-wins precedence -- exactly the",
        "# resolution KiCad's DRC engine performs. The creepage rules key only",
        "# on NetClass, so the Track<->Track world this is resolved in is",
        "# type-independent. A pair no creepage rule matches resolves to 0.0:",
        "# KiCad applies no creepage check to it, and the clearance floor still",
        "# protects it.",
        "#",
        "# Consumed by temper_placer.router_v6.pair_creepage, which turns it",
        "# into the per-class creepage halos and stamp radii of the N-layer A*",
        "# obstacle map (see docs/evidence/2026-08-16-creepage-aware-cspace.md).",
        "# Regenerate with: uv run python scripts/generate_kicad_dru.py",
        "# Drift is gated by scripts/tests/test_generate_kicad_dru.py.",
        f"# '{UNASSIGNED_NETCLASS}' is the class KiCad reports for an unassigned net.",
        "",
...
# Twin of ZONE_PAIR_MATRIX_PATH for the CREEPAGE constraint: the distance a
# pour's outline must be carved back from every other net's copper when the
# DRC judges that pair by creepage (which for HV<->LV it always does --
# 12.6mm PD3 reinforced vs 2.0mm clearance). See
# docs/evidence/2026-08-15-rust-zone-pour-design.md and the module doc of
# temper_placer.router_v6.zone_pour_creepage for why the carve uses
# max(clearance, creepage), not either alone.
ZONE_POUR_CREEPAGE_MATRIX_PATH = (
    REPO_ROOT / "packages" / "temper-placer" / "configs" / "zone_pour_creepage.generated.yaml"
)

# Track/Pad/Via-wire CREEPAGE pair matrix (2026-08-16): the per-class-pair
# creepage figure the N-layer A* obstacle map charges as its static halo and
# its routed-copper stamp radius. Before this, the router reserved only
# CLEARANCE (0.2mm floor) around obstacles and between nets, so an HV track
# could thread 0.2mm from an LV pad while the DRC grades that pair at
# 12.6mm (PD3 reinforced) -- measured as ~300 pad<->track/track<->track
# creepage violations on the routed board (see
# docs/evidence/2026-08-16-creepage-aware-cspace.md). The zone-pour table
# above already proved the derivation route for creepage: evaluate the
# emitted rules under KiCad's own last-matching-rule-wins precedence. The
# creepage rules key ONLY on NetClass (never Type/Reference), so the
# Track<->Track world this matrix is resolved in is type-independent -- the
# same figure grades a pad<->track, track<->track, track<->via or pad<->via
# pair of the same two classes. A pair no creepage rule matches resolves to
# 0.0 (KiCad applies no creepage check to it; the clearance floor still
# protects it).
PAIR_CREEPAGE_MATRIX_PATH = (
    REPO_ROOT / "packages" / "temper-placer" / "configs" / "pair_creepage.generated.yaml"
)
...
def derive_zone_pair_creepage_matrix(
    content: str,
) -> dict[tuple[str, str, str], float]:
    """Resolve ``{(zone_class, other_class, other_type): required_mm}`` for
    the CREEPAGE constraint.

    Same world model as :func:`derive_zone_pair_clearance_matrix` (the
    pour is always item type ``Zone`` against the other item), but
    resolving the ``creepage`` constraint instead of ``clearance``.  A
    pair no creepage rule matches resolves to ``0.0`` -- KiCad's own DRC
    applies no creepage check to such a pair, so the pour must not be
    carved back for one; the clearance twin (and the
    ``max(clearance, creepage)`` carve in ``_emit_zone_pours``) still
    protects that pair at its clearance figure.
    """
    rules = [r for r in _parse_rules(content) if r.constraints]
    pair_rules = [r for r in rules if any(t in r.constraints for t in PAIR_CONSTRAINT_TYPES)]
    predicates = [_compile_condition(r.condition) for r in pair_rules]
    ...
    for zone_class, other_class in itertools.product(classes, repeat=2):
        for other_type in ZONE_OTHER_TYPES:
            ...
            applicable = [
                r
                for r in _matching_rules(pair_rules, predicates, world)
                if "creepage" in r.constraints
            ]
            matrix[(zone_class, other_class, other_type)] = (
                applicable[-1].constraints["creepage"] if applicable else 0.0
            )
    return matrix


def render_zone_pair_creepage_yaml(content: str) -> str:
    """Render :func:`derive_zone_pair_creepage_matrix` as the pour's config."""
    matrix = derive_zone_pair_creepage_matrix(content)
    classes = sorted({key[0] for key in matrix})
    lines = [
        "# GENERATED by scripts/generate_kicad_dru.py -- DO NOT EDIT BY HAND.",
        "#",
        "# Per-net-class-PAIR CREEPAGE, in mm, for a POUR (zone filled copper)",
        "# against another net's Track / Pad / Via / Zone. Derived by evaluating",
        "# pcb/temper.kicad_dru's own creepage rules under KiCad's",
        "# last-matching-rule-wins precedence -- exactly the resolution KiCad's",
        "# DRC engine performs. A pair no creepage rule matches resolves to 0.0:",
        "# KiCad applies no creepage check to it, and the pour's carve uses",
        "# max(clearance, creepage) so the clearance twin still protects it.",
        "#",
        "# WHY THIS TABLE EXISTS: the outline carve previously used the DRU",
        "# CLEARANCE table only (HV<->LV 2.0mm), but the DRC judges HV<->LV",
        "# creepage at 12.6mm (PD3 reinforced). A pour carved at 2.0mm from a",
        "# +3V3 pad fills copper 2.0mm from it and violates the 12.6mm creepage",
        "# rule -- measured as the '+170V_BUS pour 2.0mm from +3V3 pad of U16'",
        "# family. Carving at max(clearance, creepage) closes it (measured: min",
        "# gap 12.58mm >= 12.6). See docs/evidence/2026-08-15-rust-zone-pour-design.md.",
        "# Regenerate with: uv run python scripts/generate_kicad_dru.py",
        "# Drift is gated by tests/router_v6/test_zone_pour_creepage.py.",
        f"# '{UNASSIGNED_NETCLASS}' is the class KiCad reports for an unassigned net.",
        "",
        'version: "1.0"',
        "source: pcb/temper.kicad_dru",
...
def main() -> None:
    ...
    pair_creepage_yaml = render_pair_creepage_yaml(content)
    PAIR_CREEPAGE_MATRIX_PATH.write_text(pair_creepage_yaml, encoding="utf-8")
    print(f"Wrote {len(pair_creepage_yaml.splitlines())} lines to {PAIR_CREEPAGE_MATRIX_PATH}")
    ...
    zone_creepage_yaml = render_zone_pair_creepage_yaml(content)
    ZONE_POUR_CREEPAGE_MATRIX_PATH.write_text(zone_creepage_yaml, encoding="utf-8")
    print(f"Wrote {len(zone_creepage_yaml.splitlines())} lines to {ZONE_POUR_CREEPAGE_MATRIX_PATH}")
    print()
    print(content)
