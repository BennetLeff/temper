...
the Rust kernels against these files -- but nothing previously noticed when
an oracle drifted: the suites compare Rust against *whatever the oracle now
says*, so an accidental edit silently redefined the reference and the
differentials kept passing against the wrong target (issues #754 P2-2,
#758). The registry is the committed record of the pinned content; this gate
makes any drift from that record a CI failure instead of an invisible
redefinition.
...
Exit codes (mirrors ``scripts/check_derived_doc_drift.py``):
  ...
  5 - tool_error: registry missing, unparseable, wrong schema, or an empty
      scan (zero oracle files and zero registry entries).
...
def load_registry(registry_path: Path) -> tuple[dict[str, str] | None, str | None]:
    """Return (files, error). ``files`` is the registry's path->hash mapping."""
    ...
    if not files:
        return None, "registry 'files' is empty -- zero oracles recorded (vacuous registry)"
    bad = []
...
def run(repo_root: Path, registry_path: Path) -> Report:
    ...
    registry_files, registry_error = load_registry(registry_path)
