...
``temper_placer.core.isolation_constants.MIN_BARRIER_WIDTH_MM`` (imported
below, not copied) -- **8.0mm**, the PD2 figure, matching
``docs/ENVIRONMENTAL_SPEC.md`` Sec 3 ("Pollution Degree: PD2") on
``origin/main``. The SSOT module landed via PR #457.

Separately, an earlier sibling branch (commit ``c58c94d8``, not an ancestor
of ``origin/main``) re-targeted the barrier constant to 12.6mm (PD3) on its
own branch. That decision has not reached ``origin/main``. Use
``--min-creepage-mm 12.6`` explicitly to measure the PD3 case; do not assume
it is the default just because a sibling branch made it one there.
...
# The SSOT (PR #457) lives in the temper_placer package, which this script
# puts on sys.path above via setup_temper_placer_path().
