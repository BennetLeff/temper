...
try:
    ...
    from temper_placer.losses.base import LossContext, CompositeLoss
...
def run_refinement_study(pcb_path: Path, config_path: Path, iterations: int = 500):
    ...
    # 3. Greedy Refinement
    start = time.time()
    greedy_state, greedy_loss = discrete_rotation_refinement(initial_state, loss_fn, search_type="greedy")
    results["greedy"] = {
        "loss": greedy_loss,
