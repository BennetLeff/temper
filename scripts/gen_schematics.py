...
class Component:
    ...
    sheet_module: str
    tstamp: str
    # Authoritative per-designator part identity, sourced from default.csv.
    # Do NOT use `part_name` (from the netlist's libsource) for anything the
...
def mcu_candidate_layout() -> SchematicLayout:
    ...
    Single-sheet, flat envelope: the isolated MCU build (`mcu`, from
    `McuCandidate::mcu.*` instance paths) is one block, so every symbol is
    emitted directly in the root sheet. A hierarchical sub-sheet would scope
    each local label to `/MCU/<net>`, which cannot match the candidate
    board's plain compiled net names; flattening keeps them identical. Root
    and sheet filenames are candidate-scoped so generation can never
    overwrite production schematics by default. This is an explicit
    strict-candidate path: callers pass the returned layout (or its JSON
    form) deliberately; omitting it keeps the production defaults unchanged.
    """
    return SchematicLayout(
...
# ---------------------------------------------------------------------------
# BOM (default.csv) -- authoritative per-designator part identity
# ---------------------------------------------------------------------------


def load_bom_values(bom_path: Path) -> dict[str, str]:
    """Map each designator to its part identity, read from atopile's default.csv.

    The netlist is NOT a usable source for this. Its `libsource.part` is keyed
    by library part, and atopile collapses every component sharing a footprint
    into one libpart entry -- so R51 (430k) and R58 (510k), both 1206, both
    report whichever MPN happened to win. The comp-level `(value ...)` field is
    literally "?" for every passive on this board. Only default.csv carries
    per-designator identity.

    This is the trap already recorded in docs/STRATEGY.md: "default.net aliases
    part identity by footprint -- use default.csv". Sourcing the schematic's
    Value from libsource made the drift gate report a false clean on the
    OVP-01 divider; see
    docs/evidence/2026-07-26-schematic-source-drift-gate-diagnosis.md.

    Raises on anything malformed. A silently-empty mapping here would restore
    exactly the vacuous-pass behaviour this function exists to remove.
    """
    if not bom_path.is_file():
        raise ValueError(
            f"BOM not found: {bom_path}. It is generated by `make netlist` "
            f"alongside default.net; the schematic value cannot be determined "
            f"without it."
        )
    ...
    with bom_path.open(encoding="utf-8", newline="") as fh:
        ...
        if reader.fieldnames is None:
            raise ValueError(f"BOM is empty: {bom_path}")
        for column in ("Comment", "Designator"):
            if column not in reader.fieldnames:
                raise ValueError(
                    f"BOM {bom_path} lacks a {column!r} column "
        ...
        for lineno, row in enumerate(reader, start=2):
            ...
            designators = (row.get("Designator") or "").strip()
            if not designators:
                raise ValueError(f"{bom_path}:{lineno}: row has no Designator")
            if not comment:
                raise ValueError(
                    f"{bom_path}:{lineno}: designator {designators!r} has an "
                    f"empty Comment (part identity)"
                )
            # One row may cover several designators: "R51,R52,R53".
            for designator in designators.split(","):
                designator = designator.strip()
                if not designator:
                    continue
                if designator in values:
                    raise ValueError(
                        f"{bom_path}:{lineno}: designator {designator!r} "
                        f"appears more than once"
                    )
                values[designator] = comment

    if not values:
        raise ValueError(f"BOM {bom_path} yielded no designators")
    return values


def apply_bom_values(netlist: Netlist, bom_values: dict[str, str]) -> None:
    """Attach authoritative per-designator values, or fail.

    Fail-closed on both directions of mismatch. Falling back to `part_name` for
    an unmatched ref is precisely the silent degradation that let the OVP-01
    drift through, so an incomplete BOM is an error, never a partial result.
    """
    netlist_refs = set(netlist.components)
    bom_refs = set(bom_values)

    missing = sorted(netlist_refs - bom_refs)
    if missing:
        raise ValueError(
            f"{len(missing)} component(s) in the netlist have no BOM entry, so "
            f"their part identity is unknown: {missing[:10]}"
            + (" ..." if len(missing) > 10 else "")
        )

    unknown = sorted(bom_refs - netlist_refs)
    if unknown:
        raise ValueError(
            f"{len(unknown)} designator(s) in the BOM are absent from the "
            f"netlist, so the two are out of sync: {unknown[:10]}"
            + (" ..." if len(unknown) > 10 else "")
        )

    for ref, comp in netlist.components.items():
        comp.display_value = bom_values[ref]
...
def _symbol_instance(
    ...
    """
    # `part_name` still drives lib_id (symbols are shared per library part),
    # but the Value the reader sees must be this designator's own identity.
    if not display_value:
        raise ValueError(
            f"{ref}: no BOM-sourced value; refusing to fall back to the "
            f"footprint-aliased libsource part {part_name!r}"
        )
  ...
  (symbol
    ...
    (property "Value" "{display_value}" (at {x:.2f} {y + 2.54:.2f} 0) (effects (font (size 1.27 1.27))))
    ...
    for idx, comp in enumerate(sheet_comps):
        ...
        parts.append(
            _symbol_instance(
                ...
                display_value=comp.display_value,
            )
        )
...
def generate_flat_root_sheet(
    ...
    pin whose connection subgraph has fewer than two items, so the label keeps
    a board-connected single-node pin in the netlist with its compiled name.
    A single-node pin also gets ``no_connect`` at the same endpoint to tell
    ERC that this intentionally isolated source net is complete. A pin absent
    from the compiled netlist gets only ``no_connect``.
    ...
    singleton_pins: set[tuple[str, str]] = set()
    for net in netlist.nets.values():
        for ref, pin in net.nodes:
            pin_net[(ref, pin)] = net.name
        if len(net.nodes) == 1:
            singleton_pins.add(net.nodes[0])
    ...
    for idx, comp in enumerate(comps):
        ...
        for num_str, _name in libpart.pins:
            ...
            parts.append(_global_label(net_name, pin_x, pin_y, label_uuid))
            if (comp.ref, num_str) in singleton_pins:
                nc_uuid = _uuid_from_seed(f"nc:{comp.ref}:{num_str}")
                parts.append(_no_connect(pin_x, pin_y, nc_uuid))
...
def main() -> None:
    ...
    parser.add_argument(
        "--bom-csv",
        type=Path,
        default=Path("elec/build/default.csv"),
        help=(
            "Path to atopile's default.csv, the authoritative source of "
            "per-designator part identity (the netlist's libsource is "
    ...
    print(f"Reading BOM: {args.bom_csv}")
    try:
        bom_values = load_bom_values(args.bom_csv)
        apply_bom_values(netlist, bom_values)
    except ValueError as exc:
        print(f"ERROR: {exc}", file=sys.stderr)
        sys.exit(1)
    print(f"  {len(bom_values)} designators with BOM-sourced values")
    ...
    if args.check:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Diff against committed files. Report every drifted sheet, not
            # just the first -- exiting early makes a developer rediscover the
            # remaining sheets one CI round-trip at a time.
            ...
            if drifted:
                print(
                    f"CHECK FAIL: {len(drifted)} of {len(files)} schematic(s) "
                    f"drifted from the netlist: {', '.join(sorted(drifted))}"
                )
                sys.exit(1)
