...
class TestRealRepoIsClean:
    def test_agent_worktree_paths_are_not_repo_inputs(self, tmp_path, monkeypatch):
        ...
        ``.claude/worktrees`` and ``.worktrees`` contain separate copies of
        this repository; their source is not an input to the checkout being
        gated.  Put known violations there and prove the real iterator
        excludes them, so the skip cannot silently hide a violation in an
        ordinary source directory.
        """
        good = tmp_path / "src" / "good.py"
        bad_claude = tmp_path / ".claude" / "worktrees" / "agent" / "bad.py"
        bad_worktrees = tmp_path / ".worktrees" / "agent" / "bad.py"
        good.parent.mkdir(parents=True)
        bad_claude.parent.mkdir(parents=True)
        bad_worktrees.parent.mkdir(parents=True)
        good.write_text("rotation = c.initial_rotation_quadrant * 90\n")
        bad_claude.write_text("rotation = c.initial_rotation_quadrant / 90\n")
        bad_worktrees.write_text("rotation = c.initial_rotation_quadrant / 90\n")

        monkeypatch.setattr(gate, "REPO_ROOT", tmp_path)
        files = gate._iter_source_files()

        assert good in files
        assert bad_claude not in files
        assert bad_worktrees not in files
        assert gate.scan() == []
