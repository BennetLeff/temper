...
candidate barrier strip at x=[42,58] (16mm wide, i.e. wide enough to hold
a 12.6mm-wide, correctly-configured barrier with margin), one HV-domain
...
def build_board(
    *,
    ...
    barrier_x: tuple[float, float] = (42.0, 58.0),
...
class TestIntrusion:
    ...
    def test_pad_body_overlaps_barrier_even_when_center_is_outside(self, tmp_path: Path) -> None:
        ...
        board = build_board()  # barrier spans x=[42,58]
        extra_fp = Footprint()
        extra_fp.entryName = "Test:LargePad"
        extra_fp.layer = "F.Cu"
        # Center at x=40 (2mm outside the barrier's left edge at x=42), but
        # a 6mm-wide pad reaches to x=43 -- 1mm into the barrier.
        extra_fp.position = Position(40, 50)
