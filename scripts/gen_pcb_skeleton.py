...
# ---------------------------------------------------------------------------
# Strict candidate path. Explicit opt-in: callers pass
# a reviewed pin map, target outline/layers, and neutral staging. The legacy
# generate_board() below (default flow placement + positional pad fallback)
# is untouched; nothing here changes its behavior.
# ---------------------------------------------------------------------------

# Provisional six-layer candidate order retained from the archived adapter.
# This is not a physical stackup; the power-stage stackup is decided in Part 4.
# Ordinal IDs do not encode this sequence's order.
...
def candidate_layer_defs() -> list[tuple[int, str, str, str | None]]:
    """Provisional six-copper-layer defs plus the legacy technical layers."""
