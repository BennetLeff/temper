...
from temper_placer.routing.constraints.spatial_index import Pad as GeoPad, Track as GeoTrack, Via as GeoVia
...
def main():
    ...
    if args.add_power_planes:
        ...
        power_nets = [name for name in net_order_temp if any(k in name.upper() for k in power_keywords)]
    ...
    if args.exclude_power_nets:
        ...
        power_nets_to_exclude = [name for name in net_order if any(k in name.upper() for k in power_keywords)]
        net_order = [name for name in net_order if name not in power_nets_to_exclude]
        console.print(f"  [yellow]Excluded {original_count - len(net_order)} power nets from main routing[/]")
