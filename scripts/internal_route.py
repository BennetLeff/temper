...
def main():
    ...
    parser.add_argument(
        "--use-cspace-pipeline",
        action="store_true",
        help="Use C-Space routing pipeline (temper-2qqd)",
    )
    parser.add_argument(
        "--cspace-resolution",
        type=float,
        default=0.1,
        help="C-Space grid resolution in mm (default: 0.1)",
    )
    ...
    # C-Space Pipeline Routing (temper-2qqd)
    if args.use_cspace_pipeline:
        console.print("\n[bold cyan]Step 4 (C-Space):[/] Running C-Space routing pipeline...")
        ...
        pipeline_config = PipelineConfig(
            resolution_mm=args.cspace_resolution,
            enable_dithering=True,
            enable_smoothing=True,
            enable_ballooning=True,
            max_dither_attempts=4,
            via_cost=args.via_cost,
        )

        pipeline = CSpaceRoutingPipeline(board, netlist, pipeline_config)
        pipeline.initialize_router()

        console.print(f"  C-Space resolution: {args.cspace_resolution}mm")
        console.print(f"  Dithering: enabled")
        console.print(f"  Smoothing: enabled")
        console.print(f"  Ballooning: enabled")

        start_time = time.time()
        pipeline_result = pipeline.route_all(net_order)
        elapsed = time.time() - start_time

        console.print(f"  ✓ C-Space routing complete in {elapsed:.2f}s")
        console.print(f"  ✓ Completion rate: {pipeline_result.completion_rate:.2f}%")

        cache_stats = pipeline.get_cache_stats()
        console.print(
            f"  Cache stats: {cache_stats['hits']} hits, {cache_stats['misses']} misses, {cache_stats['hit_rate'] * 100:.1f}% hit rate"
        )

        results = pipeline_result.net_results
