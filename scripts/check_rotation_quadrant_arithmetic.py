...
# Agent worktrees live under ``.claude/worktrees`` and ``.worktrees`` in local
# checkouts. They are separate checkouts of the same sources, not part of the
# tree being audited; traversing them both multiplies findings and makes the
# real-repo anti-vacuity test fail on otherwise clean source.
SKIP_DIR_PARTS = {"target", "target-shared", ".venv", "node_modules", ".claude", ".worktrees"}
