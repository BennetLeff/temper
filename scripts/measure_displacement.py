...
# Add temper-placer/src to sys.path
...
try:
    ...
    print(f"Error: Could not import temper_placer modules: {e}")
    sys.exit(1)
...
def measure_displacement(pcb_path: Path, config_path: Path, seed: int = 42):
    ...
    if fixed_indices.shape[0] == 0:
        ...
        fixed_positions = jnp.array([[board.origin[0] + board.width/2, board.origin[1] + board.height/2]])
    else:
        fixed_positions = netlist.get_bounds_array()[fixed_indices] # Wrong, get positions
        # Actually get positions from parse_result
        all_initial_pos = jnp.array([c.initial_position if c.initial_position else (0,0) for c in netlist.components])
