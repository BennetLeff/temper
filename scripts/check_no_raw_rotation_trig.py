...
Second sweep: 9 more sites found, none of them in the original 12/13
--------------------------------------------------------------------
A follow-up audit (see ``docs/evidence/2026-07-30-rotation-sign-remaining-
sites.md``) found 9 additional call sites carrying the same shape of bug,
none in PR #479's 12 or ``check_pad_orientation.py``'s pre-existing 13th.
...
#
# The remaining 6 entries below were added by the second sweep documented
# in this module's own docstring ("Second sweep: 9 more sites found") --
# 9 sites total, but router_v6/constraints_geometry.py and
# router_v6/escape_via_generator.py each held one call site in one file,
# and scripts/internal_route.py, scripts/bench_rust_geometry.py, and
# packages/temper-geometry/src/polygon.rs were investigated and NOT
# guarded (see that section for why: dead/broken, or genuinely isolated
# from KiCad data).
