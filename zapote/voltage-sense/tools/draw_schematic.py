...
from close_current_sense_schematic_erc import library_symbol
...
def main():
    ...
    for c in components:
        ...
        if c.ref[0] in "RC":
            ...
            names = (
                ...
                if c.ref == "U1"
                else {"1": "VBIAS", "2": "GND", "3": "EN", "4": "VIN", "5": "VREF"}
                if c.ref == "U2"
                else {}
            )
    ...
    d = [
        g._schematic_header("Positive Half-Bus Voltage Sensing / Rev A", g.ROOT_UUID)
        .replace("2026-07-15", "2026-09-11")
        .replace(
            "edit elec/src/*.ato and run make schematics",
            "Source: voltage_sense_unit.ato; replay: tools/draw_schematic.py",
        ),
        "(lib_symbols",
    ]
    for ref, symbol in sorted(symbols.items()):
        ...
        d.append(symbol.replace(f'(symbol "{sid}"', f'(symbol "VoltageSenseUnit:{sid}"', 1))
    d.append(")")
    ...
    d += [
        note("OVERVOLTAGE DETECTOR / positive half-bus", 30.48, 15.24, 1.8),
        note("ADC MONITOR / 0-250 V input envelope", 30.48, 152.4, 1.8),
        note(
            "NONISOLATED: BUS_RETURN is also host ground. Integration / physical qualification pending.",
            30.48,
            243.84,
        ),
        note("170 V nominal; OVP about 200 V. VBIAS intentionally unused.", 30.48, 254),
    ]
    ...
    for c in components:
        x, y = POSES[c.ref]
        p = net.libparts[c.part_name]
        sid = "VoltageSenseUnit:" + c.ref + "_" + g._sanitize_name(c.part_name)
    ...
    artifacts = {
        "section.kicad_sch": "\n".join(d) + "\n",
        "VoltageSenseUnit.kicad_sym": '(kicad_symbol_lib (version 20231120) (generator "zapote_voltage_sense")\n'
        + "\n".join(symbols.values())
        + "\n)\n",
        "sym-lib-table": '(sym_lib_table (version 7) (lib (name "VoltageSenseUnit")(type "KiCad")(uri "${KIPRJMOD}/VoltageSenseUnit.kicad_sym")(options "")(descr "Source-derived symbols")))\n',
    }
