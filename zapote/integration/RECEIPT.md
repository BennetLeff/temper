...
Base revision: `a599fd27fe67f82afa4ee64f9fa7d2bc0fd6c19f`. Scope: read-only source/native integration screen. No integrated PCB, full-board DRC, firmware, hardware, or physical test was changed or run.
...
The last command is expected to exit **2**. Its saved output is `matrix.tsv`: 39 required unit/edge/fault rows, 20 BLOCKED and 19 INDETERMINATE; overall BLOCKED. The test executable passes six tests. The source lock covers 28 byte-identical inputs. The checker reads and hashes each of them, verifies exact Atopile pin/net statements for source-bearing ports, and extracts numbered connector pad/net assignments from the saved native KiCad boards. `shasum` is the local SHA-256 tool; no cached generated receipt is treated as live native identity. The separate `units.tsv` ledger retains digital-construction, candidate, missing and unbound statuses; the prior buck/MCU acceptance sources are not bound in this snapshot.
...
The separate programming/UI checker at coordinator commit `ab49fd5c3` did not exist at this branch's base. The local `programming.ui` port is explicitly missing and the service-reset edge is blocked. After integration, update the source lock and port/edge status from the reviewed R5 artifact, then replay this gate; do not silently treat its branch commit string as an accepted native identity.
