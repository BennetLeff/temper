...
fn real_source_and_native_artifacts_resolve_complete_census() {
    ...
    let native: UnitNativeEvidence = serde_json::from_str(include_str!(
        "../../../fixtures/current-sense/native-stackup-fixed.json"
    ))
...
fn real_artifacts_build_current_sense_transport_without_synthetic_census() {
    let input = zapote_harness::build_current_sense_input(
        ...
        include_str!("../../../fixtures/current-sense/native-stackup-fixed.json").into(),
...
#[test]
fn real_zero_core_board_is_rejected_by_the_normal_harness() {
    let mut native: Value = serde_json::from_str(include_str!(
        "../../../fixtures/current-sense/native-stackup-fixed.json"
    ))
    .unwrap();
    let board = include_str!("../../../current-sense/evidence/stackup-fix/before.kicad_pcb");
    native["board_file_utf8"] = json!(board);
    native["board_sha256"] = json!(digest(board.as_bytes()));
    let input = zapote_harness::build_current_sense_input(
        include_str!("../../../fixtures/current-sense/source-manifest.json").into(),
        include_str!("../../../fixtures/current-sense/owner-profile.json").into(),
    ...
    assert!(
        report
            .findings
            .iter()
            .any(|f| f.rule == "DRC.BOARD.STACKUP" && f.status == zapote_core::Status::Fail),
        "zero-core board escaped: {report:?}"
    );
}

#[test]
fn missing_or_changed_stackup_bytes_cannot_retain_board_identity() {
    for remove in [true, false] {
        ...
        .unwrap();
        if remove {
            native.as_object_mut().unwrap().remove("board_file_utf8");
        } else {
            native["board_file_utf8"] = json!(native["board_file_utf8"]
                .as_str()
                .unwrap()
                .replace("(thickness 1.44)", "(thickness 0)"));
        }
        let input = zapote_harness::build_current_sense_input(
            include_str!("../../../fixtures/current-sense/source-manifest.json").into(),
            include_str!("../../../fixtures/current-sense/owner-profile.json").into(),
