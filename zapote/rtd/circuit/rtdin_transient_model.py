...
1.10 nF differential (1 nF C0G, +5% tolerance, plus a separate 50 pF allowance) and 100 pF
...
RRETURN = 1.0e-3
RWIN = 100_000.0
# C0603C102J5GACTU is 1 nF C0G, +/-5%; 50 pF is a separate
# harness/board allowance, so worst model C is 1.10 nF.
CD = 1.10e-9
CG = 100e-12
DT = 1.0e-6
TMAX = 0.100
...
def _integrate_transition(
    ...
    for step in range(steps + 1):
        ...
        # The TLV propagation number is only applied after a conservative
        # 20 mV comparator overdrive.  The static corner sweep leaves at least
        # 166 mV open-fault margin, so this threshold is below every modeled
        # open endpoint rather than relying on a zero-margin event.
...
def integrate_short(
    ...
    # A zero-ohm element is represented by a 1 micro-ohm numerical short;
    # this is below any PCB/contact resistance and avoids a singular matrix.
    target = max(target_rtd, 1.0e-6)
    return _integrate_transition(
        "healthy", initial_rtd, "healthy", target, lead, p, "low_margin"
    )


def print_analytic_bound() -> None:
    """Print a conservative corner-independent RC timing bound.

    The static 65536-corner sweep supplies the initial/final comparator
    margins.  For timing, the passive one-pole envelope uses the largest
    diagnostic and protection resistances and a capacitance larger than the
    declared single-pin C.  This is an engineering monotonic-RC bound; it is
    not a vendor MAX ADC/filter claim.
    """
    # Round upward after applying tolerance and TCR so the printed envelope
    # remains a true bound on the purchased parts.
    rdiag_max = 1.057e6
    rwin_max = 102_000.0
    c_bound = CD + CG + 100e-12
    tau_ms = (rdiag_max + rwin_max) * c_bound * 1e3
    # Pair each healthy and open endpoint from the retained full static sweep.
    endpoints = {
        "FORCE+": (0.186101844, -0.166474228),
        "SENSE+": (0.386403955, -1.119681775),
        "SENSE-": (0.423960470, -0.643625475),
        "FORCE-": (0.135419237, -1.150933045),
    }
    times = {}
    for case, (healthy, fault) in endpoints.items():
        fraction = (healthy + FAULT_OVERDRIVE_V) / (healthy - fault)
        fraction = min(max(fraction, 0.0), 1.0 - 1e-12)
        times[case] = -tau_ms * __import__("math").log1p(-fraction)
        print(f"analytic_bound_{case.lower().replace('+','p').replace('-','m')}_fraction={fraction:.6f} "
              f"time_ms={times[case]:.6f}")
    worst = max(times.values())
    print(f"analytic_bound_rdiag_max_ohm={rdiag_max:g} rwindow_max_ohm={rwin_max:g} "
          f"c_bound_nF={c_bound * 1e9:g} tau_bound_ms={tau_ms:.6f} "
          f"worst_rc_cross_ms={worst:.6f} "
          f"tlv_prop_ns={TLV_PROP_NS:g} logic_prop_ns={LOGIC_PROP_NS:g} "
          f"worst_unit_detect_ms={worst + (TLV_PROP_NS + LOGIC_PROP_NS) / 1e6:.6f} "
          "output_load_assumption_pF=15 monotonic_passive_rc_assumption=true")
...
def main() -> None:
    ...
    print(f"dt_us={DT * 1e6:g} tmax_ms={TMAX * 1e3:g} cdiff_nF={CD * 1e9:g} "
          ...
          f"tlv_prop_ns={TLV_PROP_NS:g} logic_prop_ns={LOGIC_PROP_NS:g} "
          f"fault_overdrive_v={FAULT_OVERDRIVE_V:g}")
    ...
    for rtd in (100.0, 194.1):
        ...
        for target_rtd in (10.0, 0.0):
            crossing, final_margin = integrate_short(rtd, target_rtd, 1.0, Params())
            detection = (crossing * 1e3 + (TLV_PROP_NS + LOGIC_PROP_NS) / 1e6
                         if crossing is not None else None)
            print(f"SHORT_INIT={rtd:5.1f} target={target_rtd:g} cross_ms="
                  f"{crossing * 1e3 if crossing is not None else 'NONE'} "
                  f"unit_detect_ms={detection if detection is not None else 'NONE'} "
                  f"final_low_margin_v={final_margin:+.6f}")
    print_analytic_bound()
    ...
    print(f"local_rail_caps_count=4 nominal_each_nF=100 c_accept_min_each_nF=45 "
          f"local_rail_cmin_uF={LOCAL_RAIL_CMIN * 1e6:g} "
