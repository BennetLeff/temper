...
def drawing_height(part: donor.LibPart) -> float:
...
def library_symbol(part: donor.LibPart) -> str:
...
def run(source: Path, output: Path, receipt_path: Path) -> None:
    ...
    if any(c.sheet_module not in ('rtd_pan', 'unit_io') for c in components):
        raise ValueError('postprocessor is restricted to standalone RTDUnit source')
    parts = {c.part_name: netlist.libparts[c.part_name] for c in components}
    symbols = {name: library_symbol(part) for name, part in sorted(parts.items())}
    library = '(kicad_symbol_lib (version 20231120) (generator "zapote_rtd_unit")\n'
    library += '\n'.join(symbols.values()) + '\n)\n'
    drawing = [donor._schematic_header('Standalone RTD Unit', donor.ROOT_UUID)
    ...
    allowed_open = {('MAX31865AAP+', '20'), ('TPS389001DSER', '5')}
    marked_open = []
    y = 50.8
        ...
        for column, component in enumerate(row):
            ...
            for number, _ in part.pins:
                ...
                if net is None or (component.ref, number) in singles:
                    if net is not None and (component.display_value, number) not in allowed_open:
                        raise ValueError(f'unqualified singleton pin {component.ref}.{number}')
    ...
    artifacts = {
        'section.kicad_sch': '\n'.join(drawing) + '\n',
        'rtd-unit.kicad_sym': library,
        'sym-lib-table': '(sym_lib_table (version 7)\n'
            ' (lib (name "RTDUnit")(type "KiCad")'
            '(uri "${KIPRJMOD}/rtd-unit.kicad_sym")(options "")'
            '(descr "Generated standalone RTD source symbols")))\n',
    }
...
if __name__ == '__main__':
    ...
    run(args.source.resolve(), args.output.resolve(), args.receipt.resolve())
