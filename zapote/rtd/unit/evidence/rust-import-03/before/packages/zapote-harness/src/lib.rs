    ...
    let scenarios: Vec<_> = observations
        ...
        .filter_map(|fault| {
            Some(serde_json::json!({
                "name": fault.get("name")?.as_str()?,
                "resistance_ohm": fault.get("resistance_ohm"),
                "conductor_open": fault.get("conductor_open"),
                "rail_loss": fault.get("rail_loss"),
                "expected_class": fault.get("observed_class")?.as_str()?,
                "expected_detected": fault.get("observed_detected")?.as_bool()?
            }))
        })
        .collect();
