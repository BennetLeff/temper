...
/* Board-owned RTD readiness contract. */
extern bool rtd_service_is_ready(void);
...
void transition_to(system_state_t new_state) {
    /* Every route into PAN_DET enables the 5% detection drive. Refuse that
     * transition until the RTD service is ready, including profile and UI
     * callers that bypass the button handler. */
    if (new_state == STATE_PAN_DET && !rtd_service_is_ready()) {
        return;
    }
