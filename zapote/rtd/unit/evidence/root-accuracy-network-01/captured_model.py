...
def solve(case: str, rtd: float, lead: float, c: Corner) -> Result:
    ...
    edge("bias", "sensep", c.rdiag)
    edge("bias", "sensem", c.rdiag)
...
def print_supply_and_transient_bounds() -> None:
    ...
    high_ref_load_max = 1.25 / (5900.0 * (1.0 - 0.001 - 25e-6 * 60.0) +
                                 10000.0 * (1.0 - 0.001 - 25e-6 * 60.0))
    local_known = 3.5e-3 + 2.0 * 65e-6 + 2.0 * 10e-6
    print("supply_budget")
    ...
    print(f"REF2025_VREF_divider_load_uA={(rref_load_max + high_ref_load_max) * 1e6:.3f} "
          "REF2025_IQ_spec_uA=430.000 REF2025_output_limit_mA=20.000")
    print(f"RTD_AVDD_known_local_mA={local_known * 1e3:.3f} "
          "(MAX_active+2xTLV3201+AND/NAND; logic limits are datasheet max)")
