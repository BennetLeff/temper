...
fn check_native_geometry(
    ...
    if input.applicability == "RTDUnit" {
        ...
        for requirement in &input.board.geometry.return_planes {
            ...
            // A filled polygon records area, not a proven connection into the
            // local return corridor. Require measured same-net copper on the
            // plane layer or a same-net via spanning that layer before this
            // role can be considered physically represented.
            let layer = requirement.layer.as_str();
            let has_trace = input
                .board
                .traces
                .iter()
                .any(|trace| trace.net == requirement.net && trace.layer == layer);
            let layer_index = |name: &str| match name {
                "F.Cu" => Some(0_u8),
                "In1.Cu" => Some(1),
                "In2.Cu" => Some(2),
                "B.Cu" => Some(3),
            ...
            let has_via = input.board.vias.iter().any(|via| {
                let Some(target) = layer_index(layer) else {
                    return false;
                };
                let Some(from) = layer_index(&via.from_layer) else {
                    return false;
                };
                let Some(to) = layer_index(&via.to_layer) else {
                    return false;
                };
                via.net == requirement.net && from.min(to) <= target && target <= from.max(to)
            });
            if !has_trace && !has_via {
                gaps.push(format!(
                    "{} return plane {} on {} has no native same-net trace or spanning via",
                    requirement.purpose, requirement.net, requirement.layer
                ));
                findings.push(Finding::indeterminate(
                    "DRC.RTD.NATIVE_GEOMETRY",
                    format!(
                        "filled zone for {} lacks measured copper entry on {}",
                        requirement.net, requirement.layer
                    ),
                    requirement.purpose.clone(),
                ));
