...
CARGO_TARGET_DIR=/private/tmp/zapote-rtd-target cargo run --manifest-path zapote/Cargo.toml -p zapote-harness --example current_sense_input -- \
  ...
  zapote/current-sense/evidence/stackup-fix/native-fixed.json \
...
| Physical stackup | Required actual PCB UTF-8 bytes bound to native board SHA-256; positive copper/core thickness, complete ordered copper census, intervening dielectrics, and total copper + dielectric + mask equal to declared thickness within 0.000001 mm serialization rounding |
...
The current-sense path registers 12 rule families. Rule count is not test count and neither proves universal electrical correctness. Unsupported curved copper or unsupported pad shapes are rejected by the transport/validators rather than silently approximated. Mechanical assembly and thermal/current capability remain outside the completed digital checks.
...
## Physical stackup gate

`zapote-drc::stackup::validate_board` is reusable for any unit. It reads the native S-expression, using a copied Temper parser with local error handling and parser hardening; no Python rule duplicates it. The current-sense normal path runs it unconditionally via `validate_native_export`. Older exports lacking `board_file_utf8` must be regenerated; they now fail rather than silently skip the rule. Tests cover the real zero-core board, missing/tampered board bytes even after rehashing the export, invalid/missing/duplicate thickness, total mismatch, copper census/order, a four-layer stackup, quoted parentheses and excessive nesting.

```sh
CARGO_TARGET_DIR=/private/tmp/zapote-rtd-target cargo run --manifest-path zapote/Cargo.toml -p zapote-harness --example stackup_check -- zapote/current-sense/candidate/section.kicad_pcb
```

This gate checks nominal CAD consistency, not fabrication tolerance, approved material/stackup selection, insulation certification or 3D collision clearance. Multi-sublayer dielectrics are currently rejected rather than approximated. KiCad's stackup total includes mask thickness; see its [physical stackup documentation](https://docs.kicad.org/6.0/en/pcbnew/pcbnew.pdf). The repair's fresh input/report/executables live in `evidence/acceptance-stackup-v2/`. The earlier `evidence/acceptance/` is historical and its original files are archived under `zapote/artifacts/current-sense-before-stackup-fix/`.
