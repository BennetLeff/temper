...
W("")
W("A boost switch fails short (drain-source) while the bus is at 400 Vdc. The")
W("bank discharges back through the boost diode and the fuse toward the failed")
W("switch. This is a **failed-short device** scenario, distinct from an")
W("overload: the drive into the fault is the stored bank energy, not the mains")
W("source, and the loop is the internal discharge path.")
W("")
W("The loop's resistance and inductance are **not known**. They are bounded and")
W("swept instead:")
W("")
...
W("")
W("The lower bounds are the optimistic case for the fuse (lowest impedance,")
W("highest current); the upper bounds are the optimistic case for the time")
W("constant. Section 4 gives the sourced bounds we have since established.")
W("")
_rd = BOUNDS["resistance_decomposition"]
_ld = BOUNDS["inductance_decomposition"]
_sl = BOUNDS["screen_location"]
W("## 4. Loop R and L: the bounds we are supplying")
W("")
W("Decomposed by component, each with a source and an explicit direction — a")
W("bound, not a point estimate. Unsourced components stay null and are not")
W("filled with defaults.")
W("")
W("| Term | Value | Kind / direction |")
W("| --- | ---: | --- |")
W(f"| Copper (geometry-derived, nearest cap, 20 C) | {_rd['copper']['value_ohm_lower'] * 1e3:.2f} mOhm | lower bound (every other term >= 0) |")
W(f"| Fuse (catalogue 11.6 W at 50 A) | {_rd['fuse_resistance']['value_ohm'] * 1e3:.2f} mOhm | hot figure; upper bound at fault onset |")
W(f"| Bank ESR (tan-delta max at 120 Hz) | {_rd['bank_esr']['value_ohm'] * 1e3:.2f} mOhm | upper bound (ESR falls with frequency) |")
W(f"| U9 healthy/on Rds_on (25 C max) | {_rd['u9_healthy_on']['value_ohm_max_25c'] * 1e3:.1f} mOhm | upper bound AT 25 C ONLY - not a short residual |")
W(f"| Copper at 100 C | {_rd['copper']['value_ohm_upper'] * 1e3:.2f} mOhm | upper bound |")
W(f"| **Sourced sub-total of known terms** | **<= {_sl['sourced_box']['R_upper_ohm'] * 1e3:.1f} mOhm** | |")
W("| U9 failed-short residual | **null** | not published |")
W("| U10 failed-short residual | **null** | not published |")
W("")
W(f"- **R: {_sl['sourced_box']['R_lower_ohm'] * 1e3:.2f} mOhm and above, with no closed")
W(f"  upper bound.** A combined failed-short residual above")
W("  **0.4466 ohm** would take R past the 0.64013 ohm screen; nothing")
W("  published bounds that residual.")
W(f"- **L: copper {_ld['copper']['value_h_lower'] * 1e9:.1f} nH to")
W(f"  {_ld['copper']['value_h_upper'] * 1e6:.2f} uH.** The span *is* the return-path")
W("  assumption (return current directly beneath the forward trace, versus")
W("  horizontally offset by the measured 34.5 mm worst case). Bank, fuse and")
W("  package internal inductances are not published and add >= 0.")
W("")
W("**Where this leaves the location: INDETERMINATE.** Against")
W(f"`{{400*L <= R <= 0.64013 ohm}}`, the lower condition needs L <=")
W(f"{_sl['lower_boundary_check']['holds_for_total_L_up_to_h'] * 1e6:.1f} uH (the copper")
W("term is ~9x under that, but internal inductances are unbounded), and the")
W("upper condition passes on every sourced term while the two null residuals")
W("leave it open. **Inside is plausible on all sourced evidence, and neither")
W("boundary is closed.** We are not asserting that the fault lands in the")
W("screened region.")
W("")
...
W("| Q4: a let-through I2t | Compare against a sourced bank/copper withstand I2t — the last gate before coordination can be claimed. |")
W("")
...
W("- Loop R and L bounds: AR-BOUNDS attempt-001, derived from")
