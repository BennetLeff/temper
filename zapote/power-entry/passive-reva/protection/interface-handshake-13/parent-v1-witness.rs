...
#[cfg(test)] mod parent_witness {
 use super::*;
 #[test] fn actual_permit_loss_reuses_session_and_allows_old_restart() {
  let mut r=Receiver::new(PersistentCounter::new(1));
  let c=r.boot().unwrap();r.idle_sample(true,true);r.idle_sample(true,true);
  let mut s=Source::new();assert!(s.fresh_start_intent());
  let req=s.on_challenge(c).unwrap();let ack=r.on_frame(req,true,true).unwrap().unwrap();let start=s.on_ack(ack).unwrap();
  r.on_frame(start,true,true).unwrap();assert!(r.running());
  assert_eq!(r.on_frame(start,false,true),Err(Reject::PermitLow));assert!(!r.running());
  r.idle_sample(true,true);r.idle_sample(true,true);
  r.on_frame(req,true,true).unwrap();r.on_frame(start,true,true).unwrap();
  assert!(r.running(),"witness reproduces unsafe restart, not acceptance");
 }
}
