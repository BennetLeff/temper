...
pub struct Boundary {
    pub now: u64,
    pub source_ok: bool,
    pub reset_conductor: ResetConductor,
    pub hot_health: bool,
...
impl Boundary {
    pub fn new() -> Self {
        Self {
            now: 0,
            // Power-up is fail-closed.  The HOT default-low is represented as
            // an already observed low; software still needs an explicit high
            // qualification before a session can be installed.
            source_ok: false,
            reset_conductor: ResetConductor::Healthy,
            hot_health: true,
            ...
            low_was_observed: true,
            low_captured: true,
            fault_low_observed: false,
        }
    }
    ...
    fn service_edge(&mut self) {
        ...
        match edge {
            ResetEdge::AssertAt(at) if at <= self.now => {
                self.edge = None;
                if !self.low_captured || self.reset_conductor == ResetConductor::FailedHigh {
                    return;
                }
                self.inhibit_latched = true;
                self.low_was_observed = true;
                self.fault_low_observed = true;
                self.low_captured = false;
                self.recovery_ready = false;
                self.gate_latched = false;
                self.fresh_intent = false;
                self.arm_authorized = false;
                // A short captured low still asserts reset.  If the source
                // has already returned high, schedule its later release.
                if self.source_ok && self.reset_conductor == ResetConductor::Healthy {
    ...
    fn settle_current_edge(&mut self) {
        self.service_edge();
    }
    ...
    pub fn source_ok_at(&mut self, t: u64, ok: bool) {
        ...
        self.source_ok = ok;
        if !ok {
            self.low_captured = true;
            self.recovery_ready = false;
            if self.reset_conductor != ResetConductor::FailedHigh {
                if !matches!(self.edge, Some(ResetEdge::AssertAt(_))) {
                    self.edge = Some(ResetEdge::AssertAt(self.now + SOURCE_LOW_TO_INHIBIT));
                }
            } else {
                self.edge = None;
            }
        } else if self.inhibit_latched && self.reset_conductor == ResetConductor::Healthy {
            self.edge = Some(ResetEdge::ReleaseAt(self.now + INHIBIT_RELEASE));
        } else if !self.inhibit_latched
            && self.low_was_observed
            && self.edge.is_none()
            && self.reset_conductor == ResetConductor::Healthy
        {
    ...
    pub fn reset_conductor_at(&mut self, t: u64, conductor: ResetConductor) {
        self.advance_to(t);
        self.reset_conductor = conductor;
        if conductor == ResetConductor::FailedHigh {
            self.edge = None;
        } else if conductor == ResetConductor::Open || !self.source_ok {
            self.low_captured = true;
            self.recovery_ready = false;
            if !matches!(self.edge, Some(ResetEdge::AssertAt(_))) {
                self.edge = Some(ResetEdge::AssertAt(self.now + SOURCE_LOW_TO_INHIBIT));
    ...
    pub fn hot_health_at(&mut self, t: u64, healthy: bool) {
        ...
        if !healthy {
            self.trip_local();
        } else if self.maintained_permit && self.source_ok {
            self.recovery_ready = true;
        }
        self.settle_current_edge();
    }
    ...
    pub fn permit_at(&mut self, t: u64, permit: bool) {
        ...
        if !permit {
            self.trip_local();
        } else if self.hot_health && self.source_ok {
            self.recovery_ready = true;
        }
        self.settle_current_edge();
    }
    ...
    pub fn install_session_at(&mut self, t: u64) -> Option<Session> {
        ...
        if !self.source_ok
            || !self.hot_health
            || !self.maintained_permit
            || self.inhibit_latched
            || !self.recovery_ready
            || self.edge.is_some()
            || self.reset_conductor != ResetConductor::Healthy
            || !matches!(self.decoder, Decoder::Offline)
        {
            return None;
        }
        let session = Session(self.next_session);
        self.next_session = self.next_session.checked_add(1)?;
        if self.last_session.is_some_and(|old| session.0 <= old.0) {
            return None;
        }
    ...
    pub fn fresh_intent_at(&mut self, t: u64) -> Option<u64> {
        self.advance_to(t);
        let Decoder::Idle { .. } = self.decoder else {
            return None;
        };
    ...
    pub fn request_at(&mut self, t: u64, session: Session, intent: u64) -> bool {
        self.advance_to(t);
        let Decoder::Idle { session: expected } = self.decoder else {
            return false;
        };
        if expected != session || !self.fresh_intent {
            return false;
        }
    ...
    pub fn start_at(&mut self, t: u64, start: Start) -> bool {
        ...
        let accepted = match self.decoder {
            Decoder::AwaitStart { session, intent }
                if session == start.session && intent == start.intent && self.arm_authorized =>
            {
    ...
    /// Independent hardware does not assume that a frozen decoder obeys reset.
    /// A real link watchdog must observe the reset and explicitly take the
    /// software state offline before a new session can be installed.
    pub fn decoder_reset_at(&mut self, t: u64) {
        self.advance_to(t);
...
fn arm(b: &mut Boundary, t: u64) -> Start {
    // Explicitly qualify the powered-up default-low path before installing a
    // session; Boundary::new() itself is fail-closed.
    b.source_ok_at(t, true);
...
#[cfg(test)]
fn qualified(b: &mut Boundary, t: u64) -> Session {
    b.source_ok_at(t, true);
    b.install_session_at(t).expect("session qualification")
}
...
fn main() {
    ...
    csv_case(
        "source_loss_latency",
        !b.output_enabled() && b.now - 4 == CONTRACT.source_low_to_inhibit,
        "2_ticks",
    );

    let mut b = Boundary::new();
    let start = arm(&mut b, 0);
    b.decoder = Decoder::AwaitStart {
        session: start.session,
        intent: start.intent,
    };
    b.source_ok_at(4, false);
    let pulse = b.start_at(5, start);
    let enabled_ticks = b.now;
    b.advance_to(6);
    csv_case(
        "inflight_pulse_bound",
        pulse && enabled_ticks == 5 && !b.output_enabled(),
        "1_tick_example",
    );
    ...
    csv_case(
        "same_tick_inflight_start",
        pulse && !b.output_enabled(),
        "start_then_inhibit",
    );
    ...
    b.advance_to(9);
    b.decoder_reset_at(9);
    csv_case(
        "held_stuck_start",
        !b.start_at(10, stale) && b.install_session_at(10).is_some(),
    ...
    let _ = arm(&mut b, 0);
    b.source_ok_at(4, false);
    b.reset_conductor_at(4, ResetConductor::FailedHigh);
    b.advance_to(20);
    csv_case(
        "reset_conductor_fails_high",
        b.inhibit_latched == false,
        "known_single_fault_gap",
    );
    ...
    csv_case(
        "hot_health_independent",
        !b.output_enabled() && b.decoder_running(),
        "immediate_trip_decoder_frozen",
    );
...
mod tests {
    ...
    #[test]
    fn powered_boot_starts_fail_closed_until_explicit_high_qualification() {
        let mut b = Boundary::new();
        assert!(!b.output_enabled());
        assert!(b.install_session_at(0).is_none());
        b.source_ok_at(0, true);
        assert!(b.install_session_at(0).is_some());
    }
    ...
    #[test]
    fn repeated_low_does_not_postpone_first_assertion() {
        let mut b = Boundary::new();
        let start = arm(&mut b, 0);
        b.start_at(3, start);
        b.source_ok_at(4, false);
    ...
    #[test]
    fn captured_short_low_still_asserts_reset() {
        let mut b = Boundary::new();
        let start = arm(&mut b, 0);
        b.start_at(3, start);
        b.source_ok_at(4, false);
        b.source_ok_at(5, true); // shorter than the 2-tick propagation bound
        assert!(b.output_enabled());
        b.advance_to(6);
        assert!(
            !b.output_enabled(),
            "captured low is stretched until assertion"
        );
    }

    #[test]
    fn stale_start_after_hardware_trip_cannot_rearm_frozen_await_state() {
        let mut b = Boundary::new();
        let stale = arm(&mut b, 0); // AwaitStart, no START yet
        b.source_ok_at(4, false);
        b.advance_to(6);
        b.source_ok_at(7, true);
        b.advance_to(9);
        assert!(!b.start_at(10, stale));
        b.decoder_reset_at(10);
        assert!(b.install_session_at(10).is_some());
    }

    #[test]
    fn start_at_five_has_one_tick_ordered_inflight_window_before_inhibit_six() {
        let mut b = Boundary::new();
        let start = arm(&mut b, 0);
        b.decoder = Decoder::AwaitStart {
            session: start.session,
    ...
    fn same_timestamp_start_then_reset_is_bounded_pulse() {
        ...
        assert!(
            b.decoder_running(),
            "hardware trip does not assume software reset"
        );
    }
    ...
    fn resetting_each_phase_is_fail_closed() {
        for phase in 0..4 {
            let mut b = Boundary::new();
            match phase {
                // Offline, Idle, AwaitStart and Running are each exercised.
                0 => {
                    b.source_ok_at(0, true);
                    b.source_ok_at(1, false);
                }
                1 => {
                    let _ = qualified(&mut b, 0);
                    b.source_ok_at(1, false);
                }
                2 => {
                    let session = qualified(&mut b, 0);
                    let intent = b.fresh_intent_at(1).unwrap();
                    assert!(b.request_at(2, session, intent));
                    b.source_ok_at(2, false);
                }
                _ => {
                    let start = arm(&mut b, 0);
                    assert!(b.start_at(3, start));
    ...
    fn reset_conductor_fail_high_is_explicit_single_fault_gap() {
        ...
        assert!(
            b.output_enabled(),
            "one failed-high reset path has no redundancy"
        );
        assert!(!b.fault_low_observed);
    }

    #[test]
    fn reset_conductor_open_trips_through_default_low() {
        let mut b = Boundary::new();
        let start = arm(&mut b, 0);
        b.start_at(3, start);
        ...
        assert!(
            !b.output_enabled(),
            "default-low conductor opening must assert HOT reset"
        );
        assert!(b.fault_low_observed);
    }
    ...
    fn hot_health_and_permit_trip_without_command_frame() {
        ...
        b.decoder_reset_at(5);
        let _ = b
            .install_session_at(5)
            .expect("requalification after local fault");
        b.permit_at(6, false);
        assert!(!b.output_enabled());
    }
}
