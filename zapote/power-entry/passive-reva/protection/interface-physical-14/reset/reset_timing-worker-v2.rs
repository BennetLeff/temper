...
impl Boundary {
    ...
    fn service_edge(&mut self) {
        ...
        match edge {
            ResetEdge::AssertAt(at) if at <= self.now => {
                self.edge = None;
                if !self.low_captured || self.reset_conductor != ResetConductor::Healthy { return; }
            ...
            ResetEdge::ReleaseAt(at) if at <= self.now => {
                self.edge = None;
                if !self.source_ok || self.reset_conductor != ResetConductor::Healthy { return; }
    ...
    pub fn source_ok_at(&mut self, t: u64, ok: bool) {
        ...
        if !ok {
            ...
            self.recovery_ready = false;
            if self.reset_conductor == ResetConductor::Healthy {
            ...
            self.edge = Some(ResetEdge::ReleaseAt(self.now + INHIBIT_RELEASE));
        } else if !self.inhibit_latched && self.low_was_observed {
    ...
    pub fn reset_conductor_at(&mut self, t: u64, conductor: ResetConductor) {
        ...
        if conductor != ResetConductor::Healthy { self.edge = None; }
        else if !self.source_ok { self.edge = Some(ResetEdge::AssertAt(self.now + SOURCE_LOW_TO_INHIBIT)); }
        else if self.inhibit_latched { self.edge = Some(ResetEdge::ReleaseAt(self.now + INHIBIT_RELEASE)); }
        self.settle_current_edge();
    }
    ...
    pub fn install_session_at(&mut self, t: u64) -> Option<Session> {
        ...
        if !self.source_ok || !self.hot_health || !self.maintained_permit
            || self.inhibit_latched || !self.recovery_ready
            || !matches!(self.decoder, Decoder::Offline) { return None; }
        ...
        let accepted = match self.decoder {
            Decoder::AwaitStart { session, intent }
                if session == start.session && intent == start.intent => {
                    if !self.arm_authorized { return false; }
...
fn main() {
    ...
    b.advance_to(6);
    csv_case("inflight_pulse_bound", pulse && enabled_ticks == 5 && !b.output_enabled(), "1_tick_example");
    ...
    csv_case("hot_health_independent", !b.output_enabled() && b.decoder_running(), "immediate_trip_decoder_frozen");
...
mod tests {
    ...
    fn captured_short_low_still_asserts_reset() {
        ...
        assert!(b.output_enabled());
        b.advance_to(6);
        assert!(!b.output_enabled(), "captured low is stretched until assertion");
    }
    ...
    fn same_timestamp_start_then_reset_is_bounded_pulse() {
        ...
        assert!(!b.output_enabled());
        assert!(b.decoder_running(), "hardware trip does not assume software reset");
    }
    ...
    fn resetting_each_phase_is_fail_closed() {
        for phase in 0..4 {
            ...
            match phase {
                ...
                0 => { b.source_ok_at(0, true); b.source_ok_at(1, false); }
                1 => { let _ = qualified(&mut b, 0); b.source_ok_at(1, false); }
                2 => {
    ...
    #[test]
    fn reset_conductor_open_is_distinct_default_low_gap() {
        let mut b = Boundary::new();
        let start = arm(&mut b, 0);
        b.start_at(3, start);
        ...
        b.source_ok_at(4, false);
        b.advance_to(20);
        assert!(b.output_enabled(), "open conductor has no asserted HOT reset");
        assert!(!b.fault_low_observed);
    }
