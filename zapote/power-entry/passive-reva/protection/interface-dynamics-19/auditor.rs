...
fn assert_contracts(name: &str, g: &BTreeMap<Pin, String>) -> Result<(), String> {
    ...
    if name == "clamp" {
        ...
        expect("C3", "1", "CG")?;
        same(&[("Q1", "3"), ("R1", "1"), ("U1", "3")])?;
        // ADI Figure5: the capacitor branch joins the controller side of R6.
        same(&[("Q1", "1"), ("R6", "2")])?;
        same(&[("U1", "4"), ("R6", "1"), ("R7", "1"), ("D1", "2")])?;
        same(&[("R7", "2"), ("D1", "1"), ("C3", "1")])?;
...
fn main() {
    ...
    if args.iter().any(|a| a == "--self-test") {
        ...
        if dir.join("clamp.net").exists() {
            ...
            let mut old_branch = clamp.clone();
            for (reference, pin) in [("R7", "1"), ("D1", "2")] {
                old_branch.insert(
                    Pin {
            ...
            assert!(
                assert_contracts("clamp", &old_branch).is_err(),
                "revision18 MOSFET-side branch must fail independently of expected TSV"
            );
            ...
            println!(
                "PASS live mutation tests: revision18 branch, clamp diode endpoint and HOT D1 fixed-high rejected"
            );
        }
        return;
    }
