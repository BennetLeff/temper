...
| R5 | Local UCC27624 enable inhibit and HOT rail interlocks in joined source and audit. | Driver-selection blocker: TI gives no guaranteed ENA pull-up minimum or maximum sink current across the joined AUX range, so the present shunt has no worst-case OFF proof. Its 1.1 mA maximum *total* disabled VDD current at the 12 V table fixture does not qualify the present 15 V-class rail. The three judged replacement sketches and subsequent PWM-input screens in `DRIVER-TOPOLOGY-BAKEOFF.md` remain unselected. Partial-power corner and native copper/voltage checks OPEN. |
