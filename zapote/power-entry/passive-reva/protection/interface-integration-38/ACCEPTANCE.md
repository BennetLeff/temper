...
| Rev38 source-task target entry | `firmware/main/rev38_source_test_idf.c` SHA-256 `5fb7af8681470ded15ce9ed9099028451bd41a2826e0910af3ae9080ff6231aa`; `firmware/main/CMakeLists.txt` SHA-256 `15f9eed25f55086d7616ed579b6134f810e8c50788986497aa72c4569bdd571e`; ESP-IDF target link NOT RUN |
...
| ESP target integration | **OPEN** | `TEMPER_REV38_TEST_IMAGE=ON` now selects an entry that starts the actual source task and retains zero timing lockout. Docker/ESP-IDF is unavailable locally, so this new entry has no target link receipt. The diagnostic image never runs the task; the cooker production image still has unresolved hooks. Target pin/reset captures remain NOT RUN. |
