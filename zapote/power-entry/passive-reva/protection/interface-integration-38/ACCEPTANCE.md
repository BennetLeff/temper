...
replacement sketches and the separate judge's unresolved selection.
...
| R5 | Local UCC27624 enable inhibit and HOT rail interlocks in joined source and audit. | Driver-selection blocker: TI publishes no guaranteed ENA pull-up minimum or maximum sink current, so the present shunt has no worst-case OFF proof. The three replacement sketches in `DRIVER-TOPOLOGY-BAKEOFF.md` also remain unselected after independent judgment. Partial-power corner and native copper/voltage checks OPEN. |
