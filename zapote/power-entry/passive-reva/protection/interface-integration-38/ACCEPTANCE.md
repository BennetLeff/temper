...
| R5 | Local UCC27624 enable inhibit and HOT rail interlocks in joined source and audit. | Driver-selection blocker: TI publishes no guaranteed ENA pull-up minimum or maximum sink current, so the present shunt has no worst-case OFF proof. Partial-power corner and native copper/voltage checks OPEN; see `gate-enable-corners.md`. |
