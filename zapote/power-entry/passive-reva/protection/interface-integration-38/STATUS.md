...
| U6 | Source core/runtime and new ESP32 adapter host tests pass. The adapter assigns direct STOP, heartbeat, permit set, safety sample, UART, START, and I²C pads, and orders expander latch/polarity/direction writes before sampling P3–P7. Runtime reserves the configured sample-to-edge bound before timed control and WDI pulses; a near-deadline negative test fails against the previous runtime. The complete firmware host build and all 16 CTest entries now pass, including the three power-entry targets. One `app_main` source task now owns the candidate ESP-IDF GPIO/I²C0/UART1 binding and samples completed control-task epochs. An IDF v5.3.6 ESP32-S3 compile-only check passed seven Rev38 application objects; `ESP-TARGET-COMPILE.md` records the full-image configuration blockers and limits. The task remains locked out with zero target bounds and unqualified UART final-bit, reset feed-tail and independent monitor progress; no Rev38 WDI or START is enabled. | Resolve the full IDF project build blockers, then build and run on the target; prove sole UART ownership and final bit completion, target-verified expander P1 and WDI pre-edge bounds, independent monitor progress, I²C read age, reset/boot/partial-power default states and physical reset-to-off. U6 remains OPEN. |
| U7 | No Rev38 native candidate exists. Package screens and closeout reduced the joined BOM from 127 to one physical footprint placeholder: off-board F2 `U226`. `source-build-01` freezes the current 299-reference Atopile source and resolved export. The strict native probe rejects F2 at footprint vendoring, as recorded in `NATIVE-BUILD.md`. `F2-BOARD-INTERFACE.md` records a candidate two-potential Phoenix terminal and drawing-derived pad centers, but its manufacturer drill/pin records disagree; no footprint is assigned. `bench-capture.md` maps supply, fault, reset and timing measurements to physical nodes; all captures are NOT RUN. | Reconcile F2 terminal drilling and fault-current/thermal application, join missing SELV/reset/interlock producers, create reviewed poses and outline, then generate native schematic and PCB and review source/native parity, ERC/DRC, stackup and unit gates. Physical captures remain NOT RUN. |
