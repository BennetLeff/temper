...
| U4 | `power_entry_integrated_38.ato` compiles source authority, receiver, HOT TPS3431 watchdog, and a partial UCC27624/STW gate path. It joins source PERMIT, health, STOP, physical HOT PERMIT feedback and retained HOT SESSION feedback across the assigned isolators. The driver has a hardware permission AND, AUX-biased ENA shunt, local low defaults, gate resistor and STW switch pins. HOT WDI and WDO now join AVR PD6 and the retained trip fan-in. `audit.rs` passes 52 standalone/joined pin and mutation tests, including ENA floating, watchdog WDI/WDO disconnects, abort bypass, feedback swap, and isolation bridge. `gate-enable-corners.md` and `HOT-WATCHDOG.md` record still-open electrical/timing proofs. | Prove coincident trip/reset and setup/hold corners; join actual HOT F2, rail and AUX producers, UCC28180 PWM, PFC/reservoir, and electrical corner analysis. The LVC output during intermediate HOT rail collapse, PMBT3904 saturation across temperature, UCC EN internal pull-up maximum, and AUX transient range remain unresolved. Source reset-good, interlock, STOP, heartbeat and protocol/relay GPIO producers remain external. The joined file is still partial and does not pass U4. |
...
Immediate construction order: finish U4's actual HOT fault/rail,
PWM and AUX producer paths; bind those pins to U5's AVR adapter and U6's source driver;
