...
| U6 | Source core/runtime and new ESP32 adapter host tests pass. The adapter assigns direct STOP, heartbeat, permit set, safety sample, UART, START, and I²C pads, and orders expander latch/polarity/direction writes before sampling P3–P7. A candidate ESP-IDF GPIO/I²C0/UART1 binding exists; neither IDF target build nor `app_main` integration has run. | Prove sole UART ownership and final bit completion, expander P1 pre-edge deadline, paired task epoch ownership, I²C read age, WDI feed tail, reset/boot/partial-power default states and physical reset-to-off; test on target. U6 remains OPEN. |
