...
| U4 | `receiver_isolation.ato` compiles with exact AVR/ISO7741F/ISO7742F assignments and distinct SN74LV221A-Q1 raw reset channels. `audit.rs` passes nine tests, including deliberate pin/identity/RC miswires. `gate-enable-corners.md` records an unresolved driver default-off condition. | Build **one joined** `power_entry_integrated_38.ato` with retained HOT/source memories, fault-dominant qualified history resets, actual F2/PFC and reservoir, watchdogs, supervisors, UCC27624/STW, and electrical corner analysis. The current fixture is partial. |
