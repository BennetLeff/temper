...
| U4 | `receiver_isolation.ato` compiles with exact AVR/ISO7741F/ISO7742F assignments, distinct SN74LV221A-Q1 reset channels, SN74HCS74 preparation-abort, physical-PERMIT history, post-trip disarm, SESSION and RUN memories, and an SN74HCS21 six-input trip fan-in. The permit-history D path requires disarm Q high, RUN/PERMIT low, healthy trip fan-in and asserted receiver abort. SESSION and RUN use raw request clocks and asynchronous clears; a physical PERMIT loss after seen-high clears SESSION. `audit.rs` passes 27 tests, including deliberate clear/clock/D/fan-in miswires. `gate-enable-corners.md` records an unresolved driver default-off condition. | Prove history-reset coincident-trip and setup/hold corners, join actual HOT F2, rail, watchdog and SELV source producers, then build **one joined** `power_entry_integrated_38.ato` with source memories, actual F2/PFC and reservoir, supervisors, UCC27624/STW, and electrical corner analysis. The current fixture is partial. |
