...
| U4 | `receiver_isolation.ato` compiles with exact AVR/ISO7741F/ISO7742F assignments, distinct SN74LV221A-Q1 reset channels, an SN74HCS74 preparation-abort memory clocked by one finite reset edge, and a default-low PA6 attempt-valid output. `audit.rs` passes 13 tests, including deliberate pin/identity/RC/preset/clock/default miswires. `gate-enable-corners.md` records an unresolved driver default-off condition. | Join PA6 and the real detector/rail/source/STOP producers to `HOT_PREP_TRIP_OK`, then build **one joined** `power_entry_integrated_38.ato` with the remaining HOT/source memories, fault-dominant qualified history reset, actual F2/PFC and reservoir, watchdogs, supervisors, UCC27624/STW, and electrical corner analysis. The current fixture is partial. |
