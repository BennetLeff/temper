...
| U4 | `power_entry_integrated_38.ato` compiles source authority, receiver, and a partial UCC27624/STW gate path. It joins source PERMIT, health, STOP, physical HOT PERMIT feedback and retained HOT SESSION feedback across the assigned isolators. The driver has a hardware permission AND, AUX-biased ENA shunt, local low defaults, gate resistor and STW switch pins. `audit.rs` passes 46 standalone/joined pin and mutation tests, including ENA floating, abort bypass, feedback swap, and isolation bridge. `gate-enable-corners.md` records the still-open analog OFF proof. | Prove coincident trip/reset and setup/hold corners; join actual HOT F2, rail, watchdog and AUX producers, UCC28180 PWM, PFC/reservoir, and electrical corner analysis. The LVC output during intermediate HOT rail collapse, PMBT3904 saturation across temperature, UCC EN internal pull-up maximum, and AUX transient range remain unresolved. Source reset-good, interlock, STOP, heartbeat and protocol/relay GPIO producers remain external. The joined file is still partial and does not pass U4. |
...
Immediate construction order: finish U4's actual HOT fault/rail/watchdog,
