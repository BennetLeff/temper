...
| U7 | No accepted Rev38 native section exists. `source-build-06` freezes **296 references**, including separate Würth F2 studs, the off-board F2 declaration, the DWW isolator pair and the 16-contact port. Rust audit passes **147/147** tests; the cooker mate receipt audit passes. `placement-review/04/` saves a 295-footprint diagnostic that moves four SELV-owned receiver parts out of the HOT region relative to `/03`. Exact 1,052-edge source/native pad parity, zero ERC, zero non-routing DRC, zero schematic parity issues and zero library-footprint mismatches pass. Its provisional six-layer 1.8 mm stackup passes the Rust CAD consistency gate and is manifest-hashed; this is not a fabricator or insulation approval. KiCad caps the unrouted board at 499 unconnected items. No routes, zones or slots exist. `INSULATION-COVERAGE.md` inventories all 30,135 named-net pairs; all release rules remain missing. The provisional 16 mm pad screen falls from 663 to 104 findings, all within the two DWW footprints, but still fails at their 15.2 mm opposed pads. `DWW-BARRIER-FEASIBILITY.md` defines a slot specimen and unresolved air/package/PD3 paths. Physical captures remain NOT RUN. | Review all poses and mechanical access, complete routing and select a voltage/insulation schedule, then encode every required net-pair rule and run the coverage gate on a saved routed section board. Qualify SELV supply/connector, F2 studs, DWW barrier, current/thermal construction and physical captures separately. U7 remains OPEN. |
...
- **U7:** Current `source-build-06` has 296 references. Its initial native
  preflight passed source, footprint and pin-map conversion; the user-approved
  planning `outline.json` is 360 × 250 mm. The
  ...
  1,052-edge parity and a passing provisional CAD stackup gate. The subsequent
  `/04` moves four SELV-owned receiver parts out of the HOT region; its saved
  native board and source/native pad parity pass, and the provisional 16 mm
  cross-domain pad findings fall from 663 to 104. All 104 are intrinsic to
  the two DWW footprints, whose 15.2 mm opposed-pad gap remains below the
  provisional board-surface screen. The `/04` board remains unrouted, with
  499 capped unconnected items. `DWW-BARRIER-FEASIBILITY.md` proposes a
  slot measurement specimen but leaves package PD3 use and installed air
  clearance unresolved. Thermal, assembly and isolation qualification remain
  open.
