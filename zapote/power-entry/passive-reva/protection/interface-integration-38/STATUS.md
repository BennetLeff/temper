...
surface; the selected ISO774x DW's intrinsic external path is >8 mm. The
...
| U6 | Source core/runtime and new ESP32 adapter host tests pass. The adapter assigns direct STOP, heartbeat, permit set, safety sample, UART, START, and I²C pads, and orders expander latch/polarity/direction writes before sampling P3–P7. Runtime reserves the configured sample-to-edge bound before timed control and WDI pulses; a near-deadline negative test fails against the previous runtime. The complete firmware host build and all 17 CTest entries pass. `rtd_service_sample_status()` now publishes a cross-task atomic healthy-conversion generation and readiness; the MAX31865 host tests cover repeated, stalled and faulted conversions after a valid sample. The heatsink NTC host guard now models the schematic's 100 kΩ/B4190 Vishay sensor and 10 kΩ divider, with 9 focused tests; no production ADC calibration or read hook is credited. One `app_main` source task owns the candidate ESP-IDF GPIO/I²C0/UART1 binding and samples completed control-task epochs. A default-on **diagnostic lockout** ESP-IDF v5.3.6 image linked the cooker core before the latest RTD and control edits; it held STOP low, asserted the runaway cut and never started cooker or Rev38 authorization tasks. The last production link failed on 32 unresolved cooker hooks; two now have source definitions but neither image has been rebuilt. `ESP-TARGET-COMPILE.md` records the evidence limits. The source task remains locked out with zero target bounds and unqualified UART final-bit, reset feed-tail and independent monitor progress; no Rev38 WDI or START is enabled. | Supply real production implementations and source registration for the remaining cooker hooks, then link and run the target image; define idle/heating monitor checks and sensor-age deadlines before crediting progress (`run_safety_check()` requires PLL lock even at idle), and prove sole UART ownership and final bit completion, target-verified expander P1 and WDI pre-edge bounds, I²C read age, reset/boot/partial-power default states and physical reset-to-off. U6 remains OPEN. |
| U7 | No Rev38 native candidate exists. The joined source separates off-board F2 `U226` from four-pin Phoenix terminal candidate `U227`; both retain unresolved footprint keys. `source-build-04` freezes the 295-reference Atopile source and resolved export, including the Rev38-side Molex header and no second ESP. The strict Rust bridge checks an exact assembly-only F2 declaration against that full source and keeps its identity in the receipt. Six focused tests pass. The earlier source-build-03 native probe passes the off-board fuse projection and stops on the board terminal footprint placeholder, as recorded in `NATIVE-BUILD.md`. `F2-BOARD-INTERFACE.md` records the terminal pin grouping, drawing-derived pad centers, and a Phoenix-linked SamacSys archive comparison; the selected 1017526 drill/pin records still disagree, and the alternate 1017531 archive has an inconsistent body outline. No terminal footprint is assigned. The cooker mate now has its own frozen 189-reference `cooker-source-02` export, including the existing ESP/SELV rail and reset/interlock producers. The two-source 16-contact audit passes with swap/open mutations; `COOKER-ASSEMBLY-SOURCE.md` records the narrow digital evidence. The derivative now joins cooker ESP ground pads 1/40/41 on KiCad’s stock land pattern; two canonical F1/NTC footprints and native placement remain open. Neither board nor physical harness exists. `bench-capture.md` maps supply, fault, reset and timing measurements to physical nodes; all captures are NOT RUN. | Resolve the insulation construction in `INSULATION-BASIS.md`, reconcile terminal drilling and fault-current/thermal application, define the SELV supply/connector interface, create the native Rev38 section board with reviewed poses and outline, then review source/native parity, ERC/DRC, stackup and unit gates. Physical captures remain NOT RUN. |
...
the actual ESP Rev38 service under zero timing lockout; see
`ESP-TARGET-COMPILE.md`. The default diagnostic image still does not run
that service. The new entry has no ESP-IDF target link receipt because the
local Docker daemon is unavailable, so U6 digital acceptance remains OPEN.
...
return type. These changed target bytes have only host evidence: Docker's
daemon socket is still absent, so no new ESP32-S3 image has been built.
