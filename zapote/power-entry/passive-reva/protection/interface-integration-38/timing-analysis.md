...
| Source preparation edge timing | Physical P0 challenge and P1 history-reset request edges, I²C SDA/SCL transactions and acknowledgements, STOP_N, local permit and HOT feedback, and receiver abort. | Bound the time before the P1 positive edge, including prior write/readback transactions. A post-edge deadline check cannot retract a late pulse; the candidate 5 ms transaction timeout is not a measured edge bound. |
