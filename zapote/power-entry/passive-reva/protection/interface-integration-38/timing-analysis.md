...
| AC entry and precharge | Rev38 now joins a Schurter `0031.2510` F1 holder, TDK `B82726S2163N030` CMC, `SL32 10015` NTC, `RT33K012` bypass relay, X2/MOV and one HOT0-to-PE Y1 capacitor to the bridge. The receiver PA2/32 drives the relay switch, with a local gate pull-down and coil flyback diode. A 20 A Class CC fuse and off-board block are now nominated for review, but are not in the joined source. | Join and electrically qualify the actual fuse/block/harness and protected AUX source; derive inrush/precharge/relay timing, repeated-start thermal behavior, F1/F2 clearing and MOV coordination. The relay cannot be treated as a fault-current interrupter. See `AC-INPUT.md` and `F1-SCREEN.md`. |
