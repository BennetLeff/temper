...
//! the native decoder, maintained normalizer, diagnostic metrics, and checker.
...
fn main_run(mut c: Config) -> Result<(), String> {
    ...
    fs::write(c.output.join("source-identity.json"), format!("{{\n  \"deck_sha256\": \"{deck_hash}\",\n  \"includes_sha256\": \"{inc_hash}\",\n  \"tracked_sha256\": \"{}\",\n  \"normalize_sha256\": \"{}\",\n  \"checker_sha256\": \"{}\",\n  \"decoder_sha256\": \"{}\",\n  \"event_metrics_sha256\": \"{}\",\n  \"event_audit_sha256\": \"{}\",\n  \"baseline\": \"{}\",\n  \"selected_cases\": {},\n  \"selection_mode\": \"{}\",\n  \"tracked_cli_args\": {:?},\n  \"first_invalid_snapshot\": {},\n  \"compressor\": {{\"path\": \"{}\", \"sha256\": \"{}\", \"threads\": {}, \"format\": \"{}\"}},\n  \"policy\": \"complete_review_pending\"\n}}\n", sha256_file(&c.tracked)?, sha256_file(&c.normalize)?, sha256_file(&c.checker)?, sha256_file(&c.decoder)?, sha256_file(&c.event_metrics)?, sha256_file(&c.event_audit)?, c.baseline.display(), json_string_array(&selected_ids), if c.selected_cases.is_some() { "explicit" } else { "default_all" }, tracked_args(c.wall_limit, c.end_s, c.first_invalid_snapshot), c.first_invalid_snapshot, compressor.path.display(), sha256_file(&compressor.path)?, compressor.threads, if compressor.pigz { "gzip-compatible-pigz" } else { "gzip" })).map_err(|e| e.to_string())?;
    ...
    while !queue.is_empty() || !active.is_empty() {
        ...
        while active.len() < c.workers {
            ...
            let compressor_cmd = compressor_shell_command(&compressor, &fifo, &dir.join("raw.trace.raw.gz"));
...
fn self_test() -> Result<(), String> {
    ...
    if four.len() != 5 || four[4] != "first-invalid.tsv" || five != four { return err("self-test native five-argument CLI protocol mismatch"); }
