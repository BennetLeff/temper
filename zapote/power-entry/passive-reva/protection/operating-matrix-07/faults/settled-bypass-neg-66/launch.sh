#!/bin/bash
# BYPASS-NEG launch wrapper.  Preparation only: the parent must run this after
...
run_root="$PWD/faults/settled-bypass-neg-66"
src="$PWD/faults/settled-compact-prep-24/prepared/BYPASS-NEG"
out="$run_root/full-BYPASS-NEG"
required_kib=$((16*1024*1024)) # 10 GiB runtime floor + 6 GiB archive reserve

if [ ! -d "$src" ]; then echo "prepared BYPASS-NEG source is missing" >&2; exit 77; fi
if [ -e "$out" ]; then echo "refusing existing BYPASS-NEG output" >&2; exit 77; fi
...
check_solver_inventory() {
  ...
  if [ -n "$solver_rows" ]; then
    echo "a known full solver/campaign runner is live; refusing BYPASS-NEG:" >&2
    printf '%s\n' "$solver_rows" >&2
    return 77
  fi
  return 0
...
verify_source() {
  ...
  for name in \
    ...
    case "$name" in
      case.cir) expected=c6ea3dca688918137282ed3fd3d0ffa17c2b740d68ab52bab306ee3e8b228180 ;;
      manifest.json) expected=4066ac5bf595b4aaca1b99c79035b5729d20bb217ffe1e5f3a4b6ca6a0003e01 ;;
      ...
      protection.inc) expected=d9b499bbba3f2636ac926a3c8782943d3144798cfe4faa765ceaf7774628a025 ;;
...
verify_source || exit $?
if ! mkdir "$out"; then echo "cannot create fresh BYPASS-NEG output" >&2; exit 77; fi
...
verify_source || exit $?
for source_file in case.cir manifest.json cold.cir ucc28180-pwm-latch.inc protection.inc standby.inc clamp.inc authored_logic_hysteretic.inc; do
  cmp -s "$src/$source_file" "$out/$source_file" || exit 77
done
check_space || exit $?
check_solver_inventory || exit $?
...
/private/tmp/matrix07-fault-runner45-parent \
 ...
 .662 bypass-neg .6541666666667 .002 .6541666666667 .002 .000002 .000000025 3600 900 --bypass \
