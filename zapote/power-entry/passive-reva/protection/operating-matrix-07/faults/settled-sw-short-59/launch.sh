#!/bin/bash
# SW-SHORT launch wrapper.  Preparation only: the parent must run this after
...
if [ ! -d "$src" ]; then echo "prepared SW-SHORT source is missing" >&2; exit 77; fi
if [ -e "$out" ]; then echo "refusing existing SW-SHORT output" >&2; exit 77; fi
...
check_solver_inventory() {
  ...
  if [ -n "$solver_rows" ]; then
    echo "a known full solver/campaign runner is live; refusing SW-SHORT:" >&2
    printf '%s\n' "$solver_rows" >&2
    return 77
  fi
  return 0
...
verify_source || exit $?
if ! mkdir "$out"; then echo "cannot create fresh SW-SHORT output" >&2; exit 77; fi
...
/private/tmp/matrix07-fault-runner45-parent \
 ...
 .662 switch-short .6541666666667 .002 .6541666666667 .002 .000002 .000000025 3600 900 \
