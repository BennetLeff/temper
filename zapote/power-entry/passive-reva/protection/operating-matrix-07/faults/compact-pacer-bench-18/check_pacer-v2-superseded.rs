...
fn main() -> Result<(), String> {
    ...
    for i in 0..np {
        ...
        if t >= td - tol_t && t <= end + tol_t {
            ...
            let k=((t-td)/25e-9).round(); if k >= 0.0 && (td+k*25e-9-end).abs() <= tol_t && (t-(td+k*25e-9)).abs() <= tol_t { corner_hits += 1; }
        }
        prev=Some(t); last=t;
    }
    ...
    println!("rows={np} first={first:.17e} endpoint={last:.17e} first_value={first_value:.17e} pre_delay_range={min_pre:.17e}..{max_pre:.17e} active_range={min_active:.17e}..{max_active:.17e} max_gap={max_gap:.17e} active_max_gap={max_active_gap:.17e} active_gaps={active_gaps} active_oversize={active_oversize} pre_delay_bad={pre_delay_bad} max_tri_error={max_tri_error:.17e} expected_corners={expected_corners} corner_hits={corner_hits} near_zero={near_zero} near_one={near_one}");
    if active_oversize != 0 || pre_delay_bad != 0 || max_tri_error > tol_v || corner_hits != expected_corners || near_zero == 0 || near_one == 0 { return Err("pacer checks failed".into()); }
    Ok(())
}
