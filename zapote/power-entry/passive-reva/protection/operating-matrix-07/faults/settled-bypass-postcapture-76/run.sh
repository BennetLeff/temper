#!/bin/bash
# Fixed BYPASS-NEG saved-archive analysis. No solver and no circuit acceptance.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"
PACKET="$ROOT/faults/settled-bypass-postcapture-76"
RUNROOT="$ROOT/faults/settled-bypass-neg-66"
CASE="$RUNROOT/full-BYPASS-NEG"
OUT="$PACKET/full-BYPASS-NEG"
...
assert digest(case/'manifest.json')=='4066ac5bf595b4aaca1b99c79035b5729d20bb217ffe1e5f3a4b6ca6a0003e01'
m=json.loads((case/'manifest.json').read_text())
assert m['case']=='BYPASS-NEG' and m['t_fault_s']==.6541666666667 and m['tstop_s']==.662
assert m['runner_kind']=='bypass-neg' and m['validator_kind']=='f2-open'
params=json.loads((case/'run-parameters.json').read_text())
assert params == {'tstop_s':.662,'kind':'bypass-neg','mutation_s':.6541666666667,'event_window_s':.002,'expected_fault_s':.6541666666667,'observation_s':.002,'turnoff_s':.000002,'max_gap_s':.000000025,'wall_seconds':3600,'export_timeout_s':900,'bypass':True}
...
print(json.dumps({'status':'COMPLETE_CAPTURE_BOUND_FOR_ANALYSIS','accepted':False,'runner_exit':int(runner),'capture_stage1':stage,'points':meta['points'],'raw_sha256':raw_hash,'raw_bytes':raw.stat().st_size,'runner_result':result,'phase_interval_s':[.65,.6583333333333],'note':'Terminal electrical/checker failure does not prevent diagnosis of complete raw capture; it is not waived.'},indent=2))
PY
...
OBSERVE=/private/tmp/matrix07-bypass55-parent
...
assert s['status']=='OK' and s['full_rows']==pre['points'] and abs(s['last_time_s']-.662)<=1e-9
...
"$PIGZ" -dc "$RAW" 2> "$OUT/phase-pigz.stderr" |
 ...
 "$PHASE" --end-s .662 --expected-event-s .6541666666667 --local-start-s .65 --local-end-s .6583333333333 --kind crest --tolerance .01 > "$OUT/phase.json" 2> "$OUT/phase.stderr"
codes=("${PIPESTATUS[@]}"); set -e
record_codes phase "${codes[@]}"
check_space
set +e
"$PIGZ" -dc "$RAW" 2> "$OUT/observe-pigz.stderr" |
 "$DECODER" --schema fault42 --byte-order little 2> "$OUT/observe-decoder.stderr" |
 "$OBSERVE" - "$OUT/post-injection-observations.json" 2> "$OUT/observe.stderr"
codes=("${PIPESTATUS[@]}"); set -e
record_codes observe "${codes[@]}"
...
obs=json.loads((p/'post-injection-observations.json').read_text())
assert obs['status']=='COMPLETE_POST_INJECTION_DIAGNOSTIC' and obs['accepted'] is False
assert obs['rows']==pre['points'] and obs['last_time_s']==scans[0]['last_time_s']
assert obs['injection']['time_s']==scans[0]['fault_inject_rising_edges'][0]['time_s']
print(json.dumps({'status':'COMPLETE_DIAGNOSTICS_PARENT_REVIEW_REQUIRED','accepted':False,'raw_sha256_before_after':after,'raw_bytes':pre['raw_bytes'],'full_rows':scans[0]['full_rows'],'selected_rows':scans[0]['selected_rows'],'prefix_endpoint_s':scans[0]['last_selected_time_s'],'pipeline_exits':{k:[int(x) for x in (p/(k+'.exit')).read_text().split()] for k in ['scan','audit','metrics','phase','observe']},'observability_scope':'Post-injection sampled controller/current observations; i(Vchannel) is MOS-channel current, not diode-branch current. No causal or continuous interruption conclusion.'},indent=2))
PY
