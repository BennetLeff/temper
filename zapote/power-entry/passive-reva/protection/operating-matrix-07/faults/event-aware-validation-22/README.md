...
cap is exceeded. The retained checker suffix is separately capped at eight
million rows. Parent `size_of::<fault_checks::Row>()` confirms the retained
row is 136 bytes (17 f64 values), so eight million payload rows require
1,088,000,000 bytes (~1.013 GiB). Geometric Vec capacity of 8,388,608 rows
requires 1,140,850,688 bytes (1.0625 GiB), plus small container/stream overhead.
The separate outer numbered row is not the retained suffix layout. Oversized
checked17 lines (including the header) are rejected at 16 KiB. These caps
bound what the validator will retain; a producer still must stream its input
rather than materialize an unbounded line itself.
...
```sh
rustc --edition=2021 -D warnings audit.rs -o /tmp/matrix07-event-aware-validation-22
/tmp/matrix07-event-aware-validation-22 \
...
rustc --edition=2021 -D warnings --test tests.rs -o /tmp/fault-event-tests-22
/tmp/fault-event-tests-22   # 22 passed
```
