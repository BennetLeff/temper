...
// A checked17 row contains 17 f64 values plus Vec overhead.  Eight million
...
pub(crate) const RETAINED_LIMIT: usize = 8_000_000;
...
pub(crate) fn retained_capacity(current: usize) -> Result<(), ValidationError> {
    if current >= RETAINED_LIMIT { Err(ValidationError("retained checker suffix exceeds eight-million-row cap".into())) } else { Ok(()) }
}
