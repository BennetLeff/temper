#!/bin/bash
# Fixed DIODE-SHORT saved-archive analysis. No solver and no circuit acceptance.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"
PACKET="$ROOT/faults/settled-diode-postcapture-74"
...
assert digest(case/'manifest.json')=='fd5c7f9e9703a5d0004bcd523798644068d0b378081361580bcda5c29b67baa9'
m=json.loads((case/'manifest.json').read_text())
assert m['case']=='DIODE-SHORT' and m['t_fault_s']==.6541666666667 and m['tstop_s']==.662
assert m['runner_kind']=='diode-short' and m['validator_kind']=='diode-short'
params=json.loads((case/'run-parameters.json').read_text())
assert params == {'tstop_s':.662,'kind':'diode-short','mutation_s':.6541666666667,'event_window_s':.002,'expected_fault_s':.6541666666667,'observation_s':.002,'turnoff_s':.000002,'max_gap_s':.000000025,'wall_seconds':3600,'export_timeout_s':900,'bypass':False}
