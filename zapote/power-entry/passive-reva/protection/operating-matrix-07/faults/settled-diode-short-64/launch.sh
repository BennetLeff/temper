#!/bin/bash
# DIODE-SHORT launch wrapper.  Preparation only: the parent must run this after
...
run_root="$PWD/faults/settled-diode-short-64"
src="$PWD/faults/settled-compact-prep-24/prepared/DIODE-SHORT"
out="$run_root/full-DIODE-SHORT"
required_kib=$((16*1024*1024)) # 10 GiB runtime floor + 6 GiB archive reserve

if [ ! -d "$src" ]; then echo "prepared DIODE-SHORT source is missing" >&2; exit 77; fi
if [ -e "$out" ]; then echo "refusing existing DIODE-SHORT output" >&2; exit 77; fi
...
check_solver_inventory() {
  ...
  if [ -n "$solver_rows" ]; then
    echo "a known full solver/campaign runner is live; refusing DIODE-SHORT:" >&2
    printf '%s\n' "$solver_rows" >&2
    return 77
  fi
  return 0
...
verify_source() {
  ...
  for name in \
    ...
    case "$name" in
      case.cir) expected=d5a78e2c878447110a13c54cc3052b3dff7e862dc3e79a5fbd8e973257ac5f78 ;;
      manifest.json) expected=fd5c7f9e9703a5d0004bcd523798644068d0b378081361580bcda5c29b67baa9 ;;
...
verify_source || exit $?
if ! mkdir "$out"; then echo "cannot create fresh DIODE-SHORT output" >&2; exit 77; fi
...
/private/tmp/matrix07-fault-runner45-parent \
 ...
 .662 diode-short .6541666666667 .002 .6541666666667 .002 .000002 .000000025 3600 900 \
