#!/bin/bash
# Record the frozen legacy strict-checker result for the completed F2-ZERO scan.
...
POST="$ROOT/faults/settled-postcapture-58/full-F2-ZERO"
...
PY
jq -e '.status == "OK" and .cutoff_s == 0.65 and .max_cutoff_gap_s == 1e-6 and .last_selected_time_s > 0 and .last_selected_time_s <= 0.65 and .cutoff_gap_s >= 0 and .cutoff_gap_s <= 1e-6 and (.fault_inject_rising_edges|length) == 1' "$SCAN" >/dev/null

shasum -a 256 -c "$ROOT/faults/settled-postcapture-58/tools-parent.sha256" >/dev/null
test -x "$CHECKER"
