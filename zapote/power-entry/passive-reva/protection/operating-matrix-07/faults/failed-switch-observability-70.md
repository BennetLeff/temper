...
header failure remains unexplained. Independent complete F2-ZERO decode and
electrical reanalysis succeeded with the same pinned tools; each later archive
still needs its own complete transport and electrical review.
...
preserves equal-time rows in counts, and requires at least two post-injection
rows and a strictly later final sample. It does not enforce a maximum time gap,
dense sampling, healthy prefix, phase, or continuity. Those omissions are
...
channel count or extrema establishes sampled post-injection current, but alone
does not place that current after or at controller-off samples and does not
establish a causal failed-switch mechanism. The frozen protection checker may reject earlier on a node or
...
needed. Before adding any scanner, the parent can derive a conservative
same-row overlap bound from the existing counts. `Post::observe` increments
the off, current-above and total counts over the same post-injection row set.
With N total rows, O simultaneous-off rows and C channel-above rows, at least
`max(0, O + C - N)` rows satisfy both predicates. Require `0 <= O,C <= N`.
A positive bound proves sampled coexistence of controller-off and current
above 0.1 A; it gives neither the timestamps nor duration, distinct-time count,
causality, continuous retention or physical interruption. A zero bound proves
nothing about overlap. Equal-time rows remain separate members of this set.

Only if that bound is insufficient for the required conclusion would a narrow
same-row predicate join over the unchanged archive be useful. For DIODE-SHORT only,
