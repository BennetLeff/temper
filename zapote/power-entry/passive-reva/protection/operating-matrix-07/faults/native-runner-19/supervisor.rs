...
fn ensure_absent(case: &Path) -> Result<(), String> {
    for name in ["initialization.json","source-identity.json","run-parameters.json","capture-metadata.json","raw.trace.raw.gz","raw-gzip.stderr","host.stdout","host.stderr","first-invalid.tsv","stage1.exit","capture-stage1.json","decoder.stderr","adapter.stdout","adapter.stderr","adapter-report.txt","validator.stdout","validator.stderr","validator-events.tsv","stage2.exit","result.json","host.raw.fifo","adapter.raw.fifo","checked.fifo"] {
