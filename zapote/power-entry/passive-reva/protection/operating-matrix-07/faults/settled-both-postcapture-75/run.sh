#!/bin/bash
# Fixed BOTH-SHORT saved-archive analysis. No solver and no circuit acceptance.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"
PACKET="$ROOT/faults/settled-both-postcapture-75"
RUNROOT="$ROOT/faults/settled-both-short-65"
CASE="$RUNROOT/full-BOTH-SHORT"
...
assert digest(case/'manifest.json')=='92345035c765b9132540e067b1ffe3ce06e4845095671328700386502dc0479b'
m=json.loads((case/'manifest.json').read_text())
assert m['case']=='BOTH-SHORT' and m['t_fault_s']==.6541666666667 and m['tstop_s']==.662
assert m['runner_kind']=='both-short' and m['validator_kind']=='both-short'
params=json.loads((case/'run-parameters.json').read_text())
assert params == {'tstop_s':.662,'kind':'both-short','mutation_s':.6541666666667,'event_window_s':.002,'expected_fault_s':.6541666666667,'observation_s':.002,'turnoff_s':.000002,'max_gap_s':.000000025,'wall_seconds':3600,'export_timeout_s':900,'bypass':False}
...
print(json.dumps({'status':'COMPLETE_DIAGNOSTICS_PARENT_REVIEW_REQUIRED','accepted':False,'raw_sha256_before_after':after,'raw_bytes':pre['raw_bytes'],'full_rows':scans[0]['full_rows'],'selected_rows':scans[0]['selected_rows'],'prefix_endpoint_s':scans[0]['last_selected_time_s'],'pipeline_exits':{k:[int(x) for x in (p/(k+'.exit')).read_text().split()] for k in ['scan','audit','metrics','phase','observe']},'observability_scope':'Post-injection sampled controller/current observations; failed switch branch included in i(Vchannel). No causal or continuous interruption conclusion.'},indent=2))
PY
