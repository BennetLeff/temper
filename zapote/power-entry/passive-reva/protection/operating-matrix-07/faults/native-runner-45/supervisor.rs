...
// The capture parameter remains the prepared deck's local 25 ns pacing
// bound.  The adapter and outer event-aware validator use the separate
// campaign-wide positive-gap contract of 1 us.  The frozen inner checker
// retains its own 25 ns local event guard.
const GLOBAL_MAX_GAP_S: f64 = 1e-6;

fn analysis_max_gap(capture_pacing_gap: f64) -> Result<f64, String> {
    if !capture_pacing_gap.is_finite() || capture_pacing_gap != 25e-9 {
        return Err("prepared fault campaigns require max_gap_s exactly 25 ns".into());
    }
    Ok(GLOBAL_MAX_GAP_S)
}
...
fn run_case(args: &[String]) -> Result<(), String> {
    ...
    // Preserve the immutable 25 ns capture parameter in run-parameters.json,
    // but route only the reviewed 1 us campaign bound to the outer stages.
    let analysis_gap = analysis_max_gap(args[14].parse::<f64>().map_err(|_| "invalid MAX_GAP_S".to_string())?)?;
    ...
    fs::write(case.join("gap-policy.json"), format!("{{\n  \"capture_pacing_max_gap_s\": {:.17e},\n  \"outer_adapter_validator_max_gap_s\": {:.17e},\n  \"frozen_inner_checker_max_gap_s\": {:.17e}\n}}\n", args[14].parse::<f64>().map_err(|_| "invalid MAX_GAP_S".to_string())?, analysis_gap, args[14].parse::<f64>().map_err(|_| "invalid MAX_GAP_S".to_string())?)).map_err(|e| format!("write gap policy: {e}"))?;
    ...
    let result: Result<(), String>=(|| {
        ...
        let validator_script = format!("exec {} --end-s {} --expected-fault-s {} --window-s {} --observation-s {} --turnoff-s {} --max-gap-s {} --kind {} --events validator-events.tsv{} < {}", shell_quote(&validator)?, args[7], args[11], args[10], args[12], args[13], analysis_gap, validator_kind, if bypass { " --bypass" } else { "" }, shell_quote(&checked_fifo)?);
...
mod tests { use super::*;
    ...
    #[test] fn outer_gap_is_one_us_but_local_capture_must_be_exact_25_ns(){assert_eq!(analysis_max_gap(25e-9).unwrap(),1e-6);assert!(analysis_max_gap(24.999e-9).is_err());assert!(analysis_max_gap(1e-6).is_err());assert!(analysis_max_gap(f64::NAN).is_err());}
