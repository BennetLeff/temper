...
fn analysis_max_gap(capture_pacing_gap: f64) -> Result<f64, String> {
    if !capture_pacing_gap.is_finite() || capture_pacing_gap <= 0.0 || capture_pacing_gap >= GLOBAL_MAX_GAP_S {
        return Err("capture pacing gap must be finite, positive, and below the global analysis bound".into());
    }
    Ok(GLOBAL_MAX_GAP_S)
}
