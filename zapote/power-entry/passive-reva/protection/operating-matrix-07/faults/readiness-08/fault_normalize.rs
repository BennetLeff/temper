...
//! both retained outputs through `BufWriter`, and keeps only the previous row
//! plus bounded scalar metrics. The existing normalizer is intentionally not
...
impl Kind {
    fn parse(s: &str) -> Result<Self, String> {
        match s.to_ascii_lowercase().as_str() {
            ...
            "switch-short" => Ok(Self::SwitchShort),
    ...
    fn needs_f2_transition(self) -> bool {
        matches!(self, Self::F2Crest | Self::F2Zero)
    }
}
...
impl StreamState {
    fn observe<W1: Write, W2: Write>(
        ...
        if let Some(previous) = self.previous {
            ...
            if dt <= 0.0 || dt > cfg.max_gap {
                return Err(format!(
                    "row {}: time is not strict or gap exceeds max_gap",
                    self.count + 2
                ));
            }
        } else {
    ...
    fn finish(self, cfg: Config) -> Result<String, String> {
        ...
        Ok(format!(
            ...
            self.body_peak, self.il_peak, self.f2_peak, self.d1_peak, self.d2_peak,
        ))
    }
}
...
fn normalize_stream<R: BufRead, W1: Write, W2: Write>(
    ...
    loop {
        ...
        state.observe(row, cfg, checked, supplemental)?;
    }
