...
fn main() -> Result<(), String> {
    ...
    if args.len() != 7 { return Err("usage: progress DECK WALL_SECONDS TARGET_RAW_PATH ASCII_PATH SNAPSHOT_PATH".into()); }
    if args[4].chars().any(char::is_whitespace) || args[4].contains([';', '\n', '"']) { return Err("simple raw path required".into()); }
    if args[5].chars().any(char::is_whitespace) || args[5].contains([';', '\n', '"']) { return Err("simple ascii path required".into()); }
    ...
    write_snapshot(&args[6]).or_else(|e| if FIRST_INVALID.load(Ordering::Acquire) { Err(e) } else { fs::write(&args[6], "first_invalid=false\n").map_err(|x| x.to_string()) })?;
    fs::write("capture-metadata.json", format!("{{\n  \"stop_reason\": \"{stop_reason}\",\n  \"points\": {},\n  \"first_invalid\": {},\n  \"seen_names_mask\": {},\n  \"expected_names_mask\": {},\n  \"export_mode\": \"full_trace_path_or_fifo\",\n  \"vector_slicing_smoke\": \"rejected_full_length\",\n  \"diagnostic_only\": true,\n  \"accepted\": false,\n  \"export_format\": \"ngspice-real-native\",\n  \"byte_order\": \"little\",\n  \"writer_platform\": \"{}-{}\",\n  \"schema\": \"fault42\",\n  \"fault_schema_columns\": 17,\n  \"fault_extra_columns\": 4,\n  \"validation_policy\": \"fault-transport-only-v1\"\n}}\n", POINTS.load(Ordering::Acquire), FIRST_INVALID.load(Ordering::Acquire), SEEN_NAMES_MASK.load(Ordering::Acquire), (1u64 << DIAG_NAMES.len()) - 1, std::env::consts::ARCH, std::env::consts::OS)).map_err(|e| e.to_string())?;
    command("set filetype=binary")?;
    command(&format!("write {} time {signals}", args[4]))?;
    command("set wr_singlescale")?; command("set wr_vecnames")?; command("set numdgt=17")?;
    command(&format!("wrdata {} {signals}", args[5]))?;
