...
enum Schema { Normal15, Diagnostic31, Fault42 }
...
fn expected_names(schema: Schema) -> &'static [&'static str] {
    match schema { Schema::Normal15 => &NORMAL15, Schema::Diagnostic31 => &DIAGNOSTIC31, Schema::Fault42 => &FAULT42 }
}
...
fn parse_variables(header: &str, schema: Schema) -> Result<Vec<usize>, String> {
        ...
        .filter(|line| !line.trim().is_empty()).enumerate()
    {
        ...
        if unit != expected_unit && !(canonical_name == "v(acsrc,acn)" && unit == "notype") {
...
mod tests {
    ...
    fn fixture_dynamic(names: &[&str], points: &[Vec<f64>]) -> Vec<u8> {
        assert!(points.iter().all(|point| point.len() == names.len()));
        let mut header = format!("Title: test\nFlags: real\nNo. Variables: {}\nNo. Points: {}\nVariables:\n", names.len(), points.len());
        for (i, name) in names.iter().enumerate() {
            let lower = name.to_ascii_lowercase();
            let unit = if lower == "time" { "time" } else if lower.starts_with("i(") { "current" } else { "voltage" };
    ...
    #[test]
    fn fault42_header_units_and_output_bits_are_preserved() {
        let names = FAULT42.to_vec();
        let mut first = (0..names.len()).map(|i| 0.25 + i as f64).collect::<Vec<_>>();
        let mut equal = (0..names.len()).map(|i| 10.25 + i as f64).collect::<Vec<_>>();
        let mut last = (0..names.len()).map(|i| 20.25 + i as f64).collect::<Vec<_>>();
        first[0] = 0.0; equal[0] = 0.0; last[0] = 1.0;
        let raw = fixture_dynamic(&names, &[first.clone(), equal.clone(), last.clone()]);
        let mut output = Vec::new();
        run(&mut BufReader::new(raw.as_slice()), &mut output, Schema::Fault42).unwrap();
        let lines: Vec<Vec<f64>> = String::from_utf8(output).unwrap().lines().skip(1)
            .map(|line| line.split_whitespace().map(|value| value.parse::<f64>().unwrap()).collect()).collect();
        assert_eq!(lines.len(), 3);
        assert_eq!(lines[1][0].to_bits(), equal[0].to_bits());
        assert_eq!(lines[1][31].to_bits(), equal[31].to_bits());
        assert_eq!(lines[2][41].to_bits(), last[41].to_bits());
        assert_eq!(lines[0][7].to_bits(), first[7].to_bits());
    }

    #[test]
    fn fault42_nonfinite_and_truncated_payloads_are_rejected() {
        let names = FAULT42.to_vec();
        let rows = vec![vec![0.0; names.len()], vec![1.0; names.len()]];
        let mut truncated = fixture_dynamic(&names, &rows); truncated.pop();
        assert!(run(&mut BufReader::new(truncated.as_slice()), Vec::new(), Schema::Fault42).unwrap_err().contains("truncated"));
        let mut nonfinite = fixture_dynamic(&names, &rows);
        let at = nonfinite.len() - 8;
        nonfinite[at..].copy_from_slice(&f64::NAN.to_le_bytes());
        assert!(run(&mut BufReader::new(nonfinite.as_slice()), Vec::new(), Schema::Fault42).unwrap_err().contains("non-finite"));
    }
}
