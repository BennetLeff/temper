#!/bin/bash
# BOTH-SHORT launch wrapper.  Preparation only: the parent must run this after
...
run_root="$PWD/faults/settled-both-short-65"
src="$PWD/faults/settled-compact-prep-24/prepared/BOTH-SHORT"
out="$run_root/full-BOTH-SHORT"
required_kib=$((16*1024*1024)) # 10 GiB runtime floor + 6 GiB archive reserve

if [ ! -d "$src" ]; then echo "prepared BOTH-SHORT source is missing" >&2; exit 77; fi
if [ -e "$out" ]; then echo "refusing existing BOTH-SHORT output" >&2; exit 77; fi
...
check_solver_inventory() {
  ...
  if [ -n "$solver_rows" ]; then
    echo "a known full solver/campaign runner is live; refusing BOTH-SHORT:" >&2
    printf '%s\n' "$solver_rows" >&2
    return 77
  fi
  return 0
...
verify_source() {
  ...
  for name in \
    ...
    case "$name" in
      case.cir) expected=c35139cff49f8f43db43a4dca9cff13c06c0289150b09dab12ba5aff29ffcb51 ;;
      manifest.json) expected=92345035c765b9132540e067b1ffe3ce06e4845095671328700386502dc0479b ;;
...
verify_source || exit $?
if ! mkdir "$out"; then echo "cannot create fresh BOTH-SHORT output" >&2; exit 77; fi
...
/private/tmp/matrix07-fault-runner45-parent \
 ...
 .662 both-short .6541666666667 .002 .6541666666667 .002 .000002 .000000025 3600 900 \
