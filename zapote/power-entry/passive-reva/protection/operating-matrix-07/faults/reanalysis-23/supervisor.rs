...
fn validate_manifest_binding(case: &Path, kind: &str, tstop: f64, mutation: f64, event_window: f64) -> Result<(), String> {
    ...
    if jq(&manifest, ".case | ascii_downcase")? != kind {
...
fn run(source_arg: &str, output_arg: &str, pigz_arg: &str, decoder_arg: &str, adapter_arg: &str, validator_arg: &str, bypass: bool) -> Result<(), String> {
    ...
    let max_gap = positive("max_gap_s", &jq(&parameters, ".max_gap_s")?)?;
    ...
    let result: Result<(), String> = (|| {
        let validator_script = format!("exec {} --end-s {} --expected-fault-s {} --window-s {} --observation-s {} --turnoff-s {} --max-gap-s {} --kind {} --events {}{} < {}", shell_quote(&validator)?, tstop, expected_fault, event_window, observation, turnoff, max_gap, validator_kind, shell_quote(&output.join("validator-events.tsv"))?, if bypass { " --bypass" } else { "" }, shell_quote(&checked_fifo)?);
        ...
        let mut adapter_cmd = Command::new(&adapter); adapter_cmd.current_dir(&output).args([adapter_fifo.to_str().ok_or("adapter FIFO path")?, checked_fifo.to_str().ok_or("checked FIFO path")?, "/dev/null", "adapter-report.txt", &tstop.to_string(), adapter_kind, &mutation.to_string(), &event_window.to_string(), &max_gap.to_string()]).stdout(Stdio::from(File::create(output.join("adapter.stdout")).map_err(|error| error.to_string())?)).stderr(Stdio::from(File::create(output.join("adapter.stderr")).map_err(|error| error.to_string())?));
