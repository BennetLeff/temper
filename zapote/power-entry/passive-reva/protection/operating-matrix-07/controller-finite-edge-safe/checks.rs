...
fn current_loop(d: &[Vec<f64>]) -> Result<Vec<String>, String> {
    ...
    // 0.72V offset is the nominal TI E2E interpretation; the gain and
    // dynamic pole remain independently fixed by datasheet equations.
    if (initial - 3.72).abs() > 0.0001 {
        return Err("ICOMP nominal offset mismatch".into());
    }
    if (final_v - (3.72 + 0.001 * 2.5 * 7.0 / 0.538)).abs() > 0.0001 {
