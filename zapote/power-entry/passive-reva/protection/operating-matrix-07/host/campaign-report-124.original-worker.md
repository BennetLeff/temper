...
Normal results are discrete authored-model screens, not interpolation, thermal, SOA, magnetic, fuse, or hardware evidence. The nine-point acceptance and minimum margins are recorded in [checkpoint 53](operating-envelope-checkpoint-53.md) and [margin review 89](normal-screen-margins-89.md).
...
| SW-SHORT | Complete endpoint is unavailable: current saved attempt stops before 0.662 s. A bounded prefix currently has 33,018 rows in live review, but cannot establish a complete fault envelope or whole-case result | **INDETERMINATE; pending prefix review and required endpoint** |
...
| BYPASS-NEG | Negative-control protection-bypass case currently has 28,321 rows in the live/partial evidence set; complete endpoint, waveform, and validator disposition remain unavailable | **PENDING; no pass can be inferred from a clean transport run** |
