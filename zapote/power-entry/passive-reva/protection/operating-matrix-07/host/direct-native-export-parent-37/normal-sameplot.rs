...
/// Keep the production watchdog threshold explicit and independently testable.
/// This is intentionally based on wall-clock time since the last accepted
/// simulation-time advance; a first-invalid snapshot does not reset it.
fn stalled_since(last_advance: Instant, now: Instant) -> bool {
    now.duration_since(last_advance) >= Duration::from_secs(120)
}
...
fn main() -> Result<(), String> {
    ...
    writeln!(progress, "wall_s\tsim_s\tpercent\trecent_sim_s_per_wall_s\tcallbacks\tfirst_invalid\tduplicate_count\tbackwards_count").map_err(|e| e.to_string())?;
    let start = Instant::now();
    ...
    loop {
        ...
        let stalled = stalled_since(advanced_at, Instant::now());
        ...
        let duplicate_count = DUPLICATE_COUNT.load(Ordering::Acquire);
        let backwards_count = BACKWARDS_COUNT.load(Ordering::Acquire);
        if wall - last_wall >= 10.0 || wall >= limit || done || stalled {
            let rate = if wall > last_wall { (sim - last_sim) / (wall - last_wall) } else { 0.0 };
            let row = format!("{wall:.6}\t{sim:.17e}\t{:.6}\t{rate:.12e}\t{}\t{}\t{}\t{}", 100.0 * sim / target, POINTS.load(Ordering::Acquire), first_invalid, duplicate_count, backwards_count);
    ...
    fs::write("stop.txt", format!("reason={stop_reason}\nwall_s={:.6}\nsim_s={:.17e}\npoints={}\nfirst_invalid={}\nduplicate_count={}\nbackwards_count={}\ncontinue_after_duplicate=true\ndiagnostic_only=true\n", start.elapsed().as_secs_f64(), if HAVE_TIME.load(Ordering::Acquire) { f64::from_bits(LAST_TIME_BITS.load(Ordering::Acquire)) } else { 0.0 }, POINTS.load(Ordering::Acquire), FIRST_INVALID.load(Ordering::Acquire), DUPLICATE_COUNT.load(Ordering::Acquire), BACKWARDS_COUNT.load(Ordering::Acquire))).map_err(|e| e.to_string())?;
    ...
    fs::write("capture-metadata.json", format!("{{\n  \"stop_reason\": \"{stop_reason}\",\n  \"points\": {},\n  \"first_invalid\": {},\n  \"seen_names_mask\": {},\n  \"expected_names_mask\": {},\n  \"duplicate_count\": {},\n  \"backwards_count\": {},\n  \"continue_after_duplicate\": true,\n  \"diagnostic_only\": true,\n  \"accepted\": false,\n  \"export_format\": \"ngspice-real-native\",\n  \"byte_order\": \"little\",\n  \"writer_platform\": \"{}-{}\",\n  \"schema\": \"normal15\",\n  \"validation_policy\": \"event-aware-normal-v1\",\n  \"export_mode\": \"full_trace_path_or_fifo\",\n  \"vector_slicing_smoke\": \"rejected_full_length\"\n}}\n", POINTS.load(Ordering::Acquire), FIRST_INVALID.load(Ordering::Acquire), SEEN_NAMES_MASK.load(Ordering::Acquire), (1u64 << DIAG_NAMES.len()) - 1, DUPLICATE_COUNT.load(Ordering::Acquire), BACKWARDS_COUNT.load(Ordering::Acquire), std::env::consts::ARCH, std::env::consts::OS)).map_err(|e| e.to_string())?;
    let export = native_export::export(&args[4], native_export::Schema::Normal15,
        usize::try_from(POINTS.load(Ordering::Acquire)).map_err(|_| "point count overflow")?)?;
    fs::write("native-export-receipt.json", format!(
        "{{\n  \"status\": \"complete\",\n  \"schema\": \"normal15\",\n  \"rows\": {},\n  \"columns\": {},\n  \"bytes_written\": {},\n  \"borrowed_data_bytes\": {},\n  \"buffer_bytes\": {},\n  \"export_elapsed_ms\": {},\n  \"copy_avoidance_only\": true,\n  \"accepted\": false\n}}\n",
        export.rows, export.columns, export.bytes_written, export.borrowed_data_bytes, export.buffer_bytes, export.export_elapsed_ms)).map_err(|e| e.to_string())?;
    // TEST ONLY: reference export from the same stopped plot, after all direct borrows ended.
    command("set filetype=binary")?;
    command(&format!("write reference.raw {}", native_export::Schema::Normal15.names().join(" ")))?;
...
mod tests {
    ...
    fn callback_captures_named_values_only_on_first_bad_time() {
        ...
        *SNAPSHOT.lock().unwrap() = None;
        ...
        // A duplicate captures the first-invalid snapshot but does not itself
        // make the callback stop advancing; a later accepted point is seen.
        invoke_point(0.2, 30.0);
        assert_eq!(POINTS.load(Ordering::Acquire), 3);
        assert_eq!(f64::from_bits(LAST_TIME_BITS.load(Ordering::Acquire)), 0.2);
    }

    #[test]
    fn backwards_time_sets_separate_stop_flag() {
        let _serial = TEST_SERIAL.lock().unwrap();
        HAVE_TIME.store(false, Ordering::Release);
        LAST_TIME_BITS.store(0, Ordering::Release);
        POINTS.store(0, Ordering::Release);
    ...
    #[test]
    fn nonfinite_time_sets_finite_guard_without_counting_as_backwards() {
        let _serial = TEST_SERIAL.lock().unwrap();
        HAVE_TIME.store(false, Ordering::Release);
        LAST_TIME_BITS.store(0, Ordering::Release);
        POINTS.store(0, Ordering::Release);
    ...
    #[test]
    fn stall_watchdog_threshold_is_unchanged() {
        let anchor = Instant::now();
        assert!(!stalled_since(anchor, anchor + Duration::from_secs(119)));
        assert!(stalled_since(anchor, anchor + Duration::from_secs(120)));
    }
}
