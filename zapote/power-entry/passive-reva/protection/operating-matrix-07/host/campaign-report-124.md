...
**Status: `DRAFT_PENDING_BYPASS_POSTCAPTURE_AND_FINAL_INDEX`.** This report consolidates the source-bound model evidence and protection requirements. It does not add acceptance claims, widen a criterion, or qualify hardware. SW-SHORT now has its own accepted normal prefix and retains an indeterminate full-fault disposition. BYPASS-NEG has a complete capture and a voltage-screen failure; its independent postcapture review remains pending.
...
| BYPASS-NEG | Complete 32,287,070-row recording through 662 ms. [Initial capture review](../faults/settled-bypass-neg-66/full-BYPASS-NEG/parent-capture-review-130.json): VD 651.173012 V exceeds the unchanged 500 V screen. Adapter reports no detector rise or latch-off, and channel current above 0.1 A through the endpoint. Independent normal-prefix, phase, legacy and detector-independent observations are still running. | **Voltage-screen FAIL; negative-control waveform assessment pending** |
...
Acceptance-dependent sections intentionally remain open: BYPASS-NEG own-prefix, phase and waveform review, final reproducibility/index closure, and full goal-audit signoff. This draft therefore supersedes neither the per-case acceptance receipts nor the pending statuses.
