...
fn main() -> Result<(), String> {
    ...
    if args.len()!=6 { return Err("usage: progress DECK WALL_SECONDS TARGET_SECONDS TSV_EXPORT_PATH NATIVE_RAW_PATH".into()); }
    if args[4].chars().any(char::is_whitespace) || args[4].contains([';', '\n', '"']) || args[5].chars().any(char::is_whitespace) || args[5].contains([';', '\n', '"']) { return Err("simple export paths required".into()); }
    ...
    // Native probe only: issue separate commands after TSV export so the
    // standard ngspice binary raw writer is independently exercised.
    command("set filetype=binary")?;
    command(&format!("write {} all", args[5]))?;
