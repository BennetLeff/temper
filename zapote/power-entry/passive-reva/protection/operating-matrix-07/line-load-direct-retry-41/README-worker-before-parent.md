...
the solver's full simulation arrays. Those probes do not establish full-run
resource savings or electrical acceptance.

## Launch command

Run only after the parent rechecks free space and approves one solver. The
conservative budget is a 2.2 GiB LL08 archive plus the 10 GiB floor and about
2.0 GiB for the other retained archives. The preflight `df` sample in this
...
OUT="$BASE/line-load-direct-retry-41"
RUNNER=/private/tmp/matrix07-line-load-native-runner27
...
"$RUNNER" \
  ...
  --workers 1 --wall-limit 2700 --end-s 0.65 \
