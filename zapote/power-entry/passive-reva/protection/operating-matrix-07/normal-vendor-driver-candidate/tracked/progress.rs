...
const DIAG_NAMES: [&[u8]; 16] = [
    b"xu.raw", b"xu.pwm_hold", b"pwm", b"pwm_input", b"outh", b"outl",
...
fn main() -> Result<(), String> {
    ...
    write_snapshot("first-invalid.tsv").or_else(|e| if FIRST_INVALID.load(Ordering::Acquire) { Err(e) } else { fs::write("first-invalid.tsv", "first_invalid=false\n").map_err(|x| x.to_string()) })?;
