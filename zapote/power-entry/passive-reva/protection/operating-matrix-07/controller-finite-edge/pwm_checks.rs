...
fn check(input: &str) -> Result<(), String> {
    ...
    for (j, delta) in [0.0, 1.0, 3.0].into_iter().enumerate() {
        ...
        // Authored logic chain, not TI silicon propagation specification:
        // explicit ADC 1ns + DFF clock 1ns/rise 1ps + DAC half-rise .5ns.
        // Keep the original 3ns numerical tolerance unchanged.
        let logic_delay = 1e-9 + 1e-9 + 1e-12 + 0.5e-9;
        let target = 570e-9 + delta / (m2 * 1e6) + logic_delay;
