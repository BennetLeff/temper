// Adapted from temper-design-bundle/src/sexpr.rs; see zapote/ports.toml.
...
fn tokenize(input: &str, format_name: &str) -> Result<Vec<String>, DesignBundleError> {
    ...
    while i < bytes.len() {
        ...
        if c == '"' {
            ...
            if !closed {
                return Err(format!("unterminated string literal in {format_name}"));
            }
        ...
        while i < bytes.len() {
            let cur = bytes[i] as char;
            if cur.is_ascii_whitespace() || cur == '(' || cur == ')' {
                break;
            }
            i += 1;
        }
...
fn parse_expr(
    ...
    if depth > 128 {
        return Err("S-expression nesting exceeds 128".into());
    }
    if *pos >= tokens.len() {
        return Err(format!("unexpected end of {format_name} input"));
    }
    let tok = &tokens[*pos];
    if tok == "(" {
        *pos += 1;
        ...
        loop {
            if *pos >= tokens.len() {
                return Err(format!("unbalanced parentheses in {format_name}"));
            }
            if tokens[*pos] == ")" {
                *pos += 1;
                break;
            }
            items.push(parse_expr(tokens, pos, format_name, depth + 1)?);
        }
        Ok(Sexpr::List(items))
    } else if tok == ")" {
        Err(format!("unexpected ')' in {format_name}"))
    } else {
        *pos += 1;
...
pub(crate) fn parse_document(text: &str, format_name: &str) -> Result<Sexpr, DesignBundleError> {
    ...
    if tokens.is_empty() {
        return Err(format!("empty {format_name} document"));
    }
    let mut pos = 0usize;
    let root = parse_expr(&tokens, &mut pos, format_name, 0)?;
    if pos != tokens.len() {
