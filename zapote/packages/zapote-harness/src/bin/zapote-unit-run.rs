...
mod tests {
    ...
    #[test]
    fn resolves_every_manifest_path_against_the_base() {
        let mut manifest = Manifest {
            schema: "test".into(),
            units: vec![runner::UnitRunSpec {
                unit: runner::UnitKind::ThermalSense,
        ...
        let u = &manifest.units[0];
        assert_eq!(u.source, Path::new("/run/source.json"));
        assert_eq!(u.native, Path::new("/run/native.json"));
        assert_eq!(u.board, Path::new("/run/board.kicad_pcb"));
