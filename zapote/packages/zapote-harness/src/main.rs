...
fn main() {
    ...
    let mut current_sense_input_path = None;
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--current-sense-input" => current_sense_input_path = args.next(),
            "--memory-input" => memory_input_path = args.next(),
            "--output" => output_path = args.next(),
            "--no-telemetry" => no_telemetry = true,
            "--help" | "-h" => {
                println!("zapote-rtd [--input FILE | --unit-input FILE | --current-sense-input FILE | --memory-input FILE] [--output FILE] [--no-telemetry]");
                return;
            }
            other => {
    ...
    if let Some(path) = current_sense_input_path {
        if input_path.is_some() || unit_input_path.is_some() || memory_input_path.is_some() {
            eprintln!("--current-sense-input cannot be combined with another input mode");
            std::process::exit(2);
        }
        let bytes = fs::read(path).unwrap_or_else(|e| {
            eprintln!("cannot read current-sense input: {e}");
            std::process::exit(2);
        });
        let input = zapote_harness::parse_current_sense_input(&bytes).unwrap_or_else(|e| {
            eprintln!("invalid zapote.current-sense.input.v1 input: {e}");
            std::process::exit(2);
        });
