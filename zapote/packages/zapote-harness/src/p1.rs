//! P1 power/domain composition for gate-drive saved candidates.
...
pub fn run(source: &str, native: &str, board: &[u8], unit: P1Unit) -> CheckReport {
    ...
    let entry: Result<&'static str, String> = match unit {
    ...
    r.findings.push(match &parsed {
        ...
        Ok(Err(e)) => Finding::fail(
            "ERC.POWER.P1_SOURCE_NATIVE_BINDING",
            e.clone(),
            *entry.as_ref().unwrap_or(&zapote_erc::gate_drive::ENTRY),
        ),
        Err(e) => Finding::fail(
            "ERC.POWER.P1_SOURCE_NATIVE_BINDING",
            (*e).clone(),
    ...
    // candidate.  Gate-drive bias/output branches remain indeterminate until
    // waveform bounds are supplied by the reviewed operating contract.
