...
pub fn validate_manifest(m: &Manifest) -> Result<()> {
    ...
    if got != want || m.units.len() != want.len() {
        return Err("manifest must contain each of the five maintained units exactly once".into());
    }
    Ok(())
}
...
fn required_rules(unit: UnitKind) -> Result<Vec<String>> {
    ...
    let required: Vec<String> = inventory["units"]
        .as_array()
...
pub fn run(spec: &UnitRunSpec, out: &Path, kicad: &Path, python: &Path) -> Result<UnitRunReport> {
    ...
    if matches!(spec.unit, UnitKind::GateDrive) {
        required.extend(
            [
...
#[cfg(test)]
mod manufacturing_receipt_tests {
    use super::*;
