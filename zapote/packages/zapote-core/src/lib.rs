...
pub struct Pad {
    ...
    pub size_mm: [f64; 2],
    /// Native pad drill dimensions.  Zero means a non-plated SMD pad.
    #[serde(default)]
    pub drill_mm: [f64; 2],
    /// KiCad's native pad shape enum (0 circle, 1 rectangle, 2 oval,
    /// 4 roundrect).  Keep the enum numeric at this transport boundary so
    /// the extractor's value is preserved without a donor-specific mapping.
    #[serde(default)]
    pub shape: u8,
    /// KiCad roundrect radius ratio, when the native build exposes it.
    #[serde(default)]
...
pub struct FirmwareContract {
    ...
    #[serde(default = "default_fault_latency_limit_ms")]
    pub fault_latency_limit_ms: f64,
}

fn default_fault_latency_limit_ms() -> f64 {
    100.0
}
...
pub struct FaultScenario {
    ...
    /// Standalone unit reports carry model output and independently generated
    /// observation side by side.  Legacy full-board inputs leave these unset.
    #[serde(default)]
    pub observed_class: Option<String>,
    #[serde(default)]
    pub observed_detected: Option<bool>,
    #[serde(default)]
    pub model_latency_ms: Option<f64>,
    #[serde(default)]
    pub observed_latency_ms: Option<f64>,
    /// Static detector evidence is separate from dynamic timing evidence.
    /// These fields are populated for the local rail monitor contract and do
    /// not imply a guaranteed worst-case latency.
    #[serde(default)]
    pub observed_fault_owner: Option<String>,
    #[serde(default)]
    pub observed_detection_condition: Option<String>,
    #[serde(default)]
    pub observed_powered_comparator_result: Option<String>,
}
