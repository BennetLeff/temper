...
| DC bank | `PfcPower38.VB_BANK` after F2 to half-bridge positive rail; `PfcPower38.HOT0` to half-bridge negative rail | Both high-current conductors, return, connector/cable or same-board copper, fault current, creepage, current rating, F2 behavior and physical layout must be specified and checked. The inverter cannot be connected to `VD_LOCAL` to bypass F2. |
