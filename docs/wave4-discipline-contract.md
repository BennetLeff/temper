...
| B10 | **CPython `repr(float)` exponent/NaN rendering vs Rust `{:?}`** — both languages use shortest-round-trip digit selection, so the mantissa digits always agree; the differences are (a) CPython always writes the exponent sign and zero-pads to two digits (`1e+300`, `1e-05`) where Rust writes `1e300`/`1e-5`, and (b) CPython writes `nan` where Rust writes `NaN`. Any dataclass `__repr__` with a float field (here `Violation.severity`/`.threshold`) diverges on these classes even when the stored bit patterns are identical | `repr(Violation(..., severity=1e-05, ...))` (Python `1e-05`) vs `1e-5` (Rust `{:?}`); `severity=nan` renders `nan` (Python) vs `NaN` (Rust) | Render float fields with a `py_float_str` helper: `nan` for NaN, then `format!("{v:?}")` with the exponent split/re-signed and zero-padded to two digits — never raw `{:?}` | `packages/temper-design-bundle/src/gates.rs` (`py_float_str`, ~line 89, with the divergence-class unit tests at ~line 122); pinned by `tests/placer/cp_sat/test_gates_rust_differential.py` full-repr assertions |
| B11 | **numpy constructor internals** — a numpy *constructor*'s output is not always the naive closed form. `np.linspace(start, stop, num)` computes `y[i] = i * step + start` with `step = (stop - start) / (num - 1)`, switches to `(i / div) * delta + start` when `step` underflows to exactly `0.0`, and then **assigns** `y[num - 1] = stop` rather than computing it — so the endpoint is bit-exact even where the formula would not be, and `num == 1` yields `start` (the endpoint assignment is skipped) | A Rust `start + i * step` mirror disagrees with numpy at the last sample of most ranges, and on every sample when `start == stop` | Replicate the constructor's own branch structure, not the formula: the `step == 0` alternate path, the assigned endpoint for `num > 1`, and the `num <= 1` cases. Verify against the installed numpy rather than the docs — this is implementation behaviour, not a documented contract | `packages/temper-thermal/src/thermal_potential.rs` (`linspace`, with the three branch tests); pinned by `tests/physics/test_thermal_potential_rust_differential.py::test_direct_build_grid_bit_exact` and `..._degenerate_and_denormal` (verified bit-for-bit over 4 000 random `(start, stop, num)` configurations) |
| B12 | **numpy ufunc NaN semantics differ from BOTH CPython builtins and Rust** — a third behaviour beyond B5. `np.maximum(a, b)` / `np.minimum(a, b)` **propagate** NaN from *either* operand; CPython's builtin `max`/`min` keep the *first* argument; `f64::max`/`f64::min` **discard** NaN. `np.clip(x, lo, hi)` expands to NaN-propagating `min(max(x, lo), hi)`, so a NaN in *any* of the three positions yields NaN, and an inverted `lo > hi` returns `hi` (measured: `np.clip(5.0, 10.0, 1.0) == 1.0`) — where Rust's `f64::clamp` *panics* on `min > max` or a NaN bound | `np.maximum(field, barrier)` mirrored with `f64::max` silently keeps the finite operand where the reference goes NaN; `np.clip` mirrored with `f64::clamp` panics on an inverted zone or a NaN board bound where the reference returns a value | Keep three distinct helpers and pick per call site by reading which *language's* operator the oracle used: `py_max`/`py_min` for CPython builtins, `np_maximum`/`np_minimum` for the ufuncs, `np_clip` for `np.clip`. Never `f64::max`/`min`/`clamp` | `packages/temper-thermal/src/hostmath.rs` (`py_max`, `np_maximum`, `np_clip`, with per-semantics unit tests); pinned by `tests/physics/test_thermal_potential_rust_differential.py::test_direct_phi_exclusion_nan_propagates_like_np_maximum`, `..._phi_coupling_nan_power_keeps_the_first_max_argument` and `test_module_assign_anchors_with_a_nan_board_bound` |
| B11 | **numpy pairwise summation** — `np.sum` over a float64 array is *not* naive left-to-right addition: numpy sums naively below 8 elements, with eight unrolled accumulators up to `PW_BLOCKSIZE` (128), and by recursive halving above it. Measured on numpy 2.3.5: naive summation disagrees with `np.sum` on 37–95% of random inputs for every n ≥ 8, and agrees exactly for n ≤ 7 | A shoelace `area = np.sum(cross)` reimplemented as a Rust `for … acc += x` fold diverges for any polygon with ≥ 8 vertices | Replicate `pairwise_sum_DOUBLE` (naive < 8; 8-way unrolled block, combined as `((r0+r1)+(r2+r3)) + ((r4+r5)+(r6+r7))`, tail added left-to-right; recurse at the largest multiple of 8 ≤ n/2 above 128) — and pin it directly against `np.sum` across all three branches, not only through the caller | `packages/temper-quality-oracle/src/placement_metrics.rs` (`numpy_pairwise_sum`); pinned by `tests/metrics/test_quality_rust_differential.py::test_numpy_pairwise_sum_matches_np_sum_across_branches` with the anti-vacuity companion `test_naive_summation_would_fail_the_pin`; `VERIFICATION.md` placement-metrics § |
| B12 | **CPython 3.12's compensated `sum()`** — the *builtin* `sum()` over floats is not naive addition either: CPython 3.12 switched it to the improved Kahan-Babuska (Neumaier) algorithm, carrying a compensation accumulator and returning `hi` (not `hi + c`) when `c == 0.0` to preserve `-0.0`. `np.sum`, builtin `sum()`, and a `+=` loop are **three mutually distinct** results | `total_component_area = sum(c.w * c.h for c in comps)` reimplemented as a Rust naive fold is 1 ulp off on ordinary inputs — this is how the class was found, via a `compactness_score` differential failure | Transcribe `builtin_sum_impl`'s float fast path: seed `hi` from the first element (`0 + x` promotes exactly), then per element `t = hi + x; c += (|hi| >= |x|) ? (hi - t) + x : (x - t) + hi; hi = t`, returning `hi` when `c == 0.0` else `hi + c`. Keep a *separate* naive helper for `+=` loops and use each exactly where the oracle does | `packages/temper-quality-oracle/src/placement_metrics.rs` (`py_builtin_sum` vs `naive_sum` vs `numpy_pairwise_sum`, with the divergence unit tests `builtin_sum_is_compensated_not_naive` / `builtin_sum_differs_from_pairwise_too` / `builtin_sum_preserves_negative_zero`); `VERIFICATION.md` placement-metrics § |
| B13 | **`RTLD_DEFAULT` is not `NULL` on Darwin** — the B1 mitigation depends on `dlsym` actually resolving. glibc defines `RTLD_DEFAULT` as `NULL`; Darwin defines it as `((void *) -2)`. Passing `NULL` on macOS is an *invalid handle*: `dlsym` returns null, the code silently falls back to the std intrinsic, and **LLVM lowers `f64::powf(x, 2.0)` to `x * x` and `f64::powf(x, 0.5)` to `sqrt(x)`** — reintroducing precisely the substitution B7 forbids, with no error anywhere | A `py_pow`-based clearance kernel silently computed `sqrt(dx*dx + dy*dy)`; the randomized differential missed it (the divergence rate is ~0.13%) and only the anti-vacuity mutation sweep exposed it, by showing that swapping in `sqrt` changed no test outcome | Select the handle per platform (`cfg!(target_vendor = "apple")` → `-2isize as *const u8`, else `null()`), and add a **resolution guard** test asserting `py_pow(…)` does *not* agree with the `sqrt`/`x*x` shape on search-found operands where CPython's `**` differs — a silent fallback must fail loudly | `packages/temper-quality-oracle/src/placement_metrics.rs` (`RTLD_DEFAULT`, `py_pow_resolves_to_host_libm_not_sqrt`); `tests/metrics/test_quality_rust_differential.py::test_pow_diagonal_operands_that_sqrt_gets_wrong`; `VERIFICATION.md` placement-metrics § |
