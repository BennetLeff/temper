...
**The clearance port is now the production path.** `verify_clearance()` uses
`temper_orchestration.run_clearance_check`, backed by
`temper_drc_rs.verify_route_clearance`; both `backend="auto"` and
`backend="rust"` select it, while `backend="python"` is retired. If either
extension symbol is absent, the safety check raises instead of silently
falling back to Python. The differential proof now calls the immutable
`tests/router_v6/_clearance_family_py_oracle.py` directly, and the presence
gate checks both extension modules so stale wheels cannot turn this proof into
an exit-0 skip.
