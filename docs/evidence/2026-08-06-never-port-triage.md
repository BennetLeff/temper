...
| `heuristics/` | 4,378 | PORT / NEVER-PORT split | `mcu_subsystem.py`, `power_stage.py`, `spectral.py` (networkx spectral layout), `structural.py`, `organizational.py`, `conflict.py`, `style.py`, `topological_init.py`, `graph_utils.py` (3,486 LOC) = PORT, nine real placement-heuristic algorithms. `__init__.py`, `base.py`, `pipeline.py` (892 LOC) = NEVER-PORT, package init/abstract base classes/orchestrator. **A sibling agent is porting this area now — not duplicated here.** *(2026-08-08 update: three of the nine are now ALREADY-DONE — `conflict.py`, `topological_init.py`, `power_stage.py` migrated to temper-placement-topology (`overlap_check`/`nudge_candidates`, `feasibility_check`, `clamp_position` in `heuristics.rs`); `spectral.py` is recorded JUSTIFIED-KEEP (networkx boundary) and `mcu_subsystem.py` NO-COMPUTE (structural proof) in `packages/temper-placement-topology/VERIFICATION.md`. `structural.py`/`organizational.py`/`style.py`/`graph_utils.py` retain the PORT verdict above.)* |
...
| `metrics/` | 645 | PORT / NEVER-PORT split | `aesthetic.py` + `physics.py` (408 LOC) = PORT, real numpy quality/physics metrics. `__init__.py` + `external_oracle.py` (237 LOC) = NEVER-PORT, re-export + thin adapter forwarding to other already-classified compute. **`aesthetic.py` is now ALREADY-DONE (2026-08-08 → temper-quality-oracle):** `compute_aesthetic_score` delegates to the Rust oracle, with a recorded divergence — the pre-migration module raised `NotImplementedError` (JAX retirement), so the differential pins the Rust implementation as the first working one (`tests/metrics/test_aesthetic_rust_differential.py`). `physics.py` retains the PORT verdict. |
...
| `requirements/` | 185 | PORT / NEVER-PORT split | `validators/_geometry.py` (175, `_distance`/`_point_in_rect`/`_rects_overlap` real geometry kernels) = PORT; `__init__.py` ×2 (10 LOC) = NEVER-PORT. **`validators/_geometry.py` is now ALREADY-DONE (2026-08-08 → temper-geometry):** the geometry kernels delegate to the `temper_geometry` crate, public names/signatures unchanged, with the pre-migration implementations pinned verbatim as `_oracle_*` functions in `tests/requirements/validators/test_geometry_rust_differential.py` (bit-identical parity). |
...
> **Actioned 2026-08-08** (Wave-4 Python→Rust migrations landing on `main`;
> each claim below verified against today's tree — the module is a delegation
> shim whose kernels import the named Rust crate). The modules annotated
> ALREADY-DONE in §2 above landed as follows:
>
> - `core/net_graph.py` + `core/differential_pair.py` → **temper-design-bundle**
>   (Wave C, 2026-08-08; plan `docs/plans/2026-08-08-001-feat-wavec-core-contracts-migration-plan.md`)
>   — `NetGraph`/`SubNetEdge` pyclasses in `net_graph_contracts.rs`,
>   `DifferentialPairConstraint` in `differential_pair_contracts.rs`; both
>   files are now pure-delegation re-exports (18/15 LOC).
> - `core/bus_cohort.py` `BusCohortConstraint` → **temper-design-bundle**
>   (2026-08-08; plan `docs/plans/2026-08-08-002-feat-buscohort-pyclass-migration-plan.md`,
>   DONE) — pyclass in `bus_cohort_contracts.rs`; `BusRegistry` stays Python
>   (registry state, not a numeric kernel).
> - `metrics/aesthetic.py` `compute_aesthetic_score` → **temper-quality-oracle**
>   (2026-08-08) — with a recorded divergence: the pre-migration module raised
>   `NotImplementedError` (JAX retirement), so the differential pins the Rust
>   oracle as the first working implementation
>   (`tests/metrics/test_aesthetic_rust_differential.py`).
> - `requirements/validators/_geometry.py` → **temper-geometry** (2026-08-08)
>   — `_distance`/`_point_in_rect`/`_rects_overlap` delegate to the crate's
>   geometry kernels; the pre-migration implementations are pinned verbatim as
>   `_oracle_*` functions (`tests/requirements/validators/test_geometry_rust_differential.py`).
> - `router_v6/channel_mapping.py` → **temper-geometry** (2026-08-08) — geometry
>   kernels delegate to the crate; public names/signatures unchanged.
> - `heuristics/{conflict,topological_init,power_stage}.py` →
>   **temper-placement-topology** (2026-08-08) — `overlap_check`/`nudge_candidates`,
>   `feasibility_check`, `clamp_position` in `heuristics.rs`; pinned oracles +
