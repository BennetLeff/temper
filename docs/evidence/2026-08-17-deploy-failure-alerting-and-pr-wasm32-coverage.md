<!-- provenance: commit=e81196c87b5998555feca78f27c612b11331bee7 dirty=true (HEAD at time of writing; this document's own workflow changes are the dirty state. Worktree /home/bennet/Desktop/temper/.claude/worktrees/agent-a7401e9e226726006, branch worktree-agent-a7401e9e226726006, based on origin/main e81196c87. pcb/temper.kicad_pcb untouched -- sha256 9c1f4a37b03c6433275704c3bed917f7ff16877c762f0aa8d37cc6858d7c16dd, unchanged throughout, matches docs/HANDOFF-2026-08-17.md. `packages/temper-rust-router-core/src/direct_topology.rs` not touched -- that is PR #1296's file. No deploy triggered, no merge performed.) -->
...
**This step is expected to fail on every PR until PR #1296 lands.** As of this commit, `direct_topology.rs`'s `Instant::now()` is still present on `main` — #1296 (`fix/wasm-tier-router-core-instant-panic`) fixes it and is open, mergeable, but not yet merged; I did not touch that file or that PR, per this task's constraints. Softening this new step (excluding `temper-rust-router-core`, `continue-on-error`, or any other narrowing) to keep it green in the meantime would be exactly the "make a check pass by weakening it" move this repo's operating rules forbid, and it would recreate the coverage gap for the one crate that just proved it real. I confirmed via `gh api repos/.../commits/main/check-runs` that `Rust Checks` is **already red on `main`, independent of this change** — a pre-existing `very complex type` clippy error in `temper-rust-router`, unrelated to the wasm-tier incident — so this addition does not newly block anything that was not already blocked, and it will go green on its own the moment #1296 merges, with no further edit needed here.
...
|---|---|
| PR-path wasm32 build+execute (all tiers) | Ran the exact build+execute pair locally for two real tiers (`temper-pcl-ir` happy path, exit 0; `temper-rust-router-core` failure path, exit 1, reproducing the incident's own panic text) before writing a single line of YAML around them. Fixed a `CARGO_TARGET_DIR` container-vs-bare-runner path bug the local run surfaced. |
