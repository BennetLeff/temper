<!-- provenance: commit=a3fbaff37afd739b72f2b109847813b30ceb8e88 (origin/fix/board-schematic-resync) dirty=false throughout (git status --porcelain clean apart from this document; git grep -l "^<<<<<<< " empty). Own git worktree (.claude/worktrees/creepage-pd3-gap-analysis, branch analysis/hv-creepage-pd3-gap), never the main checkout. pcb/temper.kicad_pcb sha256=b7d865b7946f55dcc0d907cccbbee12f730fd1878b30d417bd56004d1091c1d6, pcb/temper.kicad_pro sha256=f2d90755af04fea40357be3ba2ef94368a01b1afc34c450b42fad0b9e15a51ac -- byte-identical to power_pcb_dataset/drc_ceiling.json's own recorded provenance for this same commit, so every ceiling/count cited from that file applies to the exact board measured here. Extensions rebuilt in an isolated .venv (make venv-isolate, ~85s after `unset CONDA_PREFIX`, then make extensions); scripts/check_stale_extensions.py reported 10/10 fresh BEFORE and AFTER every measurement in this document, and every extension module (plus the temper_placer surface used for pad geometry) was independently confirmed to import successfully both times -- freshness alone is not sufficient per this session's own environment brief. `make netlist` was run in this worktree against its own origin/fix/board-schematic-resync base (exit 0, no errors, only atopile "Implicit Declaration Future Deprecation" warnings). kicad-cli 10.0.5 (/home/bennet/.local/bin/kicad-cli). No pcb/temper.kicad_pcb, footprint, DRU threshold, or enforced safety constant was edited anywhere in this task -- scripts/generate_kicad_dru.py's HV_CREEPAGE_ENFORCED_MM is untouched; every PD3 figure below was generated from a throwaway scratch copy of that file into a scratch path, never installed over the real generator or written into pcb/. No ratchet ceiling in power_pcb_dataset/drc_ceiling.json was raised. -->
