...
Usage::
    ...
    PYTHONPATH=packages/temper-placer/src python3 \\
        docs/evidence/2026-08-12-heatsink-colocation-pumpkin-run.py [--rot 0,1,2,3]
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--chains",
        action="store_true",
        help="add the protective-impedance chain co-location constraint",
    )
    ap.add_argument(
        "--chain-gap-mm",
        type=float,
        default=None,
        help="override MAX_CHAIN_GAP_MM (sensitivity sweep)",
    )
    args = ap.parse_args()
    ...
    chain_gap = MAX_CHAIN_GAP_MM if args.chain_gap_mm is None else float(args.chain_gap_mm)
    chain_pairs = resolve_chain_pairs(
        load_protective_impedance_chains(MANIFEST), netlist.components
    )
    print(f"[chains] {len(chain_pairs)} consecutive pairs resolved, gap<={chain_gap}mm")
    for p in chain_pairs:
        print(f"           {p.chain_name:<26} {p.a}-{p.b}  via {p.interior_net}")
    committed_chain_viols = check_chain_colocation(
        chain_pairs,
        {c.ref: tuple(c.initial_position) for c in netlist.components},
        {c.ref: int(c.initial_rotation) for c in netlist.components},
        refs_sizes,
        max_gap_mm=chain_gap,
    )
    print(f"[chains] committed board violates {len(committed_chain_viols)}/{len(chain_pairs)}")
    for v in committed_chain_viols:
        print(f"           {v.detail}")
    ...
    for rot in rots:
        ...
        if args.chains:
            chain_cs = chain_colocation_wire_constraints(
                chain_pairs, max_gap_mm=chain_gap, present_refs=frozenset(refs_sizes)
            )
            extra = extra + chain_cs
            print(f"    + {len(chain_cs)} protective-impedance chain constraints "
                  f"(gap<={chain_gap}mm)")
        ...
        if status in ("optimal", "feasible"):
            ...
            cviols = check_chain_colocation(
                chain_pairs, pos, rotations, refs_sizes, max_gap_mm=chain_gap
            )
            if cviols:
                print(f"       chain post-check FAILED ({len(cviols)}/{len(chain_pairs)}):")
                for v in cviols:
                    print(f"         {v.detail}")
            else:
                print(f"       chain post-check: all {len(chain_pairs)} pairs "
                      f"co-located within {chain_gap}mm")
            if args.out:
