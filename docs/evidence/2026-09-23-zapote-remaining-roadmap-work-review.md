...
| R6 integration | `zapote/integration/check.rs`: 7 tests; 31 source/native/receipt hashes; 39 matrix rows, 19 `BLOCKED`/20 `INDETERMINATE` | No integrated board or accepted cross-unit contract. Legacy 250 V sense cannot be joined directly to Rev38's ~390 V bus or SELV interlock. |
...
An independent integration review reproduced two row-level false downgrades: relabeling the voltage-fault edge as a current-fault connection, and editing the legacy voltage port from 250 to 450 V while leaving native/source bytes unchanged. The gate now binds all required edge endpoint/kind/contract tuples and fault input ports in Rust, and checks the 250 V legacy envelope against its locked source. Both mutation tests pass. The overall integration result remained blocked in the original reproductions and after these corrections.
