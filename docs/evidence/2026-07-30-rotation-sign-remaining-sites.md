# Rotation-sign sweep, round 2: 9 more sites beyond PR #479/#491
...
lint scoped to an exact 13-file list.
...
`.cos()`/`.sin()`) found 9 more call sites carrying the same shape of bug, none in the original
12/13 and none guarded by the lint. Each was classified individually against the test this whole
gate exists to apply: **does this site transform something that must agree with KiCad's own
placement, or is it self-contained with no KiCad correspondence?**
...
Six genuine bugs fixed; all now route through `temper_placer.geometry.kicad_transform` (the same
module PR #491 established), not a 19th independently-typed copy of the formula.
...
`scripts/check_no_raw_rotation_trig.py`'s `GUARDED_FILES` grew from 13 to 19 entries -- the 6 files
with genuine fixes above. `EXEMPT_FUNCTIONS` is now empty (the one entry it held, `_corners`, is
fixed rather than exempted). `polygon.rs`, `bench_rust_geometry.py`, `internal_route.py`'s import
breakage, and `core/state.py`'s dead functions are deliberately NOT guarded -- see the lint's own
docstring ("Second sweep" section) for the file-by-file reasoning. The enumerated-file-list design
itself is unchanged from PR #491 and, per that module's own docstring, still the right mechanism:
