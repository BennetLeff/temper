...
| `docs/evidence/k3_resolve_gated_drc.py`, `2026-08-04_domain_first_resolve_drc.py`, `k3_fixed_copper_repair_drc.py`, `k3_swap_board_write_drc.py` | archival one-off scripts behind several now-superseded `_march` entries in `drc_ceiling.json` | Not re-verified in this pass (out of live-CI scope; historical). | historical `_march` narrative only |
