...
TE Schrack RT314012 footprint after the swap commit):
...
- full domain-clearance set + unclassified-near-HV keep-away, generated
  from the fixture's FULL-classification placement, no chain-sibling
  exemption (matches the wall spike's 12,022 + 530 constraint counts)
- seed 0, 180s/round, max 4 rounds
- NOTE: ``run_clearance_repair_solve`` does not take ``fixed_copper``; the
  wall-spike variant B's fixed-copper-without-zone-items piece is NOT part
  of this caller's interface (documented follow-up; the re-solve evidence
  doc §recipe used this same caller).

Because K3's footprint is now the RT314012 (12.76mm internal coil-contact
gap), the REQ-SAFE-01 validator's intra-footprint bucket is expected to be
EMPTY on the solved placement (no intra violation exists for RT314012),
and hard_failures=[] — a HARD failure terminates the repair with
status="gap" and this script reports the classified violations.

NO src/ changes. Read-only w.r.t. pcb/ (the write happens in the next
step via the solved positions in the summary JSON).

Usage:
    uv run --no-sync python docs/evidence/scripts/k3_swap_board_write_solve.py
"""
...
def main() -> None:
    ...
    out = {
        ...
        # board-write step -- CP-SAT local frame, board origin added at
        # write time (the pd2-resolve write convention).
        "placement": {
            ref: {"position": list(p), "rotation_idx": report.final_rotations.get(ref, 0)}
            for ref, p in report.final_positions.items()
        },
    }
    out_path = REPO / "docs" / "evidence" / "k3_swap_board_write_solve_summary.json"
