...
- Commit measured: the wave4 `_mm` heal (the alias fix itself landed separately in #671)
...
Measured on the `_mm`-healed tree, on the same board, with byte-identical router output:
...
The Linux figures are from the `regression` job on that branch
...
**Caveat — the sampler omits `--all-track-errors`.** `tests/placer/cp_sat/_parallel_drc.py::run_drc_loud`
builds its `kicad-cli` argv without that flag (verified by reading the file; `#670` added it only
to the `Makefile`'s `drc` target, and `validation/_drc_api.py:330` is the other path that does
pass it). `_drc_api.py:305-320` documents the flag as load-bearing for determinism — without it
KiCad reports only a varying *subset* of the errors on each track, measured at `clearance`
334-343 and `shorting_items` 148-174 across 11 runs on a byte-identical board.

Every number in §2 — on **both** sides of the comparison — was taken through that same flagless
sampler, so the macOS↔container comparison remains apples-to-apples and the +107 finding stands.
But these are **not** absolute baseline-grade counts, and the observed spreads here (`total` 38,
`shorting_items` 22) are consistent with exactly the subset-variation that flag exists to remove.

Adding the flag to the sampler would move every count that has ever been measured through it,
including the constants those measurements seeded. It therefore belongs in its own change with
its own before/after — bundling it here would conflate two count-moving instrument fixes and
destroy the attribution this document exists to establish.
...
## 7. Established: the `_mm` fix improved connectivity by 3

`unconnected_items` measures **460** in *both* environments against a committed baseline of
**463**, with zero scatter in every sample. Attributable to the `_mm` fix (#671) restoring escape-via
