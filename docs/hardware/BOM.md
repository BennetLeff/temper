...
| J_RTD1 | PT100 Pan-Probe Connector | B4B-XH-A(LF)(SN) | JST Sales America | 1 | 1x4 THT 2.50mm | Board-side JST XH header, vertical. Pinout 1:Bias+ 2:Sense+ 3:Sense- 4:Bias-, mapping 1:1 onto the MAX31865's FORCE+/RTDIN+/RTDIN-/FORCE- (`modules.ato:2064-2069`). Mates with JST XHP-4 crimp housing (off-board, see `docs/CONNECTORS_AND_WIRING.md` §1/§2.3). |
...
>
> **`J_RTD1` added 2026-08-18 (R14 `wired_uncosted` finding).** The external PT100 pan-probe connector was instantiated in `RTDSensing` on 2026-08-07 (`ebb8aff20`, closing the PID-01..04 gap) and landed on `pcb/temper.kicad_pcb` in the 2026-08-15 resync (`96db2ccde`, which names `J_RTD1` in its own subject line) — but this document never absorbed either change, so the one connector the temperature-control loop physically cannot run without was **uncosted for 11 days on a board already carrying its footprint** (`Connector_JST:JST_XH_B4B-XH-A_1x04_P2.50mm_Vertical`, `pcb/temper.kicad_pcb:3793`). MPN/manufacturer transcribed from the component definition (`components.ato:767-783`), which records the part as DigiKey-verified (455-2249-ND). Only `J_RTD1` is instantiated: `docs/CONNECTORS_AND_WIRING.md`'s table also lists a `J_RTD2` heatsink-sensor connector, which has no `elec/src` counterpart and is therefore deliberately **not** costed here.
...
`components.ato:717` marks `TestPoint.bom_exclude = true` — these are not meant to be procured as BOM line items; listed here for rework/bring-up reference only, not for ordering. Source instantiates exactly **4** (`SafetyInterlock.tp_shutdown`, `SafetyInterlock.tp_fault`, `SafetyInterlock.tp_uvlo2_fault`, `SafetyInterlock.tp_ocp2_fault`), not the 5 originally listed nor the 2 counted 2026-07-25 — corrected from "exactly 3" on 2026-08-18, see the note below the table.
...
| TP_OCP2_FAULT | OCP2_FAULT test point | GENERIC_TEST_POINT | - | 1 | `safety.tp_ocp2_fault` (`modules.ato:3364`) — OCP-02 fault-output bench probe point. |
...
> **`TP_OCP2_FAULT` added 2026-08-18 (R14 `wired_uncosted` finding).** `SafetyInterlock.tp_ocp2_fault` has existed since `fb492106f` (2026-07-26) and this section's own headline count ("exactly 3") went stale the moment it landed — the count was a claim about source that nothing re-derived. It is a `bom_exclude` pad, so **nothing is procured against this line**; it is listed for the same rework/bring-up reason `TP1`-`TP3` are. Kept even though OCP-02 itself is de-scoped/DNF (§4.4): the OCP-02 *interface* is deliberately retained in `elec/src` per that section, and this pad probes `ocp2.fault.line` where it enters the interlock via `fault_or3`.
>
> **Why this row's `Ref` is `TP_OCP2_FAULT` and not `TP4`.** The R14 gate (`scripts/check_bom_source_reconciliation.py`) matches this column to `elec/src` variable names by normalized string identity, and `TP4` would match nothing — it would have to be silenced with a new `bom-reconciliation-allowlist.yaml` entry, the way `TP1`/`TP2`/`TP3` currently are. Suppressing a real finding to make a gate green is the failure mode that gate exists to prevent, so the label was chosen to reconcile on its own instead. `TP4` is also already spent: it named the removed `V_BOOT` point listed directly above.
...
| Microcontroller | 10 |
| Sensing (RTD chain, CT, CT bias) | 35 |
...
| **TOTAL** | **~166 (not re-verified against `elec/build/default.net` in this pass — see 2026-08-07 revision note; the 2026-08-18 delta is `J_RTD1` alone, §4.1)** |
...
| 1.9 | 2026-08-18 | **Closed the R14 gate's two remaining `wired_uncosted` findings — both BOM-stale, neither previously triaged.** `scripts/check_bom_source_reconciliation.py` had been exiting 3 on `main` (it is a hard, non-`continue-on-error` CI gate in the `consistency-gates` job) reporting `j_rtd1` (`modules.ato:2064`) and `tp_ocp2_fault` (`modules.ato:3364`) as instantiated in source with no BOM designator normalizing to match. Both confirmed real by reading source and board: **`J_RTD1`** — the PT100 pan-probe connector (JST `B4B-XH-A(LF)(SN)`), wired 2026-08-07 and placed on `pcb/temper.kicad_pcb` 2026-08-15, genuinely uncosted here until now (§4.1); **`TP_OCP2_FAULT`** — a `bom_exclude` test-point pad wired 2026-07-26, and the reason §10.2's "source instantiates exactly 3" count had been false since (§10.2). Fixed on the BOM side in both cases; no `elec/src` file and no board file was touched, and no allowlist entry was added — the findings were resolved by supplying the missing content, not by suppressing them. Category counts updated for `J_RTD1` (Sensing 34→35, TOTAL ~165→~166); test points are `bom_exclude` and are not in the category counts. |
