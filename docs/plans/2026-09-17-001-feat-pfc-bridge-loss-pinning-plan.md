...
- **Answered 2026-09-17 — see `zapote/power-entry/loss-budget/2026-09-17-rectifier-alternative-net-savings.md`.** The screen's numbers are narrower than they first appeared: the active-rectifier figures are a **conduction-only delta** (+5.80 W at 50 mOhm to -16.70 W at 100 mOhm, two evaluated assumptions rather than a design space), the passive ranges are conditional on assumed drop and slope, and bridgeless is unmodeled and needs its own current-path model. The governing relation is `P_saved = P_bridge - 450*R_hot - P_additional`, so the nominal 62.9 mOhm break-even is **51-78 mOhm before additional losses** across the committed 23-35 W bridge bracket. The bridge's 28.30 W is not the reducible prize, and the ~1.5 W bridge-versus-switch ordering stays unresolvable by any bridge measurement.
- **Next assignment:** choose one specific active-rectifier circuit with exact MOSFET and controller candidates, obtain their applicable hot Rds(on) data, quantify the additional losses, and compare across the bridge bracket. That establishes whether an opportunity exists before any measurement is commissioned. The broader architecture search stays open until those concrete comparisons exist.
