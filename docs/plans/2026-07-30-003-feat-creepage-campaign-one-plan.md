...
- **Open blockers:** The current 312-count creepage set needs reclassification under the converged trunk. Some same-package violations may require BOM or footprint decisions, and the board/router reconciliation tolerance must be made deterministic during planning. The production router's Stage 2.5 peak-memory blocker is resolved; scoped control candidates now round-trip with all 96 zones and the correct KiCad sidecars, but the first creepage pair has no legal path while inherited copper is preserved. An all-copper-stripped control routes the same net 1/1, proving the remaining blocker is inherited-copper congestion and requiring a bounded rip-up or broader reroute strategy. The control candidate's 20-sample creepage range is unchanged at 311–313.
...
- **Router handoff:** The current full-board production router did not emit a review-path candidate in two isolated attempts; bounded/net-scoped routing is tracked as GitHub issue #505. The Stage 2.5 occupancy-grid implementation now uses prepared geometry and bounded containment batches, verified by a metamorphic grid-equivalence test. A scoped `sclk` control emits a legal 1/1 candidate in about 7–9s, preserves the 96 committed zones, and measures with the matching `.kicad_pro`/`.kicad_dru` sidecars. Its 20-sample creepage range remains 311–313, so it is capability evidence only. The first creepage pair reports `no_path`, no blockers, and no ripups under the fail-closed forced-segment policy; removing all 2,482 committed copper blocks makes `discharge.k_dis2-nc` route 1/1, so the next router unit must address inherited-copper congestion without authorizing unsafe direct segments. No layout progress is credited.
- **Inherited-copper boundary:** The first rip-up design slice is report-only and typed in `router_v6/ripup_candidates.py`. It selects named track/via nets intersecting a route corridor, preserves the target net even when it has no existing copper, ranks candidates deterministically, and reports unnamed intersecting copper as an ownership gap. Hypothesis/metamorphic tests cover input-order invariance, corridor expansion monotonicity, irrelevant-copper isolation, and candidate-net stability. On the first failed `discharge.k_dis2-nc` corridor, the selector found 17 inherited blocker nets; stripping those 17 plus the target opened a temporary 1/1 route, but the selected nets were not rerouted, so this is blocker evidence only and earns no campaign credit. A follow-up repair test routed only 14/18 nets after stripping that set, with `WDT_KICK`, `a`, `discharge.k_dis2-nc`, and `power_in.r_zcd_top1-p2` still failing; a smaller top-two-plus-target set routed only 1/3. The next implementation must pair any bounded rip-up with rollback, reroute-and-verify for every selected net, and final DRC verification before it can emit a candidate board; fixed top-N density selection is not justified.
