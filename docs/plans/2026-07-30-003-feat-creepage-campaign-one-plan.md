...
- **Inherited-copper boundary:** The first rip-up design slice is report-only and typed in `router_v6/ripup_candidates.py`. It selects named track/via nets intersecting a route corridor, preserves the target net even when it has no existing copper, ranks candidates deterministically, and reports unnamed intersecting copper as an ownership gap. Hypothesis/metamorphic tests cover input-order invariance, corridor expansion monotonicity, irrelevant-copper isolation, and candidate-net stability. On the first failed `discharge.k_dis2-nc` corridor, the selector found 17 inherited blocker nets; stripping those 17 plus the target opened a temporary 1/1 route, but the selected nets were not rerouted, so this is blocker evidence only and earns no campaign credit. The next implementation must pair any bounded rip-up with reroute-and-verify for every selected net before it can emit a candidate board.
