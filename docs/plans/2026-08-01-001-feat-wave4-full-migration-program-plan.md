...
**Phase 1 verdict (2026-08-01, spike executed — `docs/evidence/2026-08-01-ortools-cpsat-spike.md`): KEEP the ortools CP-SAT engine and the Python solve boundary.** The spike enumerated the full constraint surface (13 CP-SAT constraint classes across 8 PCL handler types — the Phase B contract input). KEEP "done" is: ortools `==9.15.6755` version lock, frozen solver params (`max_time_in_seconds`, `num_search_workers` 8/4, `random_seed=0`, `log_search_progress=False`), a KTD9-style measured parity contract (bit-identical re-solve on the board corpus, tolerance 0), and the R24 post-solve audit (`placer/cp_sat/audit.py`, Rust-backed) enforced across the boundary. WRAP (Rust boundary via a vendored proto + ortools C API, same engine) is the recorded re-decidable path with triggers; REPLACE is rejected unless R1a is amended to semantic parity by product authority.

**Phase 1 blocker corrected (2026-08-04 — `docs/evidence/2026-08-04-wave4-residual-verdicts.md`). The verdict stands; the stated reason for it does not.** The 2026-08-01 spike recorded the blocker as feature coverage — "no pure-Rust engine covers the element / 2D no-overlap / product / assumption-core surface" — and marked Pumpkin's reification "needs-verification". Verified against the crate on 2026-08-04: Pumpkin 0.4.0 (released 2026-06-23) covers **12 of the 13 constraint classes** natively — `element` (C5), `times` (C7), `absolute` (C8), `clause` (C4), `table`, `cumulative`, `disjunctive_strict`, the linear family (C1/C2), `extract_core` for assumption cores (C12, the feature the spike assumed absent), and — decisively — `Constraint::implied_by`, documented as "post the constraint `r -> constraint`", which is exactly the half-reification every `OnlyEnforceIf` in this repo encodes (C3/M10, the highest-load row in the enumeration); `NegatableConstraint::reify` gives full `r <-> C` on top. The only missing native class is `AddNoOverlap2D` (C6) — which the spike's own §1.5 already records as *redundant for correctness*, kept only as a propagation strength hint. **So feature coverage is not the blocker and must not be cited as one.** The honest blocker is that acceptance is unassertable: (a) R1a's bit-identical bar cannot be asserted across solver engines by construction (heuristic search); (b) search quality on this board class is unmeasured for every candidate; (c) every pure-Rust candidate is pre-1.0 (Pumpkin 0.4.0, ~8.9k downloads); and (d) what "good enough" means here is a human judgment about candidate quality under a wall-clock budget, which no gate in this repo expresses. **(d) is the one that must fall first** — without a gate expressing solution acceptance, (b) has no pass mark to measure against, and the KEEP is re-decidable only in name.

**Two defects in the recorded solve contract (2026-08-04), which the KEEP verdict depends on and which are not yet fixed.** §4 of the spike claims the frozen params hold "at both call sites (`model.py:434-436`, `_encoder_solve.py:390-394`)". Against `origin/main`, there are **three** solve sites with three different determinism postures:

| Site | `random_seed` | `num_search_workers` | Reproducible? |
|---|---|---|---|
| `cp_sat/_encoder_solve.py:497-500` | `= seed` | `= 4` | yes |
| `cp_sat/model.py:434-436` | **never set** | `= 8` | seed defaults to 0, so stable in practice — but unpinned, and the spike wrongly records it as frozen |
| `cp_sat/unsat.py:275-277` | **never set** | **never set** | **no** — defaults to 0 = auto-detect worker count |

The `unsat.py` case is the material one: `_check_assumptions_infeasible` leaves `num_search_workers` at the auto-detect default, so the worker count varies with the host's core count and **the minimal unsat core is not reproducible across machines**. That is the MUS-refinement path the spike identifies as the portable half of the core surface. Both are prerequisites of KEEP acceptance criterion 1 (version-locked solve contract), so that criterion is **not met today** — fixing them is a code change, tracked separately from this decision record.

**Version floor is looser than the spike states.** The spike names `pyproject.toml:28` (`ortools>=9.12`) as the floor to tighten. `packages/temper-placer/pyproject.toml` declares ortools **twice** — `>=9.12` at line 28 and `>=9.10` at line 43 — and both are carried into `uv.lock`'s resolved requirement set. `uv.lock` pins 9.15.6755, so today's install matches the measured version, but package metadata permits floating well past it and the effective floor is the looser 9.10. Both declarations must be tightened, not just line 28.
