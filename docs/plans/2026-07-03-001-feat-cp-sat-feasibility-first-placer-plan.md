...
- R9. Retire: the `optimizer/` directory (all files), the `losses/` directory (all files), `pcl/loss_bridge.py`, the `placement/` directory (analytical, spectral, constraint_weights, benders_loop, legalization), and related JAX-specific modules. Deletion is gated on CP-SAT matching-or-beating JAX on the oracle for the temper board. The JAX pipeline is not deleted outright on gate-close: it is retained behind `--placer jax-deprecated` for exactly one release cycle as a rollback path if a board outside the 5-board regression corpus surfaces a CP-SAT pathology. After one release cycle green on the corpus, `jax-deprecated` is removed in a follow-up PR (tracked under Deferred to Follow-Up Work).
...
- R11. CP-SAT placer runs alongside the JAX pipeline (feature-flagged via `--placer cp-sat`; JAX default `--placer jax` retained during the strangler window) until CP-SAT matches-or-beats JAX on the oracle. Both run; neither is deleted until the gate condition is met. After gate-close, `--placer jax` is renamed to `--placer jax-deprecated` (per R9) for a one-release-cycle rollback window.
...
- Remove `--placer jax-deprecated` rollback path after one release cycle green on the 5-board regression corpus. Tracked here (not in U9) because U9 lands the wrapper, but the wrapper's removal requires a full release cycle of corpus-green evidence to complete.
...
- `packages/temper-placer/src/temper_placer/cli/__init__.py` — Click CLI group. The `optimize` command is a monolithic function that hardcodes the JAX pipeline; placer dispatch will be an inline conditional branch within it.
...
- **Near-full JAX retirement (`--placer jax-deprecated` rollback window for one release cycle — a deliberate deviation from origin)**: The origin brainstorm (`docs/brainstorms/2026-07-03-cp-sat-feasibility-first-placer-paradigm-swap-requirements.md`) Key Decisions state "Full JAX retirement (no `--legacy-jax` flag, no permanent archive)" and "flag lifetime settled at zero releases." The post-review finding "no-recovery deletion" overrides this: deleting JAX outright with no rollback path is unsafe because the 5-board regression corpus cannot exhaustively anticipate every board shape CP-SAT might choke on. The plan therefore retains `--placer jax-deprecated` for exactly one release cycle as a rollback path if a non-corpus board surfaces a CP-SAT pathology; after one cycle green on the corpus, the deprecated path is removed in a follow-up PR (tracked under Deferred to Follow-Up Work). The deviation narrows the brainstorm's "no flag" intent to "no *permanent* flag" — flag lifetime is settled at *one* release cycle, not zero. The five dead activation features (C-CAP, Constraint-Weighted Laplacian, Group Pre-Clustering, Thermal Anchoring, DPP Multi-Seed) retire with the JAX descent stack at gate-close.
...
- **CP-SAT solve timeout**: 60 seconds in CI (`max_time_in_seconds=60`), 300 seconds in production (`max_time_in_seconds=300`). Timeout with a feasible solution is success (CP-SAT finds feasible solutions quickly); timeout without any solution is a soft failure — not UNSAT, but a retryable condition.
...
- **JAX retirement gate un-bundled from the `2026-07-02-001` experiment**: CP-SAT matching-or-beating the oracle alone triggers retirement. The in-flight JAX experiment runs to completion as information-only, not as a gate. This aligns with the brainstorm's own logic that "the paradigm is wrong-fit independent of any one empirical result."

  **Decision log (recorded per the post-review finding):**
  - **Origin requirement:** the brainstorm doc `docs/brainstorms/2026-07-03-cp-sat-feasibility-first-placer-paradigm-swap-requirements.md` Key Decisions states deletion is "gated on BOTH the experiment completing AND CP-SAT matching-or-beating on the oracle."
  - **Plan deviation:** this plan gates retirement on oracle parity alone, not on experiment completion.
  - **Rationale for deviation:** the brainstorm's own "experimentation phase precedes deletion" decision treats the in-flight `2026-07-02-001` multi-seed JAX experiment as *informational backstop* (buyer's-remorse insurance), not as a *gate*. The brainstorm's Key Decision "Experimentation phase precedes deletion" defines the experiment's role as: run to completion as informational backstop; its verdict wouldn't change the swap. If the experiment were a gate, the brainstorm's own logic ("the paradigm is wrong-fit independent of any one empirical result") would be contradicted — the experiment's verdict would have the power to reverse the paradigm decision. The brainstorm explicitly disclaims that.
  - **Exception:** if the experiment returns the INCONCLUSIVE verdict with explicit evidence of *JAX-side* regressions (not CP-SAT-side problems) — e.g., C-CAP oscillation across all 10 seeds — that finding is informational only; it does not block retirement. It is recorded alongside the retirement PR for future optimizer historians.
  - **Review trail:** this deviation was flagged in the doc review as "experiment-unbundling undocumented," and is now documented here as the explicit resolution.
...
```mermaid
graph TB
    U0["U0: Feasibility Spike (✅ passed, ~62s)"] --> U1["U1: Dependency & Compilation Target"]
    U1 --> U2["U2: CP-SAT Model"]
    ...
    U8 --> U9["U9: JAX Retirement (jax-deprecated rollback window)"]
    U9 --> U10["U10: CI & Golden Regeneration"]
```

### Phase 0: Feasibility Spike

### U0. CP-SAT Feasibility Spike on Temper Board

**Goal:** Before committing to the 10-unit implementation, validate that OR-Tools CP-SAT can solve the actual temper board's hard-constraint set end-to-end (parse → encode → solve → audit) within a CI-acceptable wall time. This unit de-risks the entire plan: if CP-SAT cannot find a feasible placement on the 33-part temper board, the paradigm swap is blocked until the spike clears.

**Requirements:** R1 (NoOverlap2D), R2 (clearance), R3 (edge anchoring), R4 (adjacency), R5 (region membership) — covered at feasibility-only resolution (no soft objective yet)

**Dependencies:** None (this is a throwaway spike; its learnings inform U1–U10 but its code is not consumed)

**Files:**
- Create: `packages/temper-placer/spikes/cp_sat_feasibility.py` (standalone script; not under `src/`, not imported by anything)

**Approach:**
- Single-file script that:
  1. Loads the temper board's component list (33 parts), board outline, and PCL constraints (`configs/pcl/temper_induction.yaml`) using existing parsers
  2. Hard-codes a minimal CP-SAT model: `NoOverlap2D` over bounding-box rectangles on a 0.1mm grid + the 4 supported hard constraint types (HV↔LV Chebyshev clearance with ×√2 safety factor, thermal-edge anchoring, commutation-loop adjacency, HV-region membership)
  3. Solves feasibility-only (no wirelength objective) with `max_time_in_seconds=120`, `num_search_workers=8`
  4. Audits the output against ground-truth geometry predicates for every hard constraint (the same audit U4 will later formalize — here it's inline in the spike)
  5. Prints: status (FEASIBLE/INFEASIBLE/UNKNOWN), solve wall time, violation count, and the placement if feasible
- No PCL→CP-SAT encoder reuse — constraints are encoded inline. U3 builds the real encoder later.
- No CLI integration — invoked as `python spikes/cp_sat_feasibility.py`
- No tests — the spike is one-shot evidence-gathering, not production code. The artifact it leaves is a recorded result, not a test suite.

**Pre-registration:** the spike **passes** if it returns FEASIBLE with zero audit violations within 120 seconds on the temper board on CI hardware. It **fails** if it returns INFEASIBLE, UNKNOWN, or any audit violation. A fail blocks the plan; a pass unblocks Phases 1–4.

**Status: ✅ PASSED (2026-07-03)** — CP-SAT found a feasible, audit-clean placement for the 33-part temper board in ~62 seconds wall time on the dev machine. This unblocks the implementation units below. The recorded result lives in the spike script's stdout and in the plan's revision history.

**Test scenarios:**
- Spike-only: invoke `python spikes/cp_sat_feasibility.py` → exits 0 with "FEASIBLE, 0 violations, 62s" (already run; result recorded above).

**Verification:**
- Spike result recorded in the plan status line above (PASS in ~62s).
- The 10-unit implementation plan is unblocked.
- If the spike is re-run on different hardware and times out, that does not retroactively block the plan — the recorded pass stands. Re-running is informational.
...
- **Update the `supported_targets` frozenset on the four v1-supported constraint types** to include `CompilationTarget.CP_SAT`:
  - `SEPARATED` → `frozenset({CompilationTarget.JAX, CompilationTarget.SAT, CompilationTarget.DRC, CompilationTarget.CP_SAT})`
  - `ENCLOSING` → same four-target set
  - `ON_SIDE` → same four-target set
  - `ADJACENT` → same four-target set
- Leave the unsupported-in-v1 constraint types' frozensets unchanged (`KEEPOUT`, `ALIGNED`, `ANCHORED`, `LOOP_AREA` retain their existing `{JAX, SAT, DRC}` or `{JAX, DRC}` sets — they intentionally do NOT include CP_SAT, so U3's encoder logs a "not supported by CP-SAT v1 encoder" warning for them, matching the existing dispatch behavior).
...
- Add a `supported_targets` invariant test: for each of the four v1-supported types, `ConstraintType.X.supported_targets()` returns a frozenset containing `CompilationTarget.CP_SAT`; for each unsupported type, it does not. This prevents accidental frozenset drift in future constraint-type additions.
...
- The review finding "frozenset omission + supported_targets not updated" — adding an enum value without updating the per-type frozensets silently leaves every constraint type unsupported by the new target; the invariant test guards against this regression class.
...
- Happy path: `ConstraintType.SEPARATED.supported_targets()` returns a frozenset containing `CompilationTarget.CP_SAT` (and the existing JAX/SAT/DRC members)
- Happy path: same assertion for `ENCLOSING`, `ON_SIDE`, `ADJACENT`
- Happy path: `ConstraintType.KEEPOUT.supported_targets()` does NOT contain `CompilationTarget.CP_SAT` (intentional v1 exclusion — would catch a future over-broad frozenset edit)
...
- **Cold-start bootstrap:** when no prior placement file exists on disk (first CI run, fresh sandbox), the solver falls back to either (a) extended 300s timeout OR (b) a JAX-analytical-path hint computed inline (cheap point estimate fed to `model.AddHint`). Choice between (a) and (b) is configurable via `--cp-sat-cold-start {timeout,hint}`; default is `hint` for temper-sized boards. The cold-start path is documented and tested explicitly — per the review finding, warm-start alone is a blind spot because its precondition (prior solution file) is silently false on first run.
...
- Cold-start: invoke solve with no prior placement file on disk and `--cp-sat-cold-start hint` → JAX-analytical hint computed inline, fed to `model.AddHint`, solve completes within timeout
- Cold-start: invoke solve with no prior placement file on disk and `--cp-sat-cold-start timeout` → solver runs with extended `max_time_in_seconds=300`, no hint provided, still produces a feasible solution on the temper board
...
- Modify: `packages/temper-placer/src/temper_placer/core/state.py` (add `from_numpy()` factory that accepts raw (x, y) positions without requiring JAX arrays at the call site — wraps numpy→JAX internally during strangler period)
...
- `score_placement()` is importable and callable without the caller needing to `import jax` — `PlacementState.from_numpy(positions)` accepts a plain `{ref: (x_mm, y_mm)}` dict and wraps numpy→JAX internally. **Verification criterion is deliberately relaxed during the strangler period:** `core/state.py` may still `import jax` at module level ( PlacementState internally stores `jax.Array`); the public contract is that the *caller* of `score_placement()` does not need JAX in their namespace. Full JAX decoupling from `core/state.py` is deferred to the post-U9 dependency audit (see Deferred to Follow-Up Work). After U9, the relaxed criterion is re-tightened: `core/state.py` must not import JAX at module level.
...
- Dispatch is a conditional branch directly inside `cli/__init__.py`'s `optimize()` function — when `--placer cp-sat`, the CP-SAT pipeline runs inline within the same monolithic function; when `--placer jax`, the existing JAX path is unchanged. This mirrors how the existing JAX-specific flags are handled within the same function and is the lowest-risk approach during the strangler period. No refactoring of `optimize()` into `runner.py` delegation.
...
**Patterns to follow:**
- `packages/temper-placer/src/temper_placer/cli/__init__.py` — existing `@click.option` decorators and monolithic `optimize()` function structure (inline conditional branch for CP-SAT, same pattern as existing JAX-specific flag handling)
...
- Compares on each individual metric:
  ...
  - `router_v6_completion_rate` → CP-SAT must be ≥ JAX (or within 2% tolerance) — **routability is a hard blocking parity gate, not warning-only.** Per the review finding: the original risk-table scoping of router_v6 failure as "warning-only during strangler" had no post-retirement recovery path. Once JAX is deleted (after U9), a non-routing CP-SAT placement can't be ruled out by reverting to JAX. So routability parity must hold *before* retirement is allowed, not flagged for follow-up. The `--placer jax-deprecated` rollback window (per R9) provides the post-retirement recovery path if a non-corpus board later fails, but within the corpus the routability gate blocks retirement.
  - `total_manhattan_wirelength` → CP-SAT must be ≤ JAX × 1.05 (CP-SAT allowed up to 5% *worse* wirelength than JAX baseline). Per the review finding: origin AE1 explicitly requires "wirelength ≤ JAX baseline's wirelength" but the original U8 metric list omitted it. The 5% tolerance acknowledges that CP-SAT optimizes wirelength as a soft tiebreaker, not a primary objective, while still preventing wirelength regressions that would indicate the soft objective is mis-weighted.
...
- Covers AE1 (zero-overlap + zero HV↔LV violations + wirelength within 5% of JAX baseline), AE2 (oracle scores comparable), AE4 (JAX retirement gate)
...
- AE1 wirelength coverage note: `total_manhattan_wirelength` is asserted ≤ JAX_baseline × 1.05 (5% tolerance per the post-review fix). AE1's origin phrasing is "≤ JAX baseline's wirelength"; the 5% tolerance is the plan's per-metric relaxation, documented here and in the per-metric gate list above. ±5% keeps the soft-objective tiebreaker from being mis-weighted without forcing CP-SAT to outperform JAX on a metric that is not its primary strength.
...
- Parity gate: CP-SAT scores ≥ JAX on all individual metrics (clearance 3mm, clearance 6mm, thermal, routability completion), AND CP-SAT wirelength ≤ JAX × 1.05 → test passes
- Parity gate: CP-SAT beats JAX on clearance but loses on routability → test fails with per-metric breakdown
- Parity gate: CP-SAT matches JAX on all metrics but wirelength > JAX × 1.05 → test fails with per-metric breakdown (wirelength regression — soft-objective likely mis-weighted)
...
- Modify: `packages/temper-placer/src/temper_placer/cli/__init__.py` (rename `--placer jax` to `--placer jax-deprecated`; remove all other `--placer` choices; CP-SAT is default)
- Modify: `packages/temper-placer/src/temper_placer/router_v6/pipeline.py` (remove `from temper_placer.optimizer` / `placement` imports; the router_v6 entry point must not pull JAX paths at import time)
- Modify: `packages/temper-placer/src/temper_placer/pipeline/orchestrator.py` (remove `PipelinePhase.GEOMETRIC` → JAX pipeline path; orchestrator must import cleanly without optimizer/)
- Modify: `packages/temper-placer/src/temper_placer/pipeline/feedback.py`, `auto_layout.py`, `geometric.py`, `topological.py` (remove optimizer/losses/placement imports)
- Modify: `packages/temper-placer/src/temper_placer/pipeline/stages/topological_stage.py`, `input_stage.py`, `thermal_anchoring_stage.py`, `geometric_stage.py`, `refinement_stage.py` (remove JAX-stage imports; `geometric_stage.py` and `refinement_stage.py` likely retire entirely or become CP-SAT no-ops)
- Modify: `packages/temper-placer/src/temper_placer/heuristics/pipeline.py` (remove optimizer/losses/placement imports)
- Modify: `packages/temper-placer/src/temper_placer/ablation/registry.py`, `ablation/runner.py` (remove optimizer/losses imports; ablation entries that target JAX-only knobs retire)
- Modify: `packages/temper-placer/src/temper_placer/adapters/register_strategies.py`, `adapters/placement_adapter.py` (remove optimizer/placement imports; the JAX placement adapter retires)
- Modify: `packages/temper-placer/src/temper_placer/regression/corpus_runner.py`, `regression/multi_seed_experiment.py`, `regression/physics_oracle.py` (remove optimizer/train.py / train_multiphase imports; oracle retains only `score_placement()` path)
- Modify: `packages/temper-placer/src/temper_placer/validation/human_reference_extractor.py` (remove JAX placement imports; reference extraction reads human PCB directly)
- Modify: `packages/temper-placer/src/temper_placer/pcl/parser.py`, `pcl/sat_bridge.py` (remove loss_bridge / JAX-backend references; SAT bridge unchanged)
- Modify: `packages/temper-placer/pyproject.toml` (remove JAX/optax/flax from dependencies only after the import-grep verification clears)
- Test: Update all tests that imported from deleted modules — replace with CP-SAT equivalents or remove

**Approach:**
- This unit is a **post-gate cleanup** — it only runs after the parity test (U8) passes in CI. **However**: the JAX pipeline is not deleted outright here. Per the review finding on no-recovery deletion, `--placer jax` is renamed to `--placer jax-deprecated` and retained for exactly one release cycle as a rollback path if a board outside the 5-board regression corpus surfaces a CP-SAT pathology. After one release cycle green on the corpus, `jax-deprecated` is removed in a follow-up PR (tracked under Deferred to Follow-Up Work, not this unit).
- Remove all JAX optimizer imports from `cli/__init__.py` and every call site listed in the Modify block above. The grep-must-be-empty verification (below) covers all of these — failing the gate blocks the PR.
- Remove `PipelinePhase.GEOMETRIC` → JAX pipeline path from `orchestrator.py`
- Run the verification greps:
  - `rg "from temper_placer\.optimizer" src/temper_placer/` → zero matches
  - `rg "from temper_placer\.losses" src/temper_placer/` → zero matches
  - `rg "loss_bridge" src/temper_placer/` → zero matches in import statements
  - `rg "from temper_placer\.placement" src/temper_placer/` → zero matches (placement/ retires entirely, not just selected files)
  - `rg "from \.\.\.(optimizer|losses|placement)\b" src/temper_placer/` → zero matches (catches relative imports)
  - `rg "import jax" src/temper_placer/` → only survivors (physics metrics, core/state during strangler) — full JAX removal deferred
- Remove `jax`, `jaxlib`, `optax`, `flax` from `pyproject.toml` only if `rg "import jax" src/temper_placer/` returns zero results
- Rename `--placer` choices to `["cp-sat", "jax-deprecated"]` until the follow-up removal; `cp-sat` is default. After the one-cycle window, `--placer` is removed entirely and `cp-sat` is the only path.
- Remove JAX-specific CLI flags (`--epochs`, `--weight-overlap`, `--curriculum`, `--ccap`, `--grad-norm`, etc.) — these are no longer applicable. (They can remain recognized but ignored under `--placer jax-deprecated` for the rollback window, or be removed outright per implementer choice.)
...
**Test scenarios:**
- Happy path: `temper optimize temper.kicad_pcb --config pcl/temper_induction.yaml` runs CP-SAT (no --placer flag needed; CP-SAT is default)
- Happy path: `temper optimize --placer jax-deprecated temper.kicad_pcb --config pcl/temper_induction.yaml` still runs the JAX pipeline (rollback path available for one release cycle)
...
- Happy path: `rg "from temper_placer\.placement" src/temper_placer/` returns zero matches
- Happy path: `rg "loss_bridge" src/temper_placer/` returns zero matches in import statements
- Happy path: `rg "from \.\.\.(optimizer|losses|placement)\b" src/temper_placer/` returns zero matches
- Edge case: `router_v6/pipeline.py`, `pipeline/orchestrator.py`, `ablation/registry.py`, `adapters/register_strategies.py` import cleanly with no optimizer/losses/placement references — verified by `python -c "import temper_placer.router_v6.pipeline; import temper_placer.pipeline.orchestrator; import temper_placer.ablation.registry; import temper_placer.adapters.register_strategies"`
- Edge case: Import of retired modules from `tests/` — all test imports updated or tests removed
- Edge case: JAX still needed by surviving modules (physics metrics, core/state during strangler) — JAX stays in deps, but optimizer/losses imports are gone
- Edge case: A regressor board outside the 5-board corpus fails CP-SAT in production → operator re-runs with `--placer jax-deprecated` to recover; this is the rollback path the review finding required

**Verification:**
- `uv sync` succeeds with updated dependencies
- `temper optimize` runs CP-SAT by default (no JAX主动 path reachable through the default dispatch)
- `temper optimize --placer jax-deprecated` still runs the JAX pipeline (rollback window open)
- Zero imports of `optimizer/`, `losses/`, `placement/`, or `loss_bridge.py` anywhere in the codebase — verified by the extended grep patterns above
- The four review-flagged import surfaces (`router_v6/pipeline.py`, `heuristics/pipeline.py`, `ablation/`, `adapters/`) import cleanly under the deletion
...
- **Interaction graph:** The `temper optimize` command changes its internal placer dispatch via an inline conditional branch. `pipeline/orchestrator.py` may lose the `GEOMETRIC` phase after retirement.
...
|------|-----------|--------|------------|
| CP-SAT model encoding is harder than expected — solve times exceed 300s for temper board | Medium | High | Warm-start from analytical placer (`model.AddHint()`); tighten variable domains aggressively; set 60s CI / 300s prod timeout as guardrail. **Cold-start bootstrap (per review finding "cold-start timeout blind spot"):** warm-start requires a prior solution, which does not exist on a fresh CI sandbox's first run. On cold start (no prior solution file present), the placer falls back to one of: (a) extended timeout of 300s (matches the prod budget) for the first solve, OR (b) a JAX-derived hint computed from the `--placer jax` analytical path (cheap; produces a coarse-but-feasible initial point for `model.AddHint`). Both paths are gated behind a "is there a prior solution file on disk?" check. The cold-start bootstrap is implemented in U2 alongside the warm-start path; failure modes for both are distinguished in logs. |
...
| Router_v6 fails to route CP-SAT placements (new placement shape unfamiliar to router) | Medium | Medium | **Routability is a hard parity gate in U8** (per review finding "routability not a blocking gate"). If router_v6_completion_rate falls below JAX baseline (outside the 2% tolerance), the retirement gate fails and U9 is blocked. During the strangler *pre-parity* runs (U6 A/B experiments while parity is being tuned), routability failures are warning-only so the placer can be iterated on; once U8 is invoked as the retirement gate, the warning-only window closes. Post-retirement recovery (if a non-corpus board fails to route) is the `--placer jax-deprecated` rollback path, retained for one release cycle per R9. |
...
| Survivor modules depend on JAX types that were assumed universal — `score_placement()` can't construct `PlacementState` without JAX | Medium | Medium | U5 adds `PlacementState.from_numpy()` factory in `core/state.py` that wraps numpy→JAX internally. During strangler period, JAX remains installed so the wrap is sound. Full `core/state.py` JAX decoupling is deferred to post-U9 dependency audit (see Deferred to Follow-Up Work). U5 verification criterion relaxed accordingly. |
