"""
Pinned Python oracle for ``temper_placer/pcl/tag_dispatch.py`` (Wave 4, Phase 2).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/pcl/tag_dispatch.py`` as of commit ``5a17025b1`` (the last
commit to touch that file before this migration; the Wave-4 base is
``facaed149``). Nothing below this docstring was edited -- not one
character -- so the differential in
``test_tag_dispatch_rust_differential.py`` compares against the exact
bytes that shipped.
