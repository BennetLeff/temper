"""
Pinned Python oracle for ``temper_placer/pcl/_parse_utils.py`` (Wave 4, Phase 2).

VERBATIM copy of the pre-migration implementation as of commit ``5a17025b1``
(Wave-4 base ``facaed149``). Nothing below this docstring was edited.

DO NOT EDIT THE SEMANTICS -- this is the oracle the Rust implementation must
reproduce bit-identically. The enum types it returns (``Axis``, ``BoardSide``,
``ConstraintTier``, ``DistanceMetric``, ``EdgeType``) are imported from the
live module because they are *unchanged* by this migration: they stay Python
``enum.Enum`` classes, so oracle and Rust return the very same singletons and
the differential compares object identity, not a re-declared copy.

Original module docstring: PCL parsing utilities -- unit conversion, enum
parsing, and PCLParseError.
"""
