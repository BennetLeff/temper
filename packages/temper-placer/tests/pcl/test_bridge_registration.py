...
class TestBridgeRegistrationCompleteness:
    """Every ConstraintType with SAT in supported_targets has a handler."""
    ...
    def test_backends_registry_has_all_targets(self):
        """BaseConstraint.backends contains sat (DRC bridge retired 2026-08-09)."""
        assert "sat" in BaseConstraint.backends, "sat backend missing"
        # The DRC bridge was retired (2026-08-09); only sat remains.
        assert "drc" not in BaseConstraint.backends, "drc backend should not exist"
