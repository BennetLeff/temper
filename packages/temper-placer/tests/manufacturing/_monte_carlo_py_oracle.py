"""
Pinned Python oracle for ``temper_placer/manufacturing/monte_carlo.py``
(Wave 4, Phase 4 leftovers slice).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/manufacturing/monte_carlo.py`` as of commit
``58b302ce8`` (the tip of ``feat/wave4-phase4-leftovers-rust`` before the
monte_carlo migration). The only third-party imports are ``numpy`` (the
``Generator`` RNG, the vectorized kernel, and the ``mean``/``std``
aggregations) — exactly what the pre-migration module itself imported.
