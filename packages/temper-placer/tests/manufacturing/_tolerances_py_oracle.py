"""
Pinned Python oracle for ``temper_placer/manufacturing/tolerances.py``
(Wave 4, Phase 4 leftovers slice).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/manufacturing/tolerances.py`` as of commit ``6290942be``
(origin/main). The module imports only the Python stdlib (``dataclasses``,
``enum``), so no relative imports needed rewriting.
