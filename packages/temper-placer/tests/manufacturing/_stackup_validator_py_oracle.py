"""
Pinned Python oracle for ``temper_placer/manufacturing/stackup_validator.py``
(Wave 4, Phase 4 leftovers slice).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/manufacturing/stackup_validator.py`` as of commit
``6290942be`` (origin/main). The only non-stdlib import is
``from temper_placer.core.board import LayerStackup``, which resolves to the
Rust pyclass on the current branch — exactly what the pre-migration module
itself imported.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust pyo3 pyclasses
(``temper_io_types``) must reproduce bit-identically; any edit here
silently weakens the differential proof. If the module's contract changes,
the oracle must be re-pinned from the new base first.
"""
