"""Pinned pre-migration oracle for ``temper_placer.physics.thermal_potential``.

**Do not edit.**  This is a *verbatim* copy of the module's compute
functions AS COMMITTED at ``origin/main`` before the Rust kernels in
``packages/temper-thermal/src/thermal_potential.rs`` existed
(``f57b52d51``).  It is the reference the Wave-4 R1a differential is
pinned to; changing it would move the goalposts rather than test the
migration.

Only the pure compute is copied.  The safety gates
