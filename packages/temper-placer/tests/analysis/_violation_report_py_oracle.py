"""
Pinned Python oracle for ``temper_placer/analysis/_violation_report.py``
(Wave 4, Phase 4 — the analysis surface migration).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/analysis/_violation_report.py`` as of commit ``c5875adad``
(origin/main, the Wave-4 Phase-4 base for this migration), with only the
original module docstring replaced by this pin note. Its module-level
imports (``kiutils.board``, ``temper_placer.io.kicad_metadata``,
``temper_placer.validation._drc_api``) all resolve in the test
environment, so no import rewriting was needed.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust ``temper_drc_rs``
pyfunctions (``build_report_rows``, ``render_report``) must reproduce
