...
def test_filters_to_target_rules_only():
    errors = [
        DrcError(rule="courtyards_overlap", severity="error",
                 location=(10.0, 20.0), message="Courtyards overlap",
                 components=["D3", "C4"]),
        DrcError(rule="clearance", severity="error",
                 location=(5.0, 5.0), message="Clearance violation",
                 components=["R1"]),
        DrcError(rule="pth_inside_courtyard", severity="error",
                 location=(30.0, 40.0), message="PTH inside courtyard",
                 components=["R7"]),
    ]
    meta = _FakeMeta()
...
def test_courtyard_overlap_with_two_refs_has_overlap_area():
    errors = [
        DrcError(rule="courtyards_overlap", severity="error",
                 location=(10.0, 20.0), message="overlap",
                 components=["D3", "C4"]),
    ]
    meta = _FakeMeta()
...
def test_pth_with_single_ref_reported_without_overlap_area():
    errors = [
        DrcError(rule="pth_inside_courtyard", severity="error",
                 location=(30.0, 40.0), message="PTH inside",
                 components=["R7"]),
    ]
    meta = _FakeMeta()
    rows = _generate_report_rows(errors, meta, {})
...
def test_render_report_includes_sections():
    rows = [
        {"rule": "courtyards_overlap", "refs_sorted": ["D3", "C4"],
         "location_x": 10.0, "location_y": 20.0,
         "overlap_area_mm2": 5.5, "message": "Courtyards overlap D3 and C4",
         "n_components": 2, "components": ["D3", "C4"]},
        {"rule": "pth_inside_courtyard", "refs_sorted": ["R7"],
         "location_x": 30.0, "location_y": 40.0,
         "overlap_area_mm2": 0.0, "message": "PTH inside R7 courtyard",
         "n_components": 1, "components": ["R7"]},
    ]
...
def test_real_board_violation_count_in_expected_range():
    ...
    assert 25 <= courtyard_count <= 35, (
        f"Expected ~27-29 courtyards_overlap, got {courtyard_count}"
    )
    assert 14 <= pth_count <= 22, (
        f"Expected ~16-18 pth_inside_courtyard, got {pth_count}"
    )
