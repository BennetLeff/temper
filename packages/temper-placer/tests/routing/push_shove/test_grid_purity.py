...
class TestGridPurity:
    ...
    def test_get_cell_is_pure(self, empty_grid):
        """get_cell should be pure (same input -> same output)."""
        cell1 = get_cell(empty_grid, (5, 5))
        cell2 = get_cell(empty_grid, (5, 5))
        
        assert cell1 == cell2, "get_cell should be deterministic"

    def test_get_cell_no_side_effects(self, empty_grid):
        """get_cell should not modify grid."""
        original_state = empty_grid.occupancy.copy()
        
        _ = get_cell(empty_grid, (5, 5))
        
        assert jnp.array_equal(empty_grid.occupancy, original_state), \
            "get_cell should not modify grid"

    def test_is_occupied_is_pure(self, grid_with_obstacles):
        """is_occupied should be pure."""
        result1 = is_occupied(grid_with_obstacles, (5, 5))
        result2 = is_occupied(grid_with_obstacles, (5, 5))
        
        assert result1 == result2, "is_occupied should be deterministic"

    def test_get_neighbors_is_pure(self, empty_grid):
        """get_neighbors should be pure."""
        neighbors1 = get_neighbors(empty_grid, GridCell(5, 5, 0))
        neighbors2 = get_neighbors(empty_grid, GridCell(5, 5, 0))
        
        assert neighbors1 == neighbors2, "get_neighbors should be deterministic"

    def test_grid_with_obstacle_returns_new_grid(self, empty_grid):
        """with_obstacle should return new grid, not modify original."""
        original_occupied = is_occupied(empty_grid, (5, 5))
        
        new_grid = empty_grid.with_obstacle(GridCell(5, 5, 0))
        
        assert is_occupied(empty_grid, (5, 5)) == original_occupied, \
...
class TestGridInvariants:
    ...
    def test_neighbors_are_adjacent(self, empty_grid, x, y):
        """Property: All neighbors should be adjacent (Manhattan distance = 1)."""
        cell = GridCell(x, y, 0)
        neighbors = get_neighbors(empty_grid, cell)
    ...
    def test_interior_cell_has_4_neighbors(self, empty_grid, x, y):
        """Property: Interior cells should have 4 neighbors on same layer."""
        cell = GridCell(x, y, 0)
        neighbors = get_neighbors(empty_grid, cell, allow_layer_change=False)
        
        assert len(neighbors) == 4, f"Interior cell should have 4 neighbors, got {len(neighbors)}"

    def test_corner_cell_has_2_neighbors(self, empty_grid):
    ...
    def test_edge_cell_has_3_neighbors(self, empty_grid):
    ...
    def test_neighbors_exclude_occupied_cells(self, grid_with_obstacles, x, y):
        """Property: get_neighbors should exclude occupied cells."""
        cell = GridCell(x, y, 0)
        neighbors = get_neighbors(grid_with_obstacles, cell)
        
        for neighbor in neighbors:
            assert not is_occupied(grid_with_obstacles, (neighbor.x, neighbor.y)), \
...
class TestGridComposition:
    ...
    def test_multiple_obstacles_compose(self, empty_grid):
        """Adding multiple obstacles should compose correctly."""
        grid1 = empty_grid.with_obstacle(GridCell(5, 5, 0))
    ...
    def test_grid_with_path_is_reversible(self, empty_grid):
        """Adding and removing path should be reversible."""
        cell = GridCell(5, 5, 0)
        
        grid_with = empty_grid.with_path(cell, "NET1")
    ...
    def test_grid_operations_are_chainable(self, empty_grid):
    ...
    def test_grid_operations_are_associative(self, empty_grid, operations):
