...
#   packages/temper-placer/src/temper_placer/pipeline/metrics_observer.py
...
# packages/temper-orchestration/src/metrics.rs (plan 2026-08-09-001,
...
#
# Note: the pre-migration module imports `PipelineExecutionLog` from
# `temper_placer.pipeline.dag_observability`; at test time that name resolves
# to the ported (Rust) pyclass. The oracle only reads
# `execution_log.stage_timings` (a plain dict), and the dag module's own
# bit-exactness is pinned separately by this suite's dag section, so the
# metrics differential stays sound.
#
