...
# as they existed at commit 68ea250f (origin/main, the pre-migration pin for
# the Wave-4 pipeline-feasibility slice) and as embedded in
# tests/pipeline/_pipeline_feasibility_py_oracle.py (lines 33-423, itself a
# content-hash-pinned verbatim copy). This Phase-1 convergence oracle is
# that section extracted, byte-identical -- verified via `diff` against
# `git show 68ea250f:.../convergence.py`.
#
# This is the R1a behavioural oracle for the Rust Stage-engine port in
# packages/temper-orchestration/src/convergence.rs (plan 2026-08-09-001,
# U1). It must keep the ORIGINAL pure-Python semantics forever, including
...
# tests/pipeline/test_convergence_rust_differential.py) recomputes the
