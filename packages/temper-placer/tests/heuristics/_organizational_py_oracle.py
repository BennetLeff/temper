"""
Pinned Python oracle for ``temper_placer/heuristics/organizational.py``
(Wave 4, **Phase 5**).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/heuristics/organizational.py`` as of ``origin/main`` commit
``c5875adad``. Only this docstring differs; every import it makes
(``temper_placer.core.*``, ``temper_placer.heuristics.base``,
``temper_placer.io.config_loader``, ``temper_placer.router_v6.net_classification``) resolves to a module this phase
does NOT migrate, so nothing needed rewriting.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust kernels in
``temper_design_bundle_python.heuristics_organizational`` must reproduce
bit-identically -- including CPython set-iteration order, ``sorted``
stability, and the exact concrete Python type of every returned leaf. Any
edit here silently weakens the differential proof; re-pin from a new base
instead.
