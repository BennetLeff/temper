...
def _make_wall_ms_record(
    ...
    return {
        ...
        "metrics": {
            ...
            "n_runs": 3,
            "wall_ms_min": wall_ms - 1,
            "wall_ms_max": wall_ms + 1,
        },
        "stage_name": stage,
    }
...
class TestDetectTightenableStages:
    ...
    def test_insufficient_runs(self, tmp_path: Path):
        manifest = _load_manifest_dict(_tmp_manifest(tmp_path, [_manifest_entry()]))
        runs = [
            _make_timing_record(wall_ms=40.0, timestamp=f"2026-06-{28 - i:02d}T12:00:00Z")
            for i in range(4)
        ]
    ...
    def test_more_than_n_runs_all_below(self, tmp_path: Path):
        manifest = _load_manifest_dict(_tmp_manifest(tmp_path, [_manifest_entry()]))
        runs = [
            _make_timing_record(wall_ms=30.0 + i, timestamp=f"2026-07-{i:02d}T12:00:00Z")
            for i in range(10)
        ]
        jsonl = _tmp_jsonl(tmp_path, *runs)
        results = detect_tightenable_stages(jsonl, manifest, n_runs=7)
    ...
    def test_streak_broken(self, tmp_path: Path):
        ...
        runs = [
            *_make_records(8, 40.0, day_start=1),  # 8 qualifying, old
            _make_timing_record(
                wall_ms=90.0, timestamp="2026-07-25T12:00:00Z"
            ),  # most recent, above threshold
        ]
        jsonl = _tmp_jsonl(tmp_path, *runs)
        results = detect_tightenable_stages(jsonl, manifest, n_runs=7)
    ...
    def test_threshold_boundary(self, tmp_path: Path):
        ...
        manifest = _load_manifest_dict(
            _tmp_manifest(tmp_path, [_manifest_entry(wall_ms_mean=100.0)])
        )
        runs = [
            _make_timing_record(wall_ms=50.0, timestamp=f"2026-07-{i:02d}T12:00:00Z")
            for i in range(7)
        ]
        jsonl = _tmp_jsonl(tmp_path, *runs)
    ...
    def test_threshold_boundary_above(self, tmp_path: Path):
        ...
        runs = [
            _make_timing_record(wall_ms=50.1, timestamp=f"2026-07-{i:02d}T12:00:00Z")
            for i in range(7)
        ]
        jsonl = _tmp_jsonl(tmp_path, *runs)
        results = detect_tightenable_stages(jsonl, manifest, n_runs=7, threshold=0.50)
    ...
    def test_noise_floor(self, tmp_path: Path):
        ...
        runs = [
            _make_timing_record(wall_ms=1.0, timestamp=f"2026-07-{i:02d}T12:00:00Z")
            for i in range(7)
        ]
        jsonl = _tmp_jsonl(tmp_path, *runs)
    ...
    def test_median_odd_count(self, tmp_path: Path):
        ...
        runs = [
            _make_timing_record(wall_ms=v, timestamp=f"2026-07-{i:02d}T12:00:00Z")
    ...
    def test_multiple_stages_mixed(self, tmp_path: Path):
        ...
        runs_cg = [
            _make_timing_record(
                stage="clearance_grid", wall_ms=40.0, timestamp=f"2026-07-{i:02d}T12:00:00Z"
            )
            for i in range(7)
        ]
        runs_zg = [
            _make_timing_record(
                stage="zone_geometry", wall_ms=10.0, timestamp=f"2026-07-{i:02d}T12:00:00Z"
            )
            for i in range(7)
        ]
        ...
        runs_sg = [
            *_make_records(5, 5.0, day_start=1, stage="slot_generation"),
            _make_timing_record(
                stage="slot_generation", wall_ms=25.0, timestamp="2026-07-06T12:00:00Z"
            ),
            _make_timing_record(
                stage="slot_generation", wall_ms=25.0, timestamp="2026-07-07T12:00:00Z"
            ),
        ]
    ...
    def test_board_filter(self, tmp_path: Path):
        ...
        runs_tp = [
            _make_timing_record(
                board="temper_placed", wall_ms=40.0, timestamp=f"2026-07-{i:02d}T12:00:00Z"
            )
            for i in range(7)
        ]
        runs_ob = [
            _make_timing_record(
                board="other_board", wall_ms=40.0, timestamp=f"2026-07-{i:02d}T12:00:00Z"
            )
            for i in range(7)
        ]
    ...
    def test_stage_filter(self, tmp_path: Path):
        ...
        results = detect_tightenable_stages(
            jsonl, manifest, n_runs=7, stage_filter="clearance_grid"
        )
        assert len(results) == 1
        assert results[0].stage == "clearance_grid"
    ...
    def test_wall_ms_mean_records_used(self, tmp_path: Path):
        manifest = _load_manifest_dict(_tmp_manifest(tmp_path, [_manifest_entry()]))
        runs = [
            _make_wall_ms_record(wall_ms=40.0, timestamp=f"2026-07-{i:02d}T12:00:00Z")
            for i in range(7)
        ]
        jsonl = _tmp_jsonl(tmp_path, *runs)
        results = detect_tightenable_stages(jsonl, manifest, n_runs=7)
    ...
    def test_results_sorted_by_drop_pct_descending(self, tmp_path: Path):
        ...
        runs_sd = [
            _make_timing_record(
                stage="small_drop", wall_ms=25.0, timestamp=f"2026-07-{i:02d}T12:00:00Z"
            )
            for i in range(7)
        ]
        runs_bd = [
            _make_timing_record(
                stage="big_drop", wall_ms=100.0, timestamp=f"2026-07-{i:02d}T12:00:00Z"
            )
            for i in range(7)
        ]
...
class TestTightenCLI:
    ...
    def test_non_dry_prompts_confirmation(self, runner, tmp_repo):
        ...
        result = runner.invoke(timing, ["tighten", "--n-runs", "7"], input="n\n")
        assert result.exit_code == 0
    ...
    def test_apply_changes(self, runner, tmp_repo):
        ...
        result = runner.invoke(timing, ["tighten", "--n-runs", "7"], input="y\n")
        assert result.exit_code == 0
