"""
Pinned Python oracle for ``temper_placer/core/net_types.py`` (Wave 4, Phase 2).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/core/net_types.py`` as of commit ``37a4251e0``
(origin/main, the Wave-4 contracts-first base). The only change is the
relative import ``from .board import LayerIndex`` rewritten to the
absolute ``from temper_placer.core.board import LayerIndex`` so the file
is importable standalone from the test suite.
