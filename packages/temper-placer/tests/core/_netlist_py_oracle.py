"""
Pinned Python oracle for ``temper_placer/core/netlist.py`` (Wave 4, Phase 3).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/core/netlist.py`` as of commit ``e799183c4`` (the file's
last-touching commit on ``origin/main`` at base ``ebf9326ff``, the Wave-4
Phase-3 candidate-1 base). Only this docstring differs; the module imports
only the stdlib and ``numpy``, so no relative imports needed rewriting.
