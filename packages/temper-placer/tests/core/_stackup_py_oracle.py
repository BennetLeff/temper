"""
Pinned Python oracle for ``temper_placer/core/stackup.py`` (Wave 4, core contracts).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/core/stackup.py`` as of commit on origin/main
(``db3994773``, the Wave-4 core-contracts migration base). The module
imports only the Python stdlib (``math``, ``dataclasses``), so no relative
imports needed rewriting.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust pyo3 pyclasses and
pyfunctions (``temper_design_bundle_python``) must reproduce bit-identically;
any edit here silently weakens the differential proof. If the module's
