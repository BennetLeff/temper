"""
Pinned Python oracle for ``temper_placer/core/loop.py`` (Wave 4, Phase 2).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/core/loop.py`` as of commit ``76f38db0a``
(origin/main, the Wave-4 contracts-first base for the SECOND Phase-2
migration). The module imports only the Python stdlib (``math``,
``dataclasses``, ``enum``), so no relative imports needed rewriting.
