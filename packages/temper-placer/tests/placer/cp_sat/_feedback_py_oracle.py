"""Pinned Python oracle for the ``temper_placer/placer/cp_sat/feedback.py``
feedback classifier (orchestration-port unit U-I, Wave-4).

This file is a VERBATIM copy of the pre-migration
``temper_placer/placer/cp_sat/feedback.py`` ``FeedbackClassifier`` as of the
U-I base commit. Only the module docstring was replaced with this pin note.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust feedback kernels
(``temper_orchestration``) must reproduce bit-identically; any edit here
silently weakens the differential proof. If the module's feedback decisions
change, the oracle must be re-pinned from the new base first.
"""
