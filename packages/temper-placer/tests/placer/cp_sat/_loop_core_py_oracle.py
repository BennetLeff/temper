"""Pinned Python oracle for the ``temper_placer/placer/cp_sat/_loop_core.py``
CP-SAT place->route loop core (orchestration-port unit U-I, Wave-4).

This file is a VERBATIM copy of the pre-migration
``temper_placer/placer/cp_sat/_loop_core.py`` ``_LoopCoreMixin`` as of the
U-I base commit. Only the module docstring was replaced with this pin note.
The shared leaf types/helpers it imports (``_loop_types`` / ``_loop_utils``)
are NOT copied -- they are unchanged by this unit and resolve to the live
modules; this oracle pins only the loop-core mixin (``run``,
``_run_with_gates``, ``_solve_with_delta``, ``_solve_phase2``,
``_call_solver``).

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust ``cpsat_loop``
pyfunctions (``temper_orchestration``) must reproduce bit-identically; any
edit here silently weakens the differential proof. If the module's
orchestration changes, the oracle must be re-pinned from the new base first.
"""
