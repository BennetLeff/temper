...
def test_dsn_primitives_match_the_pinned_python():
    """`io/dsn.py`'s retired pure-Python types vs the Rust ones they delegate to."""
    import tests.io._dsn_py_oracle as _dsn_oracle
    from temper_placer.io.dsn import DSNCircle, DSNPath, DSNRect, dsn_list

    cases = [
        ("pcb", ("sample", None)),
        ("coord", (10.0, 10.5, 10.54321, 0.0, -0.0)),
        ("name", ("GND", "VCC (Power)", 'Quoted "String"', "")),
        ("mixed", (1, 2.0, "three", 10**30)),
        ("tiny", (1e-7, 1e-6, 1e-9, 123456789.123456789)),
        ("inf", (float("inf"), float("-inf"))),
    ]
    for name, args in cases:
        # `None` is the catch-all `str(v)` branch on both sides.
        args = tuple(a for a in args if a is not None) if name == "pcb" else args
        rust = dsn_list(name, *args)
        py = _dsn_oracle.dsn_list(name, *args)
        assert str(rust) == str(py), f"dsn_list({name}) bytes differ"

    nested_rust = dsn_list("pcb", "sample", dsn_list("unit", "mm"))
    nested_py = _dsn_oracle.dsn_list("pcb", "sample", _dsn_oracle.dsn_list("unit", "mm"))
    assert str(nested_rust) == str(nested_py)
    assert str(nested_rust.with_comment("c: 1")) == str(nested_py.with_comment("c: 1"))

    for r_, p_ in [
        (DSNRect("pcb", 0, 0, 100, 100), _dsn_oracle.DSNRect("pcb", 0, 0, 100, 100)),
        (DSNRect("l", -0.5, 1e-9, 1 / 3, 2 / 3), _dsn_oracle.DSNRect("l", -0.5, 1e-9, 1 / 3, 2 / 3)),
        (DSNCircle("F.Cu", 1.5, 10, 20), _dsn_oracle.DSNCircle("F.Cu", 1.5, 10, 20)),
        (DSNCircle("F.Cu", 0.6), _dsn_oracle.DSNCircle("F.Cu", 0.6)),
        (
            DSNPath("F.Cu", 0.2, [(0, 0), (10, 0), (10, 10)]),
            _dsn_oracle.DSNPath("F.Cu", 0.2, [(0, 0), (10, 0), (10, 10)]),
        ),
        (DSNPath("F.Cu", 0.2, []), _dsn_oracle.DSNPath("F.Cu", 0.2, [])),
    ]:
        assert str(r_.to_dsn()) == str(p_.to_dsn()), f"{r_!r} to_dsn bytes differ"


def test_bool_argument_formats_as_python_str_not_as_int():
    """`bool` is an `int` subclass; the DSN formatter must still emit `True`.

    The pyo3 `is_instance_of::<PyInt>()` check accepts `bool`, so without an
    explicit earlier `PyBool` arm this renders `1`. The pinned Python falls
    through to `str(v)`.
    """
    import tests.io._dsn_py_oracle as _dsn_oracle
    from temper_placer.io.dsn import dsn_list

    rust = dsn_list("flags", True, False)
    py = _dsn_oracle.dsn_list("flags", True, False)
    assert str(rust) == str(py) == "(flags True False)"
