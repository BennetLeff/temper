...
def test_compute_schema_hash_deterministic():
    ...
    h1 = compute_dsn_schema_hash(board, netlist)
    h2 = compute_dsn_schema_hash(board, netlist)
    assert h1 == h2
...
def test_compute_schema_hash_changes_with_net():
    ...
    h1 = compute_dsn_schema_hash(board, Netlist(components=comps, nets=nets_a))
    h2 = compute_dsn_schema_hash(board, Netlist(components=comps, nets=nets_b))
    assert h1 != h2
...
def test_compute_schema_hash_changes_with_footprint():
    ...
    h2 = compute_dsn_schema_hash(board, Netlist(components=comps_b, nets=nets))
    assert h1 != h2
...
def test_compute_schema_hash_stable_with_position():
    ...
    h = compute_dsn_schema_hash(board, netlist)
    assert h == compute_dsn_schema_hash(board, netlist)
...
def test_embed_header():
    ...
    result = embed_schema_header(dsn, "abc123")
...
def test_embed_header_replaces_existing():
    ...
    result = embed_schema_header(dsn, "newhash")
...
def test_extract_hash():
    ...
    assert extract_schema_hash(dsn) == "abc123def456"


def test_extract_hash_missing():
    dsn = "(pcb test)\n"
    assert extract_schema_hash(dsn) is None
...
def test_schema_hash_in_export_pcb():
    ...
    assert extract_schema_hash(dsn_text) is not None
