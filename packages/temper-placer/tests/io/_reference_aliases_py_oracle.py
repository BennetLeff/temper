"""
Pinned Python oracle for ``temper_placer/io/reference_aliases.py`` (Wave 4, Phase 3, candidate 5).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/io/reference_aliases.py`` as of commit ``79ab9bd0e`` (the
``feat/wave4-phase3-board-netlist-contracts`` base for the config/reference
loaders migration). The original module docstring is preserved verbatim
below; no semantics were altered.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust implementation
(``temper_design_bundle_python`` / ``temper_io_types``) must reproduce
...
def _validate_aliases(
    ...
    *,
    ...
    for source, target in aliases.items():
        if source == target:
            raise ValueError(
                f"{path}: {namespace} alias {source!r} maps a name to itself; "
                "a self-alias is either a typo or a live name that must not be rewritten"
            )
