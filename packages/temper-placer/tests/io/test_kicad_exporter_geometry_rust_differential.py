...
def test_snap_to_nearest_pad_outside_tolerance_unchanged():
    """Dedicated named test (not just a parametrize row) for the tolerance
    gate: a pad well outside `tolerance` must NOT be snapped to."""
    py_result = _oracle.snap_to_nearest_pad(0.0, 0.0, [(5.0, 5.0)], 0.15)
    rust_result = SNAP_TO_NEAREST_PAD(0.0, 0.0, [(5.0, 5.0)], 0.15)
    assert py_result == (0.0, 0.0)
    assert _pos(rust_result) == _pos(py_result)


def test_snap_to_nearest_pad_exact_tolerance_boundary_excluded():
    """Dedicated named test: a pad at EXACTLY `tolerance` distance is
    excluded (`best_dist` starts at `tolerance`, strict `<`)."""
    py_result = _oracle.snap_to_nearest_pad(0.0, 0.0, [(0.15, 0.0)], 0.15)
    rust_result = SNAP_TO_NEAREST_PAD(0.0, 0.0, [(0.15, 0.0)], 0.15)
    assert py_result == (0.0, 0.0)
    assert _pos(rust_result) == _pos(py_result)
...
def test_connector_boundary_exact_match_tolerance_0_01_is_exclusive():
    """`abs(ex-px) < 0.01` — at EXACTLY 0.01 (bit-exact: `0.01 - 0.0`, not a
    subtraction that merely rounds close to 0.01) the pad is NOT considered
    already-connected, so it becomes a bridge candidate instead. A prior
    version of this test used `1.01 - 1.0`, which floating point rounds to
    ``0.010000000000000009`` (not bit-exact 0.01) and so silently failed to
    exercise this boundary at all -- caught by mutation testing on the
    kernel's own `<=` mutant, which this construction correctly kills."""
    segments = [TraceSegment(net="GND", start=(0.01, 0.0), end=(5.0, 0.0), width=0.25, layer="F.Cu")]
    pad_centers = {"GND": [(0.0, 0.0)]}
    connectors = _run_both(segments, pad_centers)
    # Not treated as connected (0.01 is not < 0.01), so a short bridge is
    # generated -- exercising the exact boundary on both sides.
    assert len(connectors) == 1
...
def test_connector_exact_max_dist_boundary_is_exclusive():
    """Dedicated named test: a pad at EXACTLY `max_dist` distance is NOT
    bridged (`min_dist < max_dist` is strict)."""
    segments = [TraceSegment(net="GND", start=(0.0, 0.0), end=(1.0, 0.0), width=0.25, layer="F.Cu")]
    pad_centers = {"GND": [(3.0, 0.0)]}  # distance from (1.0, 0.0) is exactly 2.0
    connectors = _run_both(segments, pad_centers, max_dist=2.0)
    assert connectors == []
...
def test_connector_sequential_pads_same_net_second_finds_no_ref_seg():
    """Second pad on the same net is within `max_dist` of the endpoint the
    FIRST pad's connector just added (exercising the oracle's
    `endpoints.add((px, py))` sequential state update) -- but produces NO
    second connector, because `ref_seg` is looked up only against the
    ORIGINAL `net_segs`, never against a connector emitted earlier in the
    same call. The new endpoint is recorded (so a THIRD pad closer than
    0.01 to it would read as already-connected) but nothing bridges to it.
    This is a real, non-obvious behavior of the pre-migration Python -- not
    a Rust/Python divergence: `_run_both` already asserts the two sides
    produce IDENTICAL output before this function's own assertion runs."""
    segments = [TraceSegment(net="GND", start=(0.0, 0.0), end=(1.0, 0.0), width=0.25, layer="F.Cu")]
    pad_centers = {"GND": [(2.0, 0.0), (3.5, 0.0)]}
    connectors = _run_both(segments, pad_centers, max_dist=2.0)
    assert len(connectors) == 1
    assert connectors[0].end == (2.0, 0.0)
