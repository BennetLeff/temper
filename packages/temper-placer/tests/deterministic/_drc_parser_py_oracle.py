# VERBATIM pre-migration oracle for ``deterministic/feedback/drc_parser.py``.
# Wave 4, Phase 5 (deterministic hubs slice). Pinned from
# ``packages/temper-placer/src/temper_placer/deterministic/feedback/drc_parser.py``
...
# The only import adjusted for relocation is ``DRCViolation``, which now
# resolves to the (unchanged, still-Python) dataclass in the delegation shim —
# the compute pinned here is ``parse_kicad_drc`` / ``_process_raw_violation``.
...
from temper_placer.deterministic.feedback.violation_mapper import DRCViolation
