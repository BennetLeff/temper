"""VERBATIM pre-migration oracle for ``deterministic/stages/routing_metrics.py``.

Wave 4, Phase 5, batch 2 (deterministic leaf stages). Pinned from
``packages/temper-placer/src/temper_placer/deterministic/stages/routing_metrics.py``
...
The three dataclasses and their aggregation compute (`add_net`, `finalize`,
`to_dict`, `to_json`, the two properties) are pinned verbatim; the pyclasses
replicate them. ``print_summary`` is I/O and stays on the delegation shim.
"""
