...
def test_connectivity_stage_unconnected_pads():
    ...
    oracle.register_pad(
        Pad(center=Point(0, 0), shape="circle", size=(1, 1), net="A", layer=0, id="P1")
    )
    oracle.register_pad(
        Pad(center=Point(10, 0), shape="circle", size=(1, 1), net="A", layer=0, id="P2")
    )
...
def test_connectivity_stage_mixed_violations():
    ...
    oracle.register_pad(
        Pad(center=Point(0, 10), shape="circle", size=(1, 1), net="A", layer=0, id="P3")
    )

    # Group 3: Isolated track (orphan)
    oracle.register_track(
        Track(start=Point(10, 10), end=Point(15, 10), width=0.25, net="A", layer=0)
    )
