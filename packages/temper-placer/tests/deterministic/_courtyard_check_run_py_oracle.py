"""
D6 run-orchestration oracle for `deterministic/stages/courtyard_check.py`.
...
Rust port (`temper-orchestration`'s `CourtyardCheckStage`) is the differential
subject. The shapely collision detection and the CPython `random.random()`
noise stay single-source and are driven through FFI by the port.
"""
