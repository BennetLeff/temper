...
#   packages/temper-placer/src/temper_placer/deterministic/feedback/orchestrator.py
# at the U-F dispatch base (origin/main, 565078e54). Relative imports are
# adapted to absolute paths so the oracle imports from the test tree; every
# other line is the verbatim pre-migration source.
#
# This is the R1a behavioural oracle for the U-F Rust feedback-loop port in
# packages/temper-orchestration (Rust Orchestration Engine plan
# 2026-08-09-001, orchestration-port unit U-F): the `AutomatedZeroDRC.run()`
# iterate-until-clean LOOP (solve -> run DRC -> map violations -> adjust
# zones -> re-solve until clean or the iteration cap), now implemented in
# `temper-orchestration`'s `run_automated_zero_drc` pyfunction, which drives
# the per-iteration call-backs (pipeline.run, the DRC runner, the report
# parser, the violation mapper, the zone adjuster and the config update) in
# the oracle's order and terminates on the oracle's conditions. The leaf
# helpers (`drc_parser`/`violation_mapper`/`zone_adjuster`) are the shared
# delegation shims (their compute is pinned individually by the Wave-4
# Phase-5 differentials); what this oracle pins is the LOOP.
#
# It must keep the ORIGINAL pure-Python loop semantics forever, including
...
#
# test_orchestrator_rust_differential.py recomputes the sha256 of
