...
def test_orchestrator_loop_terminates_on_zero_violations(mock_netlist, initial_config):
    ...
    with patch(
        "temper_placer.deterministic.feedback.orchestrator.parse_kicad_drc", return_value=[]
    ):
...
def test_orchestrator_adjusts_and_retries_on_violations(mock_netlist, initial_config):
    ...
    violations_sequence = [[DRCViolation(type="clearance", items=["of Q1"], pos=(10, 10))], []]
...
def test_orchestrator_stops_after_max_iterations(mock_netlist, initial_config):
    ...
    with patch(
        "temper_placer.deterministic.feedback.orchestrator.parse_kicad_drc",
        return_value=[violation],
    ):
        orchestrator = AutomatedZeroDRC(
            pipeline, mock_netlist, initial_config, drc_runner, max_iterations=3
        )
        orchestrator.adjuster.violation_threshold = 1
        orchestrator.run()
