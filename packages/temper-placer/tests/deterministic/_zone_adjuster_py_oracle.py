# VERBATIM pre-migration oracle for ``deterministic/feedback/zone_adjuster.py``.
# Wave 4, Phase 5 (deterministic hubs slice). Pinned from
# ``packages/temper-placer/src/temper_placer/deterministic/feedback/zone_adjuster.py``
...
# The only import adjusted for relocation is ``MappedViolation``, which now
# resolves to the (unchanged, still-Python) dataclass in the delegation shim —
# the compute pinned here is ``ZoneAdjuster.compute_adjustments``.
