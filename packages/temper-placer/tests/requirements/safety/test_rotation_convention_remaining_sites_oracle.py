...
sweep: 12 additional R(+theta)/R(-theta) call sites in 9 candidate areas,
found beyond PR #479's 12 and ``check_pad_orientation.py``'s pre-existing 13th (see
``scripts/check_no_raw_rotation_trig.py``'s module docstring, "Second
sweep" section, for the full classification of all 12).

Eight call sites were updated: seven live KiCad-derived call sites across six
files, plus the dead ``scripts/internal_route.py`` registration formula. All
were fixed to route through ``temper_placer.geometry.kicad_transform`` -- the
same single sanctioned implementation the original 12 use:
  ...
  6. ``visualization/model.py::Rectangle.corners``
  7. ``visualization/board_renderer.py::get_pad_shapes``'s pad-corner path
  8. ``scripts/internal_route.py``'s pad-position registration formula

The other four call sites in three candidate areas
(``packages/temper-geometry/src/polygon.rs::rotate_polygon``,
``scripts/bench_rust_geometry.py``, and the two dead functions in
``core/state.py``) were investigated and left unchanged: the first two have
no KiCad correspondence, and the latter are unreachable dead code. See the
lint's docstring for the full classification.
