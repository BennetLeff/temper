"""Pinned Python oracle for ``temper_placer/regression/reporter.py``.

VERBATIM copy of the pre-migration implementation of
``temper_placer/regression/reporter.py`` (as of ``origin/main`` commit
``2426f5cf5``, the Wave-4 tail-tooling migration base). Do NOT edit the
semantics: the differential suite
(``tests/regression/test_reporter_rust_differential.py``) compares the Rust
pyclasses (``temper-orchestration`` ``reporter`` module) against this file
byte-for-byte, and its content hash is registered in
``scripts/oracle_hashes.json``. All four classes — ``BatteryVerdictReport``,
``MetricDelta``, ``BoardResult``, ``RegressionReporter`` — stay here
VERBATIM as evidence; the production ``reporter.py`` is now a delegation
shim re-exporting the Rust pyclasses (public API unchanged).
