"""Pinned Python oracle for
``temper_placer/router_v6/neighbor_validity.py``.

This file is a VERBATIM copy of the pre-migration implementation as of
commit ``ce278aa09`` (origin/main at task start). Only this module
docstring is replaced; every executable line below is byte-identical to
that revision. The module imports only ``numpy`` / stdlib plus a
``TYPE_CHECKING``-only ``OccupancyGrid`` reference, so no imports needed
rewriting.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust kernel in
``packages/temper-geometry/src/neighbor_validity.rs`` must reproduce
BIT-IDENTICALLY; any edit here silently weakens the differential proof.
If the module's contract changes, the oracle must be re-pinned from the
new base first.
