...
class TestZoneLayersForNet:
    ...
    def test_cgnd_is_not_zone_eligible(self):
        """FIXED 2026-07-29 (restoring commit 27368038, lost by a
        sibling-branch code move -- see _zone_layers_for_net()'s
        docstring): eligibility is driven by
        routing_strategy=="plane_required" (core/design_rules.py), which
        GND does not declare (only ACMains/HighVoltage do). GND nets no
        longer get automatic zone treatment from this function."""
...
class TestCrossClassZoneClearance:
    ...
    def test_power_and_hv_resolve_to_stricter_cross_class(self):
        ...
        FIXED 2026-07-29 (restoring commit 27368038, lost by a
        sibling-branch code move -- see _zone_layers_for_net()'s
        docstring): previously used vcc (Power), which after the
        routing_strategy-driven eligibility fix no longer gets a zone at
        all -- Power is not "plane_required". Only ACMains/HighVoltage
        remain zone-eligible, and both declare an identical 6.0mm own
        clearance, so a class_pairs override is needed to actually
        exercise the cross-class lookup path (rather than coincidentally
        matching each class's own clearance)."""
    ...
    def test_same_class_nets_keep_own_clearance(self):
        ...
        FIXED 2026-07-29: previously used two GND-class nets (PWR_RTN/
    ...
    def test_fallback_to_max_clearance_no_class_pair(self):
        ...
        FIXED 2026-07-29: previously used vcc/+15V (both Power), which no
        longer get zones. Uses ACMains/HighVoltage instead -- both declare
        6.0mm, so max(6.0, 6.0) == 6.0 exercises the fallback branch even
        though the two classes carry equal clearance."""
        from temper_placer.router_v6.adapter import _write_routes_to_content
    ...
    def test_single_netclass_no_cross_class(self):
        ...
        FIXED 2026-07-29: previously used vcc (Power, 0.25mm), no longer
...
class TestPriorityInversion:
    ...
    def test_acmains_priority_higher_than_highvoltage_in_emitted_zones(self):
        ...
        FIXED 2026-07-29 (restoring commit 27368038, lost by a
        sibling-branch code move -- see _zone_layers_for_net()'s
        docstring): previously compared ACMains against Power ("vcc");
        Power no longer gets a zone at all post-fix (only
...
class TestStitchIsolatedPads:
    ...
    FIXED 2026-07-29 (restoring commit 27368038, lost by a sibling-branch
    code move -- see _zone_layers_for_net()'s docstring): previously used
    "vcc" (Power), which after the routing_strategy-driven eligibility fix
    is no longer zone-eligible -- _stitch_isolated_pads() now delegates
    its own eligibility check to _zone_layers_for_net(), so a "vcc"
    fixture would exercise the ineligibility short-circuit instead of the
    geometry logic these tests exist to check.
    """
