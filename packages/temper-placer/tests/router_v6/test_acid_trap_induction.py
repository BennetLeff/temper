...
def test_acid_trap_add_compliant_route() -> None:
    ...
    compliant = CompiledRoute(
        ...
        path=RoutePath(
            net_name="WIDE",
            coordinates=[(0.0, 0.0), (10.0, 0.0), (20.0, -10.0)],
            layer_name="F.Cu",
            path_length=24.14,
        ),
        width_mm=0.3,
    ...
    assert report.trap_count == 0, f"Adding compliant route caused {report.trap_count} acid traps"
...
def test_acid_trap_modify_preserves_compliance() -> None:
    ...
    rr.compiled_routes["WIDE"] = CompiledRoute(
        ...
        path=RoutePath(
            net_name="WIDE",
            coordinates=[(5.0, 0.0), (15.0, 0.0), (25.0, -10.0)],
            layer_name="F.Cu",
            path_length=24.14,
        ),
        width_mm=0.3,
    ...
    report = detect_acid_traps(rr)
    assert report.trap_count == 0, f"Route modification caused {report.trap_count} acid traps"
...
def test_acid_trap_remove_preserves_compliance() -> None:
    ...
    assert after <= before, f"Route removal increased acid traps from {before} to {after}"
