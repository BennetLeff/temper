# mypy: ignore-errors
"""PINNED ORACLE -- verbatim ``_compute_net_order`` as it stood on
``origin/main`` @ 19ddbbbc8 (2026-08-18), before the experiment-mode
selector was added to
``temper_placer/router_v6/_astar_ordering.py``.

Why this file exists
--------------------
``_compute_net_order`` is the function that decides the order in which
nets are handed to Stage 4's A*. It is **pure Python with no Rust
counterpart** -- ``router_v6/net_ordering.py``'s Rust-backed
``order_nets_py`` is a DIFFERENT function that does not govern this path
(its result is assigned to an unused local at ``route_stage.py:99``).
Under the owner's standing "always migrate Python to Rust" directive this
function is therefore a porting candidate, and this oracle is step 1 of
that port: the pinned reference the Rust must reproduce exactly.

It also serves an immediate purpose. The shipped module now carries a
``TEMPER_NET_ORDER_MODE`` experiment selector whose default
(``"baseline"``) must be a bit-exact no-op. ``test_astar_ordering_oracle_
differential.py`` asserts that against THIS file, over the production
board's real channel-path corpus -- not a synthetic fixture, because a
differential only proves what you feed it.

Do not "fix" this file to match a future implementation. If the shipped
code and this oracle disagree, the shipped code changed behaviour.
"""
