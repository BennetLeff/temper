"""Pinned Python oracle for ``router_v6/astar_core.py``'s Tier-3
N-layer, via-aware A* search (``_astar_search_3d`` / ``_route_segment_3d``).

DO NOT EDIT -- THIS IS THE REFERENCE.
======================================
Every line below the ``BEGIN VERBATIM EXTRACTION`` marker is a **verbatim**
``git show`` extraction from commit ``9019da63fe1f8cfccb98c53fafbbf0a8537ee7a6``
...
``test_astar_nlayer_rust_differential.py``) re-runs the identical extraction
...
----------------------
``astar_core._astar_search_3d`` is the ONLY genuinely N-layer, via-aware
search in the router. The 2D search beneath Tiers 1 and 2 of
``_astar_nlayer._astar_route_nlayer`` has been Rust-backed since the U5 port
(``temper_rust_router_core::astar::astar_kernel_3d`` -- a 2D kernel whose
``_3d`` suffix is an inherited misnomer from the retired JIT kernel), but
Tier 3 remained pure Python. This file pins Tier 3's pre-migration behaviour
so ``temper_rust_router_core::astar_nlayer`` can be proven to reproduce it
bit-exactly on real ``pcb/temper.kicad_pcb`` geometry before the Python is
deleted.

What is NOT pinned here
-----------------------
The imports below the docstring are NOT part of the verbatim extraction --
they are this file's own standalone-runnability preamble. Two names are
imported from live modules rather than copied, because they are stable
infrastructure the migration does not touch:

  - ``STANDARD_LAYER_ORDER`` (``core.board``) -- the canonical 4-layer SSOT
    tuple ``_astar_search_3d`` consults for its layer-preference order.
  - ``get_monitor_state`` (``router_v6.astar_monitor``) -- the runtime
    monitor hook, inert (returns ``None``) unless a monitor is installed.

``OccupancyGrid`` is likewise NOT copied: the oracle operates on whatever
grid objects it is handed, using only their public
``is_free`` / ``grid_to_world`` / ``world_to_grid`` / ``mark_via_blocked`` /
``width_cells`` / ``height_cells`` / ``cell_size`` surface.
"""
