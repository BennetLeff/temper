"""Pinned Python oracle for ``router_v6/constraints_geometry.py`` (Wave 4).

VERBATIM copy of the pre-migration implementation of
``temper_placer/router_v6/constraints_geometry.py`` as of commit ``c5875adad``
(origin/main).  Do NOT edit the semantics: this is the oracle the Rust
kernel in ``temper-geometry`` (``drc_constraints_geometry.rs``) must
reproduce bit-identically.  Any edit here silently weakens the differential
proof; if the module's contract changes, re-pin the oracle from the new base
first.

Only the module docstring and the ``__all__`` name of the re-exported
``Point`` differ from the original file; every executable statement below is
byte-identical to the pinned commit (asserted by
``test_oracle_is_verbatim_copy`` in the differential suite).
