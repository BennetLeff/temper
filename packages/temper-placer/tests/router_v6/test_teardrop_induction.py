...
def test_teardrop_modify_preserves_compliance() -> None:
    ...
    assert after == before, f"Route modification changed teardrop count from {before} to {after}"
...
def test_teardrop_remove_preserves_compliance() -> None:
    ...
    after = insert_teardrops(rr).teardrop_count
    assert after <= before, f"Route removal increased teardrop count from {before} to {after}"
