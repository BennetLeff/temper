...
class TestProjectOutsideKeepout:
    ...
    def test_outside_keepout_identity(self):
        ...
        result = project_outside_keepout(np.array([1.0, 1.0]), self.KEEPOUT)
    ...
    def test_inside_keepout_projected(self):
        ...
        result = project_outside_keepout(np.array([7.5, 7.5]), self.KEEPOUT)
        # Nearest edge from center (7.5, 7.5) is left at x=5
        assert np.allclose(result, np.array([5.0, 7.5]))

    def test_on_keepout_edge_identity(self):
        """Point on keepout boundary returns unchanged."""
        result = project_outside_keepout(np.array([5.0, 7.5]), self.KEEPOUT)
        assert np.allclose(result, np.array([5.0, 7.5]))
...
class TestProjectOntoBoard:
    ...
    def test_inside_identity(self):
        ...
        result = project_onto_board(np.array([50.0, 50.0]), self.MARGIN, self.BOARD_W, self.BOARD_H)
        assert np.allclose(result, np.array([50.0, 50.0]))

    def test_below_left_corner_clamped(self):
        """Point below minimum x and y clamped."""
        result = project_onto_board(np.array([-1.0, -1.0]), self.MARGIN, self.BOARD_W, self.BOARD_H)
        assert np.allclose(result, np.array([3.0, 3.0]))
...
class TestProjectOntoHalfPlane:
    def test_hv_above_boundary_identity(self):
        ...
        assert np.allclose(result, np.array([50.0, 30.0]))

    def test_hv_below_boundary_projected(self):
        """Point below boundary, HV sign (+1), projected to boundary."""
        result = project_onto_half_plane(np.array([50.0, 10.0]), 20.0, 1.0)
    ...
    def test_lv_below_boundary_identity(self):
        ...
        result = project_onto_half_plane(np.array([50.0, 10.0]), 20.0, -1.0)
        assert np.allclose(result, np.array([50.0, 10.0]))
...
class TestProjectOntoEdgeStrip:
    ...
    def test_bottom_edge_inside_strip(self):
        ...
        result = project_onto_edge_strip(
            np.array([50.0, 10.0]), self.BOARD_W, self.BOARD_H, self.MAX_DIST, "bottom"
        )
        assert np.allclose(result, np.array([50.0, 10.0]))
    ...
    def test_center_clamped_to_bottom_strip(self):
        ...
        result = project_onto_edge_strip(
            np.array([50.0, 50.0]), self.BOARD_W, self.BOARD_H, self.MAX_DIST, "bottom"
        )
        assert np.allclose(result, np.array([50.0, 20.0]))
    ...
    def test_top_edge_center_clamped(self):
        ...
        result = project_onto_edge_strip(
            np.array([50.0, 50.0]), self.BOARD_W, self.BOARD_H, self.MAX_DIST, "top"
        )
        assert np.allclose(result, np.array([50.0, 80.0]))

    def test_left_edge_clamped(self):
        """Point far right clamped to left."""
        result = project_onto_edge_strip(
            np.array([90.0, 50.0]), self.BOARD_W, self.BOARD_H, self.MAX_DIST, "left"
        )
        assert np.allclose(result, np.array([20.0, 50.0]))

    def test_right_edge_clamped(self):
        """Point far left clamped to right."""
        result = project_onto_edge_strip(
            np.array([10.0, 50.0]), self.BOARD_W, self.BOARD_H, self.MAX_DIST, "right"
        )
        assert np.allclose(result, np.array([80.0, 50.0]))
    ...
    def test_invalid_edge_raises(self):
        ...
        with pytest.raises(ValueError, match="Invalid edge"):
            project_onto_edge_strip(
                np.array([50.0, 50.0]), self.BOARD_W, self.BOARD_H, self.MAX_DIST, "north"
            )
...
class TestProjectOntoSide:
    ...
    def test_top_side_above_midline_clamped(self):
        ...
        result = project_onto_side(np.array([50.0, 70.0]), self.BOARD_H, self.MIDLINE, "top")
        assert np.allclose(result, np.array([50.0, 50.0]))

    def test_top_side_below_midline_identity(self):
        """Below midline, top, unchanged."""
        result = project_onto_side(np.array([50.0, 30.0]), self.BOARD_H, self.MIDLINE, "top")
        assert np.allclose(result, np.array([50.0, 30.0]))

    def test_bottom_side_below_midline_clamped(self):
        """Below midline, bottom, clamped."""
        result = project_onto_side(np.array([50.0, 30.0]), self.BOARD_H, self.MIDLINE, "bottom")
        assert np.allclose(result, np.array([50.0, 50.0]))

    def test_bottom_side_above_midline_identity(self):
        """Above midline, bottom, unchanged."""
        result = project_onto_side(np.array([50.0, 70.0]), self.BOARD_H, self.MIDLINE, "bottom")
        assert np.allclose(result, np.array([50.0, 70.0]))

    def test_invalid_side_raises(self):
        """Invalid side identifier raises ValueError."""
        with pytest.raises(ValueError, match="Invalid side"):
            project_onto_side(np.array([50.0, 50.0]), self.BOARD_H, self.MIDLINE, "both")
...
class TestIdentityProjection:
    def test_returns_same_point(self):
        ...
        point = np.array([42.0, 73.0])
        assert np.all(identity_projection(point) == point)
