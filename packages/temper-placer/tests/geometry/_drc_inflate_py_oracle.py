"""
Pinned Python oracle for ``temper_placer/geometry/drc_inflate.py``
(Wave 4, Phase 4 — the geometry remainder).

This file is a VERBATIM copy of the pre-migration implementation of
``temper_placer/geometry/drc_inflate.py`` as of commit ``ebf9326ff``
(origin/main, the Phase-4 base). Only this module docstring is replaced;
every executable line below is byte-identical to that revision. The module
imports only ``numpy`` / ``shapely`` / stdlib, so no imports needed
rewriting.

DO NOT EDIT THE SEMANTICS. This is the oracle the Rust kernels in
``packages/temper-geometry/src/drc_inflate.rs`` must reproduce
...
Original module docstring, preserved for the record::

    Precompute Minkowski-inflated pad dimensions for DRC proxy loss.

    The Minkowski inflation (pad_polygon.buffer(trace_width/2)) is done once
    per component using Shapely (non-JAX). The inflated polygons are
    simplified to axis-aligned bounding box dimensions (widths, heights)
    stored as JAX arrays.

    At evaluation time, only pairwise AABB distance checks run in JAX —
    lightweight, differentiable, and amortizes the expensive Shapely
    inflation.
