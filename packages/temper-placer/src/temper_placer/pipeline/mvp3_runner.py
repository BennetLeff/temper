...
class MVP3Runner:
    ...
    def run(self) -> MVP3Result:
        ...
        try:
            ...
            if raw_board.footprints:
                for fp in raw_board.footprints:
                    ...
                    if ref == "J_AC_IN":
                        ...
                        print(
                            f"DEBUG J_AC_IN graphicItems count: {len(fp.graphicItems) if hasattr(fp, 'graphicItems') else 'N/A'}"
                        )
            ...
            if len(self.courtyards_map) > 0:
                print(
                    f"DEBUG: Extracted {len(self.courtyards_map)} courtyards. Keys: {list(self.courtyards_map.keys())[:5]}"
                )
            ...
            if hasattr(constraints, "feedback") and constraints.feedback.max_iterations > 1:
                logger.info(
                    f"Enabling Automated Zero-DRC Feedback Loop (max_iterations={constraints.feedback.max_iterations})"
                )
    ...
    def _export_to_kicad(self, final_state: BoardState, parse_result) -> None:
        ...
        strip_result = strip_routing(
            input_pcb=self.pcb_path, output_pcb=self.output_path, keep_zones=True, keep_fills=False
        )
        logger.info(
            f"Stripped {strip_result.traces_removed} traces and {strip_result.vias_removed} vias"
        )
        ...
        # Pass components for center offset conversion (bbox-center to footprint-origin)
