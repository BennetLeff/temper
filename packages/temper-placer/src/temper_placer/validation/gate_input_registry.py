...
_CI_SCRIPT_SURVEY: list[tuple[str, str, str]] = [
    ...
    ("check_inline_oracles.py", "scripts/inline_oracle_hashes.json", "inline-oracle content-hash gate (2026-08-18): pins the oracle blocks that live INSIDE test files, which check_oracle_hashes.py's _*_py_oracle.py glob cannot see -- the blind spot that let 35e3f914a delete 13 pinned inline oracles while the file-based registry check ran and reported success; probe is the check itself"),
    ...
    ("update_inline_oracle_hashes.py", "scripts/inline_oracle_hashes.json", "inline-oracle registry generator; not wired into CI (referenced in the workflow comment); shares its discovery/hashing with check_inline_oracles.py by import so generator and gate cannot drift apart; probe harness deferred"),
