...
_CI_SCRIPT_SURVEY: list[tuple[str, str, str]] = [
    ...
    ("check_pad_core_polygon_oracle.py", "pcb/temper.kicad_pcb", "pad CORE-POLYGON rotation-convention gate (#1380, the Rust twin of #1376's world-position gate): asserts every implementation that builds a pad's rotated core rectangle agrees with pcbnew via scripts/kicad_pad_polygon_oracle.py. Covers the copper-rectangle half of the R(+theta) vs R(-theta) question -- the one 70 of 137 HV<->SELV creepage verdicts flip on (docs/evidence/2026-08-24-pd3-creepage-burndown-remeasure.md). Behavioural, not a lint. Its own scripts/tests/test_check_pad_core_polygon_oracle.py runs in the same CI step and is the probe. Registered 2026-08-25, same completeness gap U4 caught for check_rust_coverage_illusions.py and write_board_sync_stamp.py"),
    ("check_pad_world_position_oracle.py", "pcb/temper.kicad_pcb", "pad world-position rotation-convention gate (#1376): asserts every implementation that places a footprint child in world coordinates agrees with pcbnew via scripts/kicad_pad_rotation_oracle.py -- the R(+theta) vs R(-theta) question that docs/evidence/2026-08-24-pd3-creepage-burndown-remeasure.md shows 70 of 137 HV<->SELV creepage verdicts flip on. Behavioural, not a lint. Its own scripts/tests/test_check_pad_world_position_oracle.py runs in the same CI step and is the probe. Registered 2026-08-25 -- it was invoked in python-tests.yml without a survey entry, the same completeness gap U4 caught for check_rust_coverage_illusions.py and write_board_sync_stamp.py on 2026-08-24"),
    ("check_router_clearance_floor.py", "scripts/", "router clearance-floor guard (#1113); source-scan + ratchet probe; probe harness deferred"),
    ("check_rust_coverage_illusions.py", "packages/", "coverage-illusion gate (2026-08-18, four same-day incidents): a Rust module that SHARES a Python module's NAME is not a Rust module that IMPLEMENTS it. Scans Rust and Python source for name-alike pairs whose live call path does not actually reach the Rust side; runs its own --self-test in the same CI step, which is the probe. Registered 2026-08-24 -- it was invoked in python-tests.yml without a survey entry, which is the completeness gap U4 exists to catch"),
    ("write_board_sync_stamp.py", "elec/build/default.net", "writes the pcb/temper.kicad_pcb.source-digest provenance stamp from the netlist CI just reconciled against (2026-08-13 desync postmortem). It is a WRITER, not a checker -- the gate is the `git diff --exit-code` that follows it in the same step (#1465's regen-and-diff), and it refuses to run unless check_netlist_board_reconciliation.py has just passed. Needs the compiled netlist (elec/build/, not committed), so no standalone probe; registered 2026-08-24 after #1465 added the invocation without a survey entry"),
    ...
    ("check_creepage_clearance_drift.py", "elec/src/constraints.ato", "cross-source creepage/clearance SSOT drift gate -- ENABLED 2026-08-15 (ci/unsilence-checks-batch-2): the python-tests.yml step invoking it was un-commented; it reports 4 mismatch families (exit 3) on origin/main by design (2 investigated non-defects, 2 DECISION-NEEDED families), in the non-required board-provenance-requirements-gates job -- visible honest red, not merge-blocking. Its own 38 unit tests pass against synthetic fixtures."),
    ...
    ("check_import_fallback_hygiene.py", "packages/", "first-party ImportError fallback hygiene gate; its dedicated scripts/tests/test_check_import_fallback_hygiene.py mutation-style fixtures exercise silent fallback, propagation, observability, and allowlist behavior in the same CI step"),
    ("sync_kicad_netclass_assignments.py", "pcb/temper.kicad_pro", "net -> netclass assignment correspondence gate: checks pcb/temper.kicad_pro net_settings.netclass_assignments against design_rules.py's TEMPER_NET_ASSIGNMENTS; requires the real board/project and is covered by scripts/tests/test_sync_kicad_netclass_assignments.py; probe harness deferred"),
    ...
    ("check_wasm_covered.py", "tools/wasm/", "wasm-covered CI-offload resolver (#wasm-covered-timing); registry-at-commit census of which tests the wasm tier mirrors; probe harness deferred"),
    ...
    ("check_pd2_compartment_evidence.py", "docs/specs/pd2_compartment_evidence.yaml", "PD2/8.0mm creepage compartment-evidence gate; BLOCKING as of 2026-08-18 -- this entry went on describing the gate as advisory, pending the compartment being built, long after the workflow step's continue-on-error was removed, which is the exact stale-prose shape check_ci_survey_advisory_drift.py exists to catch (and the same shape as its check_netclass_class_param_correspondence.py precedent). The wiring is deliberate: under PD3 the gate reports NOT_APPLICABLE (exit 0, pass by design) because the compartment prerequisite is moot while PD3 governs, and it fails closed if a future PR flips HV_CREEPAGE_ENFORCED_MM back to PD2 without committing docs/specs/pd2_compartment_evidence.yaml. Only the prose changed here; no gate or threshold moved. Probe harness deferred."),
    ...
    # 2026-08-18 trunk-health triage: 2 more gates invoked in python-tests.yml
    # but never added to this survey -- the same U4-completeness gap as the
    # 2026-08-12 and 2026-08-13 batches above, and caught the same way. Both
    # landed 2026-08-17 and both were wired straight into a blocking step.
    # Declared-inputs/reasons summarized from each script's scripts/manifest.yaml
    # entry and module docstring.
    ("check_fact_registry_drift.py", "docs/specs/REQUIREMENTS.md", "scalar fact-drift gate (2026-08-17 fact-deduplication audit, docs/evidence/2026-08-17-fact-dedup-inventory-and-gate.md): a non-creepage/clearance value declared in more than one home must agree across homes, or the gate fails closed -- the companion to check_creepage_clearance_drift.py, which deliberately covers only millimetre figures. Facts live in an in-file REGISTRY tuple (mains_voltage_v / pollution_degree / via geometry / gate-drive net names / HV-LV separation threshold) checked against their declared homes; REQUIREMENTS.md REQ-SYS-01 is the authoritative home for the mains-voltage fact. RED BY DESIGN on origin/main and deliberately so: two homes are pinned-oracle-entangled (tests/core/_design_rules_py_oracle.py, tests/core/_specification_py_oracle.py), so reconciling them needs one coordinated PR with an authorized oracle re-pin rather than a mechanical edit. BLOCKING (no continue-on-error on either of its two CI steps). Probe harness deferred; scripts/tests/test_check_fact_registry_drift.py covers it directly."),
    ("check_clone_drift.py", "scripts/clone_drift_registry.py", "registered clone-pair structural-drift gate (2026-08-18 stitch-congestion incident): resolves both qualified functions for every reviewed pair and fails when normalized-AST similarity drops below its recorded floor; missing files, missing or ambiguous qualnames, and an empty registry fail closed. BLOCKING (no continue-on-error); scripts/tests/test_check_clone_drift.py provides the two-sided drift/reconciliation probe and CLI exit-code coverage."),
    ("check_orphaned_python_modules.py", ".orphaned-python-module-inventory", "orphaned-module gate (2026-08-17, docs/evidence/2026-08-17-python-deprecation-spike.md / PR #1302): fails when a whole production .py module under packages/*/src has zero importers anywhere -- the third dead-code shape neither check_unwired_kernels.py (Rust-registered-but-uncalled) nor vulture_gate.py (unused names inside an imported file) can see. Resolves Python absolute/relative imports, importlib.import_module/reload string literals, [project.scripts] console-script targets, and Rust-side py.import/PyModule::import runtime calls. Shrink-only ledger in .orphaned-python-module-inventory; NEW_ORPHANED and STALE_ENTRY both fail. Documents its own blind spots (mutually-referencing dead clusters, computed import targets) rather than claiming completeness. BLOCKING (no continue-on-error on its CI step). Probe harness deferred."),
