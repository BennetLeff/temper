...
class ComponentOutcome:
    component_idx: int; ref: str
    routability_score: float; nets_connected: int; nets_failed: int
    any_failed: bool; position: tuple[float, float]
    solver_time_ms: float

@dataclass
class BoardResult:
    board_id: str; n_components: int; n_nets: int; solver_status: str
    outcomes: list[ComponentOutcome] = field(default_factory=list)
    spearman_rho: float = 0; spearman_p: float = 1
    point_biserial_r: float = 0; point_biserial_p: float = 1
    auc_roc: float | None = None; auc_ci: tuple[float|None, float|None] = (None, None)
    top_k_precision: float = 0; cohens_d: float = 0
    gradient_chi2_p: float | None = None; gradient_odds_ratio: float | None = None
    score_mean_failed: float = 0; score_mean_routed: float = 0
    notes: list[str] = field(default_factory=list)

@dataclass
class ExperimentReport:
    board_results: list[BoardResult]; total_components: int; total_boards: int
    pooled_spearman_rho: float = 0; pooled_spearman_ci: tuple[float, float] = (0, 0)
    pooled_auc_roc: float | None = None
    verdict: str = "DO_NOT_WIRE"; verdict_detail: str = ""

def _scipy():
    try: from scipy import stats; return stats
    except ImportError: return None

def _numpy_auc(scores, binary):
    desc = np.argsort(scores)[::-1]; sl = binary[desc]
    pos = int(np.sum(binary)); neg = len(binary) - pos
...
def compute_scores(stats, var_to_net, n_components, unsat_core=None, solver_status="unknown"):
    if n_components == 0: return np.zeros(0)
    ...
    if solver_status == "unsat" and unsat_core:
        ...
        for idx in unsat_core:
            if idx < len(var_to_net) and var_to_net[idx] < n_nets: pn[var_to_net[idx]] = 1.0
        ...
        ratio = stats.get("clause_to_var_ratio",0); t = stats.get("cpu_solve_time_ms",0)
        counts = np.bincount([n for n in var_to_net if 0<=n<n_nets], minlength=n_nets).astype(float)
...
def _spearman(scores, target):
    s = _scipy(); return (0.0,1.0) if not s or len(scores)<3 or np.all(scores==scores[0]) or np.all(target==target[0]) else tuple(map(float, s.spearmanr(scores, target)))

def _point_biserial(scores, binary):
    s = _scipy(); return (0.0,1.0) if not s or len(scores)<3 or np.all(scores==scores[0]) or np.all(binary==binary[0]) else tuple(map(float, s.pointbiserialr(binary, scores)))

def _auc(scores, binary):
    if len(scores) < 5: return None, None, None
    pos = int(np.sum(binary)); neg = len(binary) - pos
    if pos == 0 or neg == 0: return None, None, None
    auc = _numpy_auc(scores, binary)
    q1 = auc/(2-auc) if auc < 1 else 1.0; q2 = 2*auc*auc/(1+auc) if auc > 0 else 0
...
def _fisher_z(rs, ns):
    ...
    zs = [np.arctanh(r) for r,_ in v]; ws = [n for _,n in v]
    pz = np.average(zs, weights=ws); pr = np.tanh(pz)
...
def run_single_board(pcb_path, repo_root, score_threshold=0.5, proximity_mm=5.0):
    ...
    try:
        ...
        if not parsed or not hasattr(parsed, "components"): return None
    ...
    try:
        ...
        mb = ModelBuilder(skeletons={}, nets=pcb.nets, channel_widths={},
                          design_rules=getattr(pcb, "design_rules", None), diff_pairs=dp, pcb=pcb)
        cm = mb.build(); rr = solve_topology_rust(list(cm.variables), list(cm.constraints), net_names)
        st = rr.get("status","unknown")
    except Exception: rr = {"status":"unknown","solver_time_ms":0}
    stats = extract_stats(rr)
    vtn = rr.get("var_to_net") or [0]*len(net_names)
    scores = compute_scores(stats, vtn, nc,
                            unsat_core=rr.get("unsat_core",[]), solver_status=st)
    outcomes = []
    try:
        ...
        else: routing_outcomes = {}
    except Exception: routing_outcomes = {}
    for i, comp in enumerate(pcb.components):
        ...
        outcomes.append(ComponentOutcome(component_idx=i, ref=refs[i] if i<len(refs) else f"C{i}",
            routability_score=float(scores[i]) if i<len(scores) else 0,
            nets_connected=len(nets), nets_failed=nf, any_failed=nf>0,
            position=tuple(positions[i].tolist()), solver_time_ms=stats.get("cpu_solve_time_ms",0)))
    ...
    sr, sp = _spearman(sa, fa); pb, pp = _point_biserial(sa, ba)
    auc, alo, ahi = _auc(sa, ba)
    tk = float(np.mean(ba[np.argsort(sa)[-5:]])) if len(sa) >= 5 and np.sum(ba) > 0 else 0.0
    fv = sa[ba>0]; rv = sa[ba==0]
    cd = float((np.mean(fv)-np.mean(rv))/max(np.std(sa,ddof=1),1e-10)) if len(fv)>1 and len(rv)>1 else 0
    mf = float(np.mean(fv)) if len(fv)>0 else 0; mr = float(np.mean(rv)) if len(rv)>0 else 0
    return BoardResult(board_id=bid, n_components=nc, n_nets=len(net_names), solver_status=st,
        outcomes=outcomes, spearman_rho=sr, spearman_p=sp, point_biserial_r=pb, point_biserial_p=pp,
        auc_roc=auc, auc_ci=(alo,ahi), top_k_precision=tk, cohens_d=cd,
        score_mean_failed=mf, score_mean_routed=mr)
...
def run_experiment(pcb_paths, repo_root, score_threshold=0.5, proximity_mm=5.0):
    ...
    for p in pcb_paths:
        ...
        if br is None: print("SKIP"); continue
        results.append(br)
        print(f"{br.n_components}c, ρ={br.spearman_rho:.3f} (p={br.spearman_p:.3f}) {'✓' if br.spearman_p<0.05 else '✗'}")
    if not results: return ExperimentReport(board_results=[], total_components=0, total_boards=0)
    ...
    if ars: para, paci = _fisher_z([a for a,_ in ars], [n for _,n in ars])
    else: para, paci = None, (None, None)
    gates = {"predictive": pr>=0.30 and pci[0]>0, "discriminative": para is not None and para>=0.65 and (paci[0] or 0)>0.50}
    passed = sum(gates.values())
    v = "WIRE" if passed==2 else ("BORDERLINE" if passed==1 else "DO_NOT_WIRE")
    def fm(x): return f"{x:.3f}" if x is not None else "N/A"
    return ExperimentReport(board_results=results, total_components=sum(r.n_components for r in results),
        total_boards=len(results), pooled_spearman_rho=pr, pooled_spearman_ci=pci, pooled_auc_roc=para,
        verdict=v, verdict_detail=f"ρ={fm(pr)} CI=({fm(pci[0])},{fm(pci[1])}), AUC={fm(para)}, gates={gates}")
...
def cli_main():
    ...
    ap.add_argument("--corpus", action="store_true"); ap.add_argument("--golden", action="store_true")
    ...
    if args.corpus:
        ...
        if m.exists():
            import yaml; data = yaml.safe_load(open(m)) or {}
    ...
    if args.golden:
        ...
        if gm.exists():
            import yaml; data = yaml.safe_load(open(gm)) or {}
            for e in data.get("boards", []):
                if e.get("path"): paths.append(repo / e["path"])
    if not paths: ap.error("Provide --pcb, --corpus, or --golden")
    ...
    if args.output:
        out = {"verdict": report.verdict, "verdict_detail": report.verdict_detail,
               ...
               "total_components": report.total_components, "total_boards": report.total_boards,
               "board_results": [{"board_id": r.board_id, "n_components": r.n_components,
                                  "spearman_rho": r.spearman_rho, "spearman_p": r.spearman_p,
                                  "auc_roc": r.auc_roc, "top_k_precision": r.top_k_precision,
                                  "score_mean_failed": r.score_mean_failed,
                                  "score_mean_routed": r.score_mean_routed,
                                  "notes": r.notes} for r in report.board_results]}
