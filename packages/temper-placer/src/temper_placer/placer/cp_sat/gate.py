...
from temper_placer.validation.drc_runner import DrcResult, run_drc
...
class AcceptanceGate:
    ...
    def inner_gate(
        ...
        return auditor.audit(constraints)
    ...
    def truth_gate(self, pcb_path: Path | None = None) -> DrcResult:
        ...
        if not path.exists():
            logger.warning("PCB file %s does not exist; skipping DRC", path)
            return DrcResult(error_count=0, warning_count=0)
        return run_drc(path)
    ...
    def accept(
        ...
        """
        audit_report = self.inner_gate(placement, constraints)
