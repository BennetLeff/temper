...
class NetClassRule(BaseModel):
    ...
    description: str = Field(default="", description="Human-readable description")
    voltage_v: float = Field(default=0.0, ge=0, description="Working voltage for creepage calculation")
    max_current_rating: float | None = Field(default=None, description="Maximum current in Amps")
    routing_strategy: str | None = Field(default=None, description="Routing strategy: plane_required, plane_preferred, wide_trace, standard")
    via_cost_multiplier: float = Field(default=1.0, ge=0, description="Multiplier for via cost (higher = fewer vias)")
...
class DifferentialPairRule(BaseModel):
    ...
    spacing_mm: float = Field(default=0.2, gt=0, description="Nominal gap between differential traces in mm")
    coupling_tolerance_mm: float = Field(default=0.5, ge=0, description="Maximum deviation from nominal spacing in mm")
    impedance_ohm: float | None = Field(default=None, description="Target differential impedance in Ohms")
...
class SignalToHVClearance(BaseModel):
    ...
    required_clearance_mm: float = Field(default=6.0, ge=0, description="Minimum clearance from signal path to HV pins")  # allow-safety-constant: IEC standard clearance
    max_path_length_mm: float = Field(default=20.0, gt=0, description="Maximum allowed signal path length in mm")
