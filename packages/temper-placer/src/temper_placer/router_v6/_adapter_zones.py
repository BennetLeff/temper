"""Copper-pour zone emission for the router_v6 adapter.

Extracted from ``_adapter_convert.py`` (temper-N7-cap5, to keep that module
under the LOC cap): zone-pour eligibility/margin resolution for
ACMains/HighVoltage nets (driven from ``NetClassRules.routing_strategy ==
"plane_required"``, not a hardcoded netclass list), plus isolated-pad
stitching and zone-pour emission.
"""
...
def _stitch_isolated_pads(
    ...
    *,
    ...
    # Lazy import to avoid a circular dependency: _adapter_convert imports
    # _emit_zone_pours (and this function) from this module at module scope,
    # so this module cannot import _adapter_convert at module scope in turn.
