...
class RoutabilityAggregator:
    ...
    def compute_scores(
        ...
        Returns:
            (routability_scores: (N,) np.ndarray, score_mean: float)
        """
        if n_components == 0:
        ...
        per_component = np.clip(per_component, 0.0, 1.0)
        score_mean = float(np.mean(per_component))
    ...
    def _sat_fine_grained_scores(
        ...
        a_n = np.ones(n_nets) / max(n_nets, 1)
        ...
        # Normalize
        b_max = max(float(np.max(b_n)), 1.0)
        a_max = max(float(np.max(a_n)), 1.0)
        d_max = max(float(np.max(d_n)), 1.0)
        c_max = 1.0
    ...
    def _decision_level_proxy(
        ...
        if n_nets == 0:
            return np.zeros(0)
        return net_var_counts.astype(np.float32) / max(float(np.max(net_var_counts)), 1.0)
