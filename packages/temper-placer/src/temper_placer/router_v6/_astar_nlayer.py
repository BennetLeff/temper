...
def run_astar_pathfinding_nlayer(
    ...
    def attempt_route(net_name: str):
        ...
        if route_path:
            ...
            # holes_co_located/annular_width counts). The 0.2mm tolerance
            # catches every genuinely co-located via (A* places vias at
            # grid-cell centres up to ~0.07mm from the exact pad centre;
            # drill radii sum >= 0.3mm) without touching vias that merely
            # pass near a large pad.
            tht_pad_positions = [
                (px, py)
            ...
            if tht_pad_positions or len(route_path.via_positions) != len(
                ...
                for vx, vy in route_path.via_positions:
                    ...
                    seen.add(key)
                    if any(
                        # 0.2mm tolerance: the A* places vias at grid-cell
                        # centres, which can sit up to ~0.07mm from the
                        # exact pad centre; 0.2mm catches every genuinely
                        # co-located via (drill radii sum >= 0.3mm) without
                        # touching vias that merely pass near a large pad.
                        abs(vx - px) < 0.2 and abs(vy - py) < 0.2
            ...
            # routed_paths is assigned HERE, AFTER the THT/dedupe filter
            # above -- the earlier assignment before the filter left
            # routed_paths pointing at the unfiltered object, so
            # place_vias() emitted the THT-pad vias and duplicates the
            # filter had removed from the stamp (holes_co_located stayed
            # at 48 on the first fixed route for exactly this reason).
