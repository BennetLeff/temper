...
class RoutingValidator:
    def __init__(self, default_cell_size_mm: float, grid_size: tuple[int, int], num_layers: int):
        ...
        ] = {}  # Keyed by world-scale (finest grid) coordinates
    ...
    def _grid_to_world(self, gx: int, gy: int) -> tuple[str, str]:
        return (
            str(gx * self.default_cell_size),
            str(gy * self.default_cell_size),
    ...
    def validate(
        ...
        for net_name, path in routed_paths.items():
            ...
            path_cell_size = (
                path.cell_size if hasattr(path, "cell_size") else self.default_cell_size
            )
            ...
            if len(net.pins) > 1:
                ...
                for comp_ref, pin_name in net.pins:
                    ...
                    for layer in range(self.num_layers):
                        if (mx, my, layer) in self.occupancy_map and self.occupancy_map[
                            (mx, my, layer)
                        ] == net.name:
                            found = True
                            break
                        ...
                        for dx in range(-search_px, search_px + 1):
                            for dy in range(-search_px, search_px + 1):
                                for layer in range(self.num_layers):
                                    ...
                                    if (
                                        mkey in self.occupancy_map
                                        and self.occupancy_map[mkey] == net.name
                                    ):
                                        found = True
                                        break
                                if found:
