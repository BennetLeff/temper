...
// Exposes every eligible `#[test]` in `encoder`/`ipc`/`loss` as a flat,
// indexable `(&'static str, fn())` list so a `wasm32` entry point
// (`temper-wasm-test-runner`) can call each one -- `cargo test`'s libtest
// harness cannot target `wasm32-unknown-unknown`.  Off by default; never
// enabled for the Python extension module.  Generated by
// `scripts/gen_wasm_test_registry.py --crate temper-constraints`.
#[cfg(feature = "wasm-registry")]
pub mod wasm_test_registry;

// Everything in `py` below is the pyo3 boundary: exception types, the
// `#[pyfunction]`/`#[pyclass]` wrappers around the pure kernels in
// `constraints`/`loss`, and the `#[pymodule]` entry point itself.  None of it
// exists in a `--no-default-features` (`wasm32`) build --
// `scripts/gen_wasm_test_registry.py --crate temper-constraints --census`
// prints the full module census.  `loss.rs` has zero pyo3 coupling, and
// `encoder.rs`/`ipc.rs` keep their pure kernels' error types plain
// (`Result<_, String>` / `f64`) precisely so this single `#[cfg(feature =
// "python")]` boundary can sit here, at the wrapper layer, rather than
// requiring pyo3 types to leak into the kernels themselves.
#[cfg(feature = "python")]
mod py {
    ...
    fn temper_constraints(m: &Bound<'_, PyModule>) -> PyResult<()> {
        ...
        m.add_function(wrap_pyfunction!(crate::encoder::mm_to_units_py, m)?)?;
        m.add_function(wrap_pyfunction!(crate::encoder::units_to_mm_py, m)?)?;
