...
class LossContext(BaseLossContext):
    ...
    def from_netlist_and_board(
        ...
        fiducial_indices = np.array(fiducial_indices_list, dtype=np.int32)
        component_type_indices = {
            t: np.array(idxs, dtype=np.int32)
            for t, idxs in comp_type_map.items()
        }

        # Convert net class indices to NumPy arrays
        net_class_indices = {
            nc: np.array(indices, dtype=np.int32)
        ...
        if use_centrality_weighting:
            ...
            centrality = np.ones(netlist.n_components) / max(netlist.n_components, 1)
        ...
        if domains:
            domain_bounds = np.array([d.bounds for d in domains], dtype=np.float32)
            domain_star_points = np.array(
                [d.star_point if d.star_point else [0.0, 0.0] for d in domains],
                dtype=np.float32
            )
            domain_has_star = np.array([d.star_point is not None for d in domains], dtype=bool)
        else:
            domain_bounds = np.zeros((0, 4), dtype=np.float32)
            domain_star_points = np.zeros((0, 2), dtype=np.float32)
            domain_has_star = np.zeros((0,), dtype=bool)

        geometry_context = GeometryContext(
            bounds=bounds,
            fixed_mask=fixed_mask,
            origin=np.array(getattr(board, 'origin', (0.0, 0.0)), dtype=np.float32),
        ...
        netlist_context = NetlistContext(
            ...
            hv_indices=np.array(hv_indices, dtype=np.int32),
            lv_indices=np.array(lv_indices, dtype=np.int32),
            fiducial_indices=fiducial_indices
        )

        # 3. Create ConstraintContext
        is_star_net = np.array([
    ...
    def _precompute_net_arrays(
        ...
        for net in valid_nets:
            ...
            if centrality is not None and centrality.shape[0] > 0:
                # Boost net weight by max centrality (hub prioritization)
                max_c = np.max(centrality[np.array(net_comp_indices)])
        ...
        return (
            ...
            np.array(layer_counts, dtype=np.int32),
            max_pins,
        )
    ...
    def _precompute_loop_arrays(
        ...
        for loop in loop_constraints:
            ...
            if centrality is not None and centrality.shape[0] > 0 and loop_comp_indices:
                # Boost loop weight by max centrality
                max_c = np.max(centrality[np.array(loop_comp_indices)])
                weight = weight * (max_c * n_components)
        ...
        return (
            ...
            np.array(offsets, dtype=np.float32),
            np.array(masks, dtype=bool),
            np.array(max_areas, dtype=np.float32),
            np.array(weights, dtype=np.float32),
        )
    ...
    def _precompute_path_arrays(
        ...
        for path in path_constraints:
            try:
                ...
                weight = path.weight
                if centrality is not None and centrality.shape[0] > 0:
                    max_c = np.max(centrality[np.array([from_idx, to_idx])])
                    weight = weight * (max_c * n_components)
                weights.append(weight)
        ...
        return (
            np.array(indices, dtype=np.int32),
            np.array(offsets, dtype=np.float32),
            np.array(max_lengths, dtype=np.float32),
            np.array(weights, dtype=np.float32),
        )
    ...
    def _precompute_star_ground_arrays(
        ...
        return (
            ...
            np.array(has_anchors, dtype=bool),
        )
...
def smooth_step(x: np.ndarray | float, edge0: float = 0.0, edge1: float = 1.0) -> np.ndarray | float:
    """
...
class CompositeLoss(LossFunction):
    ...
    def __call__(
        ...
        epoch: int = 0,
        total_epochs: int = 1,
        weight_overrides: np.ndarray | None = None,
        net_virtual_nodes: np.ndarray | None = None,
    ) -> LossResult:
        """
        ...
        """
        total = 0.0
        breakdown: dict[str, np.ndarray | float] = {}
        ...
        for i, wloss in enumerate(self.losses):
            ...
            # NaN/Inf guard
            normalized_value = np.nan_to_num(normalized_value, nan=1e6, posinf=1e6, neginf=1e6)
...
def apply_fixed_mask_to_gradients(
    ...
    """
    grad_pos = np.where(fixed_mask[:, None], 0.0, grad_pos)
    grad_rot = np.where(fixed_mask[:, None], 0.0, grad_rot)
