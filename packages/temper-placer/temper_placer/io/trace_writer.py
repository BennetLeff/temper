...
def write_exact_routes_to_pcb(
    template_pcb: Path,
    output_pcb: Path,
    routing_results: dict, # dict[net_name, ExactRoutePath]
) -> int:
    """Write exact geometry routing results to KiCad PCB file directly using kiutils.
    
    Args:
        template_pcb: Input PCB path to use as base
        output_pcb: Output path
        routing_results: Dictionary of net_name -> ExactRoutePath
        
    Returns:
        Number of items added
    """
    from temper_placer.io.kicad_parser import parse_kicad_pcb
    from temper_placer.router_v6.exact_geometry_router import ExactRoutePath
    
    # Parse template
    parse_result = parse_kicad_pcb(template_pcb, normalize=False)
    board = parse_result.board
    
    items_added = 0
    
    for net_name, path in routing_results.items():
        if not path.segments and not path.vias:
            continue
            
        # Find net object in board to get reference
        board_net = next((n for n in board.nets if n.name == net_name), None)
