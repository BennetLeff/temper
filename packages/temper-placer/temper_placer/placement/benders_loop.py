...
class BendersOptimizer:
        ...
        try:
            ...
            for iteration in range(self.max_iterations):
                ...
                if not self.check_routability:
                    return self._build_result(
                        BendersStatus.FEASIBLE
                        if master_result.status == "OPTIMAL"
                        else BendersStatus.ERROR,
                    ...
                    if router_result and router_result.failure_count == 0:
                        ...
                        if self.require_drc_clean:
                            ...
                            if drc_result and drc_result.actionable_error_count == 0:
                                ...
                                if self.verbose:
                                    print(
                                        f"  ✗ DRC has {drc_result.actionable_error_count} actionable errors"
                                    )
    ...
    def _check_routability(self, positions: dict[str, tuple[float, float]]) -> tuple[bool, list]:
        ...
        try:
            ...
            # 3. Create or update cut generator with design_rules
            from temper_placer.placement.benders_cut_generator import BendersCutGenerator

            if design_rules is not None:
                # Populate net_class_assignments from constraints file if not present
                if not design_rules.net_class_assignments:
                    from temper_placer.io.config_loader import load_constraints
                    from pathlib import Path

                    # Path(__file__) = .../temper-placer/src/temper_placer/placement/benders_loop.py
                    # Need: .../temper-placer/configs/temper_constraints.yaml
                    temper_placer_root = Path(__file__).parent.parent.parent.parent
                    constraints_path = temper_placer_root / "configs" / "temper_constraints.yaml"
                    if constraints_path.exists():
                        try:
                            ...
                            # Extract net_classes mapping directly from YAML
                            net_classes = config.get("net_classes", {})
                            for net_name, class_name in net_classes.items():
                                design_rules.net_class_assignments[net_name] = class_name
                            if self.verbose:
                                print(
                                    f"  Loaded {len(design_rules.net_class_assignments)} net class assignments from constraints"
                                )
                        except Exception as e:
                            if self.verbose:
                                print(
                                    f"  Warning: Failed to load net class assignments from constraints: {e}"
                                )

                self.design_rules = design_rules
                self._cut_generator = BendersCutGenerator(design_rules=design_rules)
            else:
                raise ValueError("design_rules is required but router pipeline returned None")

            # 4. Extract nets from PCB
    ...
    def _check_routability_ultrafast(
        ...
        sizes = {c["ref"]: (c["width_mm"], c["height_mm"]) for c in data["components"]}
        ...
        if self.verbose:
            print(
                f"  Ultra-fast: congestion={result.congestion_score:.1%}, overlaps={result.overlap_count}"
            )
    ...
    def _generate_cuts_from_mincut(self, min_cut_edges: list) -> list[RoutabilityCut]:
        ...
        # Generate cuts
        # Sort cuts by required gap (descending) to prioritize most critical cuts
        cuts = self._cut_generator.generate_cuts(blocking, iteration=self.current_iteration)
        cuts.sort(key=lambda c: c.gap_required, reverse=True)

        # Limit to top 10 cuts per iteration to allow gradual convergence
        # Adding too many cuts at once can make the ILP locally infeasible
        max_cuts = 10
        if len(cuts) > max_cuts:
            if self.verbose:
                print(f"Limiting cuts to top {max_cuts} (from {len(cuts)})")
            cuts = cuts[:max_cuts]
    ...
    def _run_router_pipeline(self):
        ...
        try:
            ...
            return (result.stage2.skeletons, result.stage2.channel_widths, result.pcb.design_rules)
    ...
    def _extract_nets_from_placement(self, positions: dict[str, tuple[float, float]]) -> dict:
        ...
        try:
            ...
            for net in parse_result.netlist.nets:
                ...
                pads_for_net = [pad for pad in parse_result.pads if pad.net == net.name]
    ...
    def _run_actual_router(self, positions: dict[str, tuple[float, float]]):
        ...
        try:
            ...
            if self.verbose:
                success = (
                    pathfinding_result.success_count
                    if hasattr(pathfinding_result, "success_count")
                    else len(pathfinding_result.routed_paths)
                )
                failed = (
                    pathfinding_result.failure_count
                    if hasattr(pathfinding_result, "failure_count")
                    else len(pathfinding_result.failed_nets)
                )
            ...
            routes, vias = self._extract_routes_for_export(
                pathfinding_result, width_assignment, result.pcb.design_rules
            )
    ...
    def _run_drc_check(self, pcb_file: Path):
        ...
        try:
            ...
            if self.verbose:
                actionable = (
                    len(result.actionable_violations)
                    if hasattr(result, "actionable_violations")
                    else 0
                )
                cosmetic = (
                    len(result.cosmetic_violations) if hasattr(result, "cosmetic_violations") else 0
                )
