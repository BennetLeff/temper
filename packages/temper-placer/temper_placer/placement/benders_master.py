...
class PlacementConstraints:
    ...
    # Movement budget
    max_single_movement_mm: float = 50.0
    max_total_movement_mm: float = 300.0
...
class BendersMasterProblem:
    ...
    def _create_position_variables(self) -> None:
        ...
        for comp in self._comp_list:
            ...
            self._x[ref] = solver.NumVar(-solver.infinity(), solver.infinity(), f"x_{ref}")
            self._y[ref] = solver.NumVar(-solver.infinity(), solver.infinity(), f"y_{ref}")
    ...
    def _add_hv_clearance_constraints(self) -> None:
        ...
        lv_components = [
            c for c in self._comp_list if c.classification not in ("HV", "FIXED") and not c.hv_nets
        ]
        ...
        for hv in hv_components:
            ...
            clearance = (
                self.constraints.ac_clearance_mm if has_ac else self.constraints.hv_clearance_mm
            )
    ...
    def _add_movement_constraints(self) -> None:
        ...
        for comp in self._comp_list:
            if comp.classification == "FIXED":
                continue
            solver.Add(self._delta_x[comp.ref] + self._delta_y[comp.ref] <= max_single)
    ...
    def _load_zone_constraints_from_yaml(cls) -> dict[str, list[tuple[str, str, float]]]:
        ...
        try:
            ...
            print("⚠ Warning: temper_constraints.yaml not found, using empty zone constraints")
            return {}

        except Exception as e:
            print(f"⚠ Warning: Failed to load zone constraints: {e}")
            return {}
    ...
    def validate_zone_compliance(
        ...
        Args:
            positions: Current positions {ref: (x, y)}
        ...
        for comp_ref, constraints in self.constraints.zone_constraints.items():
    ...
    def add_routability_cut(
        self, cut_type: str, components: list[str], gap_required: float
    ) -> None:
        """
        ...
        # Use Big-M formulation to allow component re-ordering or orthogonal escape
        # Rigid constraints (x2 - x1 >= gap) lock the relative order and prevent
        # vertical stacking to resolve horizontal bottlenecks.
        M = self.BIG_M
        
        c1, c2 = components[0], components[1]
        
        if c1 not in self._x or c2 not in self._x:
            return

        w1, h1 = self.components[c1].width_mm, self.components[c1].height_mm
        w2, h2 = self.components[c2].width_mm, self.components[c2].height_mm
        
        # Binary variables for relative position
        # We can reuse existing ones if available, or create new ones
        # For simplicity in this cut generation, we create new local binaries 
        # to represent the specific resolution strategy for this cut
        b_left = solver.BoolVar(f"cut_b_left_{c1}_{c2}_{len(self._cuts)}")
        b_right = solver.BoolVar(f"cut_b_right_{c1}_{c2}_{len(self._cuts)}")
        b_above = solver.BoolVar(f"cut_b_above_{c1}_{c2}_{len(self._cuts)}")
        b_below = solver.BoolVar(f"cut_b_below_{c1}_{c2}_{len(self._cuts)}")

        # Standard clearance for the non-critical axis
        std_clearance = self.constraints.min_component_clearance_mm
        
        # Horizontal Cut: Needs extra space in X, or standard space in Y
        ...
        # 1. c1 left of c2
        solver.Add(self._x[c1] + w1/2 + x_gap <= self._x[c2] - w2/2 + M * (1 - b_left))
        
        # 2. c1 right of c2
        solver.Add(self._x[c2] + w2/2 + x_gap <= self._x[c1] - w1/2 + M * (1 - b_right))
        
        # 3. c1 below c2
        solver.Add(self._y[c1] + h1/2 + y_gap <= self._y[c2] - h2/2 + M * (1 - b_below))
        
        # 4. c1 above c2
        solver.Add(self._y[c2] + h2/2 + y_gap <= self._y[c1] - h1/2 + M * (1 - b_above))

        # Require at least one condition to be true
        constraint = solver.Add(b_left + b_right + b_above + b_below >= 1)
    ...
    def from_json(cls, json_path: str | Path) -> "BendersMasterProblem":
        ...
        constraints = PlacementConstraints(
            ...
            # Zone constraints - loaded from temper_constraints.yaml
            # This ensures single source of truth for zone definitions
            zone_constraints=cls._load_zone_constraints_from_yaml(),
        )
        ...
        if result.movements:
            ...
            for ref, movement in top_movers:
                if movement > 0.01:
                    ...
                    print(
                        f"  {ref}: {movement:.2f}mm ({old_x:.1f},{old_y:.1f}) -> ({new_x:.1f},{new_y:.1f})"
                    )
...
if __name__ == "__main__":
    ...
    json_path = (
        sys.argv[1] if len(sys.argv) > 1 else "packages/temper-placer/data/benders_input.json"
    )
