    ...
    def apply(self, context: PlacementContext) -> HeuristicResult:
        ...
        for ref in phase_config.components:
            ...
            if "U_GATE" in ref:
                ...
                # Q1 is at X=25. We want U_GATE at X≈25 (Safe Zone).
                # Previous offset -max_dist * 0.8 (-16mm) put it at X=9, but forces pulled it to X=28.
                # Let's target X=24 explicitly relative to Q1 (Offset -1.5mm)
                offset = (-1.5, 0)
            elif "C_BOOT" in ref or "C_VCC" in ref:
                # Bootstrap/decoupling - Left of driver to avoid AC barrier
                # Previous offset (+3) put it to the right, crossing the barrier.
                # New offset (-4.5) puts it at X=21 (Left of U_GATE)
                offset = (-4.5, 3 if "BOOT" in ref else -3)
            elif "R_GATE" in ref:
                # Gate resistors - between driver and IGBTs
                offset = (-0.5, 3 if "H" in ref else -3)
            else:
            ...
            placement = ComponentPlacement(
                ...
                position=(float(x), float(y)),
                rotation=0,
                confidence=1.0,  # Lock these positions to prevent drift into HV zone
                placed_by=self.name,
            )
            result.placements[ref] = placement
