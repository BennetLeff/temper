...
# ---------------------------------------------------------------------------
# PCL layer (packages/temper-design-bundle/src/pcl.rs, pcl_parse.rs)
#
# Consumed by temper_placer.pcl._parse_utils and temper_placer.pcl.tag_dispatch,
# which re-export the Tag* / ComponentRef pyclasses under their original names
# and build `TagExpr` as a Union of them — so these must be declared as
# *classes*, not aliases, or every annotation using TagExpr becomes
# "Variable ... is not valid as a type".
#
# The enum returns are `Any` on purpose: the concrete ConstraintTier /
# DistanceMetric / Axis / BoardSide / EdgeType live in
# temper_placer.pcl.constraints (not in this extension), and the Python
# wrappers in _parse_utils.py carry the precise annotations.
# ---------------------------------------------------------------------------

def pcl_parse_distance_with_unit(value: Any) -> float: ...
def pcl_parse_tier(tier_value: Any) -> Any: ...
def pcl_parse_metric(metric_value: str | None) -> Any: ...
def pcl_parse_axis(axis_value: str) -> Any: ...
def pcl_parse_board_side(side_value: str) -> Any: ...
def pcl_parse_edge_type(edge_value: str) -> Any: ...
def pcl_tag_le(left: Any, right: Any) -> bool: ...
def pcl_tag_closure() -> dict[Any, frozenset[Any]]: ...
def pcl_tag_to_component_refs(expr: Any, netlist: object) -> list[str]: ...
def pcl_resolve(expr: Any, comp: object) -> bool: ...
def pcl_check_overconstrained(*args: Any, **kwargs: Any) -> Any: ...
def pcl_components(expr: Any, netlist: object) -> list[Any]: ...
...
class ComponentRef:
    def __init__(self, refdes: str) -> None: ...

class LayerAssignment:
    """`#[pyclass]` from deterministic_leaves.rs — a per-net layer decision."""
    ...
    @property
    def allow_layer_change(self) -> bool: ...
