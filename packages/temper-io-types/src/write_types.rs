...
//!
//! The former `footprint_value_py` binding is intentionally not part of this
//! surface.  The write path no longer calls that helper: value-bearing
//! footprint data comes from the live parse-engine boundary
//! (`extract_footprint_info_py`).  Keeping a second, test-only Value reader
//! registered here would preserve a superseded API with no production caller.
//!
