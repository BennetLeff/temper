...
create_exception!(temper_io_types, BoardIdentityError, pyo3::exceptions::PyValueError);
...
fn verify_config_matches_netlist(
    ...
    if !missing.is_empty() {
        ...
        Err(ConfigBoardMismatchError::new_err(format!(
...
// ---------------------------------------------------------------------------
// design_bundle_preflight: preflight_identity
// ---------------------------------------------------------------------------

#[pyfunction]
#[pyo3(signature = (pcb_path, netlist_path, *, min_overlap = 0.95, bring_up = false))]
fn preflight_identity(
    ...
    let pcb_bytes = std::fs::read(&pcb_path).map_err(|e| {
        pyo3::exceptions::PyFileNotFoundError::new_err(format!(
            "Cannot read PCB file {}: {}",
            pcb_path.display(),
            e
        ))
    })?;
    let netlist_bytes = std::fs::read(&netlist_path).map_err(|e| {
        pyo3::exceptions::PyFileNotFoundError::new_err(format!(
            "Cannot read netlist file {}: {}",
    ...
    match tdb.call_method1(
        "preflight_identity",
        (
            pcb_path.to_str().unwrap_or(""),
            pcb_bytes,
            netlist_bytes,
            min_overlap,
            bring_up,
        ...
        Err(e) => {
            ...
            Err(BoardIdentityError::new_err(msg))
        }
    }
}
...
fn temper_io_types(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add(
        "BoardIdentityError",
        m.py().get_type::<BoardIdentityError>(),
    )?;
    ...
    m.add_function(wrap_pyfunction!(preflight_identity, m)?)?;
