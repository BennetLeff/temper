...
fn check_signers(package: &JointQualificationPackage) -> Result<(), QualificationError> {
    ...
    for signoff in &package.signoffs {
        ...
        let expected_verifier = if signoff.role == "iso.verification" {
            "ct07.verification"
        } else if signoff.role == "ct07.verification" || signoff.role.starts_with("iso.") {
            "iso.verification"
        } else {
            "ct07.verification"
        };
...
pub fn evaluate_u9_json(input: &str) -> Result<String, QualificationError> {
    ...
    if signoffs
        ...
        .is_none_or(|rows| rows.len() != REQUIRED_SIGNER_ROLES.len())
    {
...
pub mod tests {
    ...
    fn package() -> JointQualificationPackage {
        ...
        let roles = JOINT_AXIS_OWNERS
            ...
            .map(|(axis, role)| Signoff {
                ...
                } else if role == "ct07.verification" || role.starts_with("iso.") {
    ...
    pub const WASM_TESTS: &[(&str, fn())] = &[
        (
            "owner_joint_candidate::tests::exact_boundary_is_inclusive_and_fractional_terms_round_up",
            exact_boundary_is_inclusive_and_fractional_terms_round_up,
        ),
        (
            "owner_joint_candidate::tests::over_budget_rejects_and_missing_signer_stops",
            over_budget_rejects_and_missing_signer_stops,
        ),
        (
            "owner_joint_candidate::tests::duplicate_uncertainty_or_noncanonical_decimal_stops",
            duplicate_uncertainty_or_noncanonical_decimal_stops,
        ),
        (
            "owner_joint_candidate::tests::negative_timing_and_missing_direct_endpoint_stop",
            negative_timing_and_missing_direct_endpoint_stop,
        ),
        (
            "owner_joint_candidate::tests::approval_requires_every_captured_byte_binding",
            approval_requires_every_captured_byte_binding,
        ),
    ];
