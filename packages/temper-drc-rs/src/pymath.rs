...
/// Exact `2^e` for every `i32` exponent, matching `temper-geometry`'s copy.
///
/// The obvious `2f64.powi(e)` is NOT sufficient here: for `e = -1024`
/// (reachable from `frexp` of any `x >= 2^1023`, i.e. the top binade),
/// `powi(-1024)` underflows to `0.0`. Then `v * scale = v * 0 = 0`,
/// `h = 0`, and the final `h / scale = 0 / 0 = NaN` — `py_hypot` returns
/// NaN where CPython returns the correct `x`. The subnormal `2^-1024`
/// (`5.56e-309`) must be built from its bit pattern to keep the full
/// mantissa precision. Same class as `temper-geometry/src/pad_geometry.rs`
/// (`pow2`, fixed there first; this port exists to eliminate the
/// divergence, and the proptest `p15_py_hypot_zero_returns_abs` caught
/// the copy still carrying the old `powi`).
fn pow2(e: i32) -> f64 {
    if e > 1023 {
        return f64::INFINITY;
    }
...
pub(crate) mod tests {
    ...
    fn hypot_pinned_values() {
        ...
        // (same pin as temper-geometry's `py_hypot_matches_cpython_on_the_top_binade`).
        assert_eq!(py_hypot(1e308, 1e308).to_bits(), 0x7FE92C80954C51F5);
        assert!(py_hypot(f64::MAX, 0.0) == f64::MAX);
        assert!(py_hypot(1.091009947397983e308, 0.0) == 1.091009947397983e308);
    }
...
mod proptests {
    ...
    /// Normal f64 values — avoids NaN and infinity so properties about
    /// the selected argument hold without NaN special cases.
    ...
    proptest! {
        // -----------------------------------------------------------------
        // py_max — structural properties
        // -----------------------------------------------------------------

        /// P1. py_max(a,b) returns the larger of two non-NaN values.
        ///
        /// CPython's builtin `max` returns `b if b > a else a`, so for
        /// non-NaN operands the result is the conventional maximum.
        #[test]
        fn p1_py_max_returns_larger(a in normal(), b in normal()) {
            let r = py_max(a, b);
            let expected = a.max(b);
            prop_assert_eq!(r, expected,
                "py_max({},{})={} != max={}", a, b, r, expected);
        }

        /// P2. py_max returns one of its two arguments (identity for all
        /// non-NaN inputs).  On a tie, the first argument wins per
        /// CPython semantics.
        #[test]
        fn p2_py_max_returns_one_of_inputs(a in normal(), b in normal()) {
            let r = py_max(a, b);
            prop_assert!(r.to_bits() == a.to_bits() || r.to_bits() == b.to_bits(),
                "py_max({},{})={} is neither a nor b", a, b, r);
        }

        /// P3. When the first argument is NaN, py_max returns NaN
        /// (CPython `max` keeps the first argument).
        #[test]
        fn p3_py_max_nan_first_returns_nan(b in normal()) {
            let nan = f64::NAN;
            prop_assert!(py_max(nan, b).is_nan());
        }

        /// P4. When only the second argument is NaN, py_max keeps the
        /// first (non-NaN) argument.
        #[test]
        fn p4_py_max_nan_second_returns_first(a in normal()) {
            let nan = f64::NAN;
            prop_assert_eq!(py_max(a, nan), a);
        }

        // -----------------------------------------------------------------
        // py_min — structural properties
        // -----------------------------------------------------------------

        /// P5. py_min(a,b) returns the smaller of two non-NaN values.
        #[test]
        fn p5_py_min_returns_smaller(a in normal(), b in normal()) {
            let r = py_min(a, b);
            let expected = a.min(b);
            prop_assert_eq!(r, expected,
                "py_min({},{})={} != min={}", a, b, r, expected);
        }

        /// P6. py_min returns one of its two arguments.
        #[test]
        fn p6_py_min_returns_one_of_inputs(a in normal(), b in normal()) {
            let r = py_min(a, b);
            prop_assert!(r.to_bits() == a.to_bits() || r.to_bits() == b.to_bits());
        }
        ...
        #[test]
        fn p7_py_min_nan_first_returns_nan(b in normal()) {
            prop_assert!(py_min(f64::NAN, b).is_nan());
        }

        /// P8. NaN second: py_min(x, NaN) keeps x.
        #[test]
        fn p8_py_min_nan_second_returns_first(a in normal()) {
            prop_assert_eq!(py_min(a, f64::NAN), a);
        }

        // -----------------------------------------------------------------
        // py_round_to_int — structural properties
        // -----------------------------------------------------------------

        /// P9. py_round_to_int(x) is an exact integer (fractional part 0).
        #[test]
        fn p9_py_round_to_int_is_integer(x in normal()) {
            let r = py_round_to_int(x);
            prop_assert_eq!(r.trunc(), r,
                "py_round_to_int({})={} has fractional part {}", x, r, r.fract());
        }

        /// P10. py_round_to_int(x) differs from x by at most 0.5.
        #[test]
        fn p10_py_round_to_int_diff_at_most_half(x in normal()) {
            let r = py_round_to_int(x);
            prop_assert!((r - x).abs() <= 0.5 + 1e-12,
                "py_round_to_int({})={} differs by {} > 0.5", x, r, (r-x).abs());
        }

        /// P11. Round-half-even: py_round_to_int(0.5) == 0.0 and
        /// py_round_to_int(1.5) == 2.0 — even integer tie-breaking.
        /// Property: for all integers n, py_round_to_int(n + 0.5) is even.
        #[test]
        fn p11_py_round_to_int_ties_to_even(
            n in -1_000_000i64..1_000_000
        ) {
            let x = n as f64 + 0.5;
            // For exact representable n + 0.5, the round must go to even.
            // This only holds when n + 0.5 is exactly representable,
            // which is true for |n| < 2^52 ≈ 4.5e15.
        ...
        // -----------------------------------------------------------------
        // py_hypot — structural properties
        // -----------------------------------------------------------------

        /// P12. py_hypot is non-negative.
        #[test]
        fn p12_py_hypot_non_negative(a in normal(), b in normal()) {
            prop_assert!(py_hypot(a, b) >= 0.0);
        }

        /// P13. py_hypot is symmetric.
        #[test]
        fn p13_py_hypot_symmetric(a in normal(), b in normal()) {
            prop_assert_eq!(py_hypot(a, b), py_hypot(b, a));
        }

        /// P14. py_hypot(a, b) >= max(|a|, |b|).
        #[test]
        fn p14_py_hypot_ge_max_abs(a in normal(), b in normal()) {
            let h = py_hypot(a, b);
            let m = a.abs().max(b.abs());
            prop_assert!(h >= m, "py_hypot({},{})={} < max={}", a, b, h, m);
        }

        /// P15. py_hypot(0, x) == |x| for non-NaN x.
        #[test]
        fn p15_py_hypot_zero_returns_abs(x in normal()) {
            prop_assert_eq!(py_hypot(0.0, x), x.abs());
            prop_assert_eq!(py_hypot(x, 0.0), x.abs());
        }
    }
}
