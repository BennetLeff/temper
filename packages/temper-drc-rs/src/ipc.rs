...
pub fn net_currents() -> &'static HashMap<String, f64> {
    static CURRENTS: LazyLock<HashMap<String, f64>> = LazyLock::new(|| {
        ...
        // Added 2026-08-14 (PR #1195 copper-weight/layer-awareness follow-up,
        // docs/evidence/2026-08-13-router-nlayer-routing.md SS4): these three
        // nets matched no key above, so `get_net_current` silently fell back
        // to DEFAULT_SIGNAL_CURRENT (0.1A) for them -- safe by coincidence
        // for the first two, not by citation. Recorded explicitly so the
        // real figure is auditable rather than an accidental fallback.
        //
        // "BYPASS_RELAY-COIL" (power_in.bypass_relay-coil1/-coil2): NOT
        // invented. `elec/src/modules.ato`'s own PowerInput comment states
        // "Coil driver: RELAY_CTRL (3.3V GPIO) cannot drive the 75mA/12V
        // coil directly". Independently re-derived from that same file's
        // cited component values (r_relay_drop.value = 39ohm, power_15v =
        // 15V) and the incumbent relay's own cited coil spec (Omron
        // G4A-1A-E DC12: 160ohm/75mA/900mW -- docs/evidence/
        // 2026-07-30-pd3-part-selection-k1-c6-t1.md SS1.5, CITED-PRIMARY via
        // DigiKey/Newark/Octopart): I = 15V / (160+39)ohm = 75.4mA, matching
        // the source comment to within rounding.
        map.insert("BYPASS_RELAY-COIL".into(), 0.0754);

        // "Q_RELAY_DRV-G" (power_in.q_relay_drv-g): the AO3400A gate node,
        // driven from a 3.3V GPIO (`relay_ctrl`) through r_gate = 1kohm
        ...
        map.insert("Q_RELAY_DRV-G".into(), 0.0033);

        // "NTC-NO" (power_in.ntc-no): NOT the coil-drive path above -- this
        // is the AC-mains-side junction downstream of the inrush-limiting
        // NTC and in parallel with the bypass relay's NO contact
        // (`elec/src/modules.ato`: `ntc.p2 ~ d1.A`, `bypass_relay.NO ~
...
pub(crate) mod tests {
    ...
    fn test_get_net_current_router_nlayer_routing_nets() {
        assert!((get_net_current("power_in.bypass_relay-coil1") - 0.0754).abs() < 1e-9);
        assert!((get_net_current("power_in.bypass_relay-coil2") - 0.0754).abs() < 1e-9);
        assert!((get_net_current("power_in.q_relay_drv-g") - 0.0033).abs() < 1e-9);
