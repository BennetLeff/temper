...
impl DrcRule for GroundPlaneCheck {
    ...
    fn check(&self, board: &BoardState, constraints: &ConstraintSet) -> Vec<Violation> {
        ...
        // configured ground/return zone at all, this rule cannot tell
        // "noisy component legitimately has no ground plane" apart from
        // "no ground-zone data was ever provided" — treating the latter as
        // a violation made it fire unconditionally for every noisy-classed
        // component on any board lacking gnd-named `constraints.zones`
        // entries (the real config included), training reviewers to ignore
        // it. Mirrors the early return `safety::IsolationCheck` already
        // takes for the same "nothing configured to check against" case.
        if gnd_zone_names.is_empty() {
            return violations;
        }
...
pub(crate) mod tests {
    ...
    #[cfg_attr(test, test)]
    fn ground_plane_no_gnd_zone_configured_no_false_positive() {
        // 2026-08-08 vacuity remediation, Task 4: before the fix, a noisy
        // component on a board with ZERO gnd-named zones (this project's
        // real `temper_constraints.yaml` today: power_zone, driver_zone,
        // control_zone, interface_zone — none match gnd/ground/return)
        // unconditionally fired EMC_GND_001. This is the regression test
        // for that false-positive-storm bug.
        let comp = make_component("C1", 50.0, 50.0, "power_switching");
        let board = make_board(vec![comp]);
        let constraints = ConstraintSet {
            zones: vec![ZoneDefinition {
        ...
        assert!(
            violations.is_empty(),
            "no gnd-named zone configured must not produce a false positive, got {}",
            violations.len()
        );
    }
    ...
    pub const WASM_TESTS: &[(&str, fn())] = &[
        ...
        ("rules::emc::ground_plane::tests::ground_plane_no_gnd_zone_configured_no_false_positive", ground_plane_no_gnd_zone_configured_no_false_positive),
