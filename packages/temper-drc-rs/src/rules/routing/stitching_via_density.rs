...
use crate::board::{BoardState, CopperZone};
...
/// Collect the positions of all ground-connected vias.
fn collect_ground_vias(board: &BoardState) -> Vec<Point<f64>> {
    board
        .vias
        .iter()
...
/// Check the maximum nearest-neighbour gap among ground vias within a
/// single copper zone. Returns a violation if the gap exceeds
/// `MAX_SPACING_MM`.
fn check_zone_via_density(
    zone: &CopperZone,
    gnd_vias: &[Point<f64>],
) -> Option<Violation> {
...
impl DrcRule for StitchingViaDensityCheck {
    ...
    fn check(&self, board: &BoardState, _constraints: &ConstraintSet) -> Vec<Violation> {
        ...
        let gnd_vias = collect_ground_vias(board);
        ...
        for zone in &board.zones {
            ...
            if let Some(v) = check_zone_via_density(zone, &gnd_vias) {
                violations.push(v);
            }
        }
