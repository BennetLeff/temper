...
fn type_system_guards_fail_to_compile_for_the_stated_reason() {
    ...
    // DrcCount -- a capped (saturated) count cannot be laundered into an
    // honest one.
    t.compile_fail("tests/compile_fail/drc_count_fabricated.rs");
}
...
fn each_guard_expectation_still_pins_its_intended_error_code() {
    const EXPECTED: &[(&str, &str, &str)] = &[(
        "drc_count_fabricated",
        "E0451",
        "DrcCount fields private -- a saturated count cannot be laundered \
         into an honest one",
    )];
