...
pub(crate) mod tests {
    ...
    fn test_assembly_single_cell_grid() {
        ...
        for hs in [
            ...
            // `2.0 * 0.5 / 1.0` is `2 * k_c / dx2` with the fixture's
            // k_c = 0.5 and dx2 = 1.0 written out, so the assertion reads
            // as the formula it pins.  clippy const-folds both operands of
            // the `/` to 1.0 and calls it `eq_op`; the lint only reaches
            // this body in a non-test build (the wasm registry compiles
            // these modules as ordinary items), where it is wrong.
            #[allow(clippy::eq_op)]
            let diagonal = 2.0 * 0.5 / 1.0;
            assert_eq!(got.2[0], diagonal);
            assert_eq!(got.3[0], diagonal * 40.0 + 0.2);
        }
    }
    ...
    pub const WASM_TESTS: &[(&str, fn())] = &[
        ("thermal_scorer::tests::test_conductivity_field_matches_reference", test_conductivity_field_matches_reference),
        ("thermal_scorer::tests::test_conductivity_field_uniform_when_no_copper", test_conductivity_field_uniform_when_no_copper),
        ("thermal_scorer::tests::test_heat_source_field_matches_reference", test_heat_source_field_matches_reference),
        ("thermal_scorer::tests::test_heat_source_field_zero_power_skipped", test_heat_source_field_zero_power_skipped),
        ("thermal_scorer::tests::test_heat_source_field_off_grid_high_is_noop", test_heat_source_field_off_grid_high_is_noop),
        ("thermal_scorer::tests::test_heat_source_field_off_grid_low_wraps_like_numpy", test_heat_source_field_off_grid_low_wraps_like_numpy),
        ("thermal_scorer::tests::test_assembly_matches_reference", test_assembly_matches_reference),
        ("thermal_scorer::tests::test_assembly_single_cell_grid", test_assembly_single_cell_grid),
    ];
