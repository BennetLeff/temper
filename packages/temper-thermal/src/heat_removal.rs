...
#[cfg(feature = "python")]
#[pyfunction]
#[pyo3(signature = (cell_size_mm, origin_x, origin_y, height_cells, width_cells, xs, ys, r_cs, r_sa, h_conv_background = H_CONV_BACKGROUND))]
#[expect(
    clippy::too_many_arguments,
...
pub(crate) mod tests {
    ...
    pub const WASM_TESTS: &[(&str, fn())] = &[
        ("heat_removal::tests::empty_devices_background_only", empty_devices_background_only),
        ("heat_removal::tests::device_sink_accumulates", device_sink_accumulates),
        ("heat_removal::tests::board_heatsinked_device_skipped", board_heatsinked_device_skipped),
        ("heat_removal::tests::off_grid_footprint_is_noop", off_grid_footprint_is_noop),
        ("heat_removal::tests::overlapping_footprints_accumulate", overlapping_footprints_accumulate),
        ("heat_removal::tests::degenerate_inputs_raise_like_cpython", degenerate_inputs_raise_like_cpython),
        ("heat_removal::tests::subnormal_underflow_band_raises_division_by_zero", subnormal_underflow_band_raises_division_by_zero),
    ];
