...
#[expect(clippy::too_many_arguments, reason = "GEOS port mirrors Envelope::intersects' 4-point signature 1:1; a config struct would change the ported shape")]
...
#[expect(clippy::too_many_arguments, reason = "GEOS port mirrors Distance::segmentToSegment's 4-point signature 1:1; a config struct would change the ported shape")]
fn seg_seg_dist(ax: f64, ay: f64, bx: f64, by: f64, cx: f64, cy: f64, dx: f64, dy: f64) -> f64 {
    ...
    let no_intersection = if !env_intersects(ax, ay, bx, by, cx, cy, dx, dy) {
        ...
        if denom == 0.0 {
            ...
            #[expect(clippy::manual_range_contains, reason = "NaN-sensitive GEOS port; RangeInclusive::contains treats NaN as outside, changing behavior")]
...
pub(crate) mod tests {
    ...
    fn test_scan_returns_closest_pair() {
        ...
        let (best, pair) = copper_scan(&[a, c], &[b], &[1, 2], &[3]);
    ...
    fn test_scan_skips_shared_object_in_sublist() {
        ...
        let (best, pair) = copper_scan(&[p0, p2], &[p0, p1, p2], &[100, 102], &[100, 101, 102]);
    ...
    pub const WASM_TESTS: &[(&str, fn())] = &[
        ...
        ("clearance_geometry::tests::test_circle_pad_distance", test_circle_pad_distance),
        ("clearance_geometry::tests::test_containment_zero", test_containment_zero),
        ("clearance_geometry::tests::test_scan_returns_closest_pair", test_scan_returns_closest_pair),
        ("clearance_geometry::tests::test_scan_skips_self_pair_on_shared_object", test_scan_skips_self_pair_on_shared_object),
        ("clearance_geometry::tests::test_scan_skips_shared_object_in_sublist", test_scan_skips_shared_object_in_sublist),
        ("clearance_geometry::tests::test_scale_doubling_is_exact", test_scale_doubling_is_exact),
    ];
