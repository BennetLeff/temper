//! Wave 4, Phase 4: topological placement compute, consolidated from the
...
//! moved. The pure kernels live in the sibling `force`/`graph`/`heuristics`/
//! `numeric`/`placement`/`propagation`/`zone` modules; this module is the
//! wholly-pyo3 surface that exposes them to `temper_geometry`, exactly as the
//! old crate's inline bridge did for `temper_placement_topology`.
//!
//! # Ordering contract
//!
