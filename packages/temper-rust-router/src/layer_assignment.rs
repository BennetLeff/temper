...
///
/// Plain Rust in and out (no pyo3 type anywhere in the signature or body) so
/// [`tests::assign_layers_py_preserves_input_order_and_duplicates`] can call
/// it without pyo3 present. [`assign_layers_py`] is the `#[pyfunction]`
/// wrapper that gives Python the same behaviour under the same name.
fn assign_layers(net_names: &[String]) -> Vec<AssignRow> {
    net_names
        .iter()
        .map(|n| {
            let a = assign_one(n);
...
pub(crate) mod tests {
    ...
    fn assign_layers_py_preserves_input_order_and_duplicates() {
        let net_names = ["GND".to_string(), "SPI_CLK".to_string(), "GND".to_string()];
        let rows = assign_layers(&net_names);
    ...
    pub const WASM_TESTS: &[(&str, fn())] = &[
        ("layer_assignment::tests::all_seven_patterns_compile", all_seven_patterns_compile),
        ("layer_assignment::tests::every_default_constraint_allows_all_four_layers", every_default_constraint_allows_all_four_layers),
        ("layer_assignment::tests::first_match_wins_order_is_source_order", first_match_wins_order_is_source_order),
        ("layer_assignment::tests::exact_literal_alternatives_do_not_prefix_match", exact_literal_alternatives_do_not_prefix_match),
        ("layer_assignment::tests::embedded_newline_defeats_even_the_catch_all", embedded_newline_defeats_even_the_catch_all),
        ("layer_assignment::tests::ordinary_names_are_unaffected_by_the_newline_case", ordinary_names_are_unaffected_by_the_newline_case),
        ("layer_assignment::tests::ground_suffix_pattern_is_a_true_suffix_not_a_substring_match", ground_suffix_pattern_is_a_true_suffix_not_a_substring_match),
        ("layer_assignment::tests::assign_layers_py_preserves_input_order_and_duplicates", assign_layers_py_preserves_input_order_and_duplicates),
    ];
