...
/// Resolve the exact router execution scope without a Python policy owner.
///
/// An explicit target scope overrides the default name policy and profiling
/// limit, but never the canonical Rust-computed net order. Without an
/// explicit scope, the caller may transport the legacy policy's eligible
/// names and Rust validates, orders, and truncates them.
pub fn select_routable_nets(
    net_order: &[String],
    target_nets: Option<&[String]>,
    default_routable: &[String],
    max_nets: Option<usize>,
) -> Result<Vec<String>, String> {
    let ordered: BTreeSet<_> = net_order.iter().map(String::as_str).collect();
    if ordered.len() != net_order.len() || net_order.iter().any(|name| name.trim().is_empty()) {
        return Err("net order must contain unique, non-empty names".into());
    }
    let selected = if let Some(targets) = target_nets {
        let target_set: BTreeSet<_> = targets.iter().map(String::as_str).collect();
        if targets.is_empty()
            || target_set.len() != targets.len()
            || targets.iter().any(|name| name.trim().is_empty())
            || !target_set.is_subset(&ordered)
        {
            return Err("target nets must be a unique, non-empty subset of the net order".into());
        }
        net_order
            .iter()
        ...
        let default_set: BTreeSet<_> = default_routable.iter().map(String::as_str).collect();
        if default_set.len() != default_routable.len()
            || default_routable.iter().any(|name| name.trim().is_empty())
            || !default_set.is_subset(&ordered)
        {
            return Err("default routable nets must be a unique subset of the net order".into());
        }
        let mut rows: Vec<_> = net_order
            .iter()
            .filter(|name| default_set.contains(name.as_str()))
            .cloned()
            .collect();
        if let Some(limit) = max_nets {
            rows.truncate(limit);
        }
        rows
    };
...
#[cfg(feature = "python")]
#[pyfunction]
pub fn select_routable_nets_py(
    net_order: Vec<String>,
    target_nets: Option<Vec<String>>,
    default_routable: Vec<String>,
    max_nets: Option<usize>,
) -> PyResult<Vec<String>> {
    select_routable_nets(
        &net_order,
        target_nets.as_deref(),
        &default_routable,
        max_nets,
    )
...
pub fn register(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(select_routable_nets_py, m)?)?;
    Ok(())
}
...
pub(crate) mod tests {
    ...
    #[cfg_attr(test, test)]
    fn explicit_route_scope_overrides_default_policy_and_limit() {
        let order = vec!["a".into(), "target".into(), "z".into()];
        let targets = vec!["target".into()];
        assert_eq!(
            select_routable_nets(&order, Some(&targets), &[], Some(0)),
            Ok(targets)
        );
    }

    #[cfg_attr(test, test)]
    fn default_route_scope_is_ordered_and_bounded() {
        let order = vec!["a".into(), "b".into(), "c".into()];
        let defaults = vec!["c".into(), "a".into()];
        assert_eq!(
            select_routable_nets(&order, None, &defaults, Some(1)),
            Ok(vec!["a".into()])
        );
    }
