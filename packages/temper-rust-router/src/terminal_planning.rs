...
fn extract_net_terminals(
    ...
    let mut occurrence_by_pin: HashMap<(&str, &str), usize> = HashMap::new();
    for (component_ref, pad_name) in net_pins {
        ...
        // Physical pad identity is (component, pad number, occurrence), not
        // merely (component, pad number). K2/K3 deliberately duplicate relay
        // contact pad numbers for current sharing; first-match lookup collapses
        // the two holes onto one coordinate and makes the second terminal
        // disappear. The Nth repeated net-pin reference resolves to the Nth
        // matching physical pin in component encounter order, matching the
        // parser's occurrence contract.
        let occurrence = occurrence_by_pin
            .entry((component_ref.as_str(), pad_name.as_str()))
            .or_insert(0);
        let pad_occurrence = PadOccurrence::new(pad_name, *occurrence);
        *occurrence += 1;
        let pin = match comp
            .pins
            .iter()
            .filter(|p| &p.name == pad_name || &p.number == pad_name)
            .nth(pad_occurrence.occurrence())
        {
            Some(p) => p,
            None => continue,
        };
...
pub(crate) mod tests {
    ...
    #[cfg_attr(test, test)]
    fn extract_net_terminals_resolves_duplicate_pad_numbers_by_occurrence() {
        let comp = ComponentRow {
        ...
        let net_pins = vec![
            ("K2".to_string(), "3".to_string()),
            ("K2".to_string(), "3".to_string()),
        ];
        let out = extract_net_terminals("discharge.k_dis1-no", &net_pins, &[comp], &[]);
        assert_eq!(out.len(), 2);
        assert_eq!((out[0].x, out[0].y), (0.0, 0.0));
        assert_eq!((out[1].x, out[1].y), (7.5, 0.0));
    }
    ...
    pub const WASM_TESTS: &[(&str, fn())] = &[
        ...
        ("terminal_planning::tests::extract_net_terminals_resolves_duplicate_pad_numbers_by_occurrence", extract_net_terminals_resolves_duplicate_pad_numbers_by_occurrence),
    ];
