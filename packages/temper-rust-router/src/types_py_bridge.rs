...
pub fn bridge_bundle_manifest(
    ...
    let bundles_list: &Bound<'_, PyList> = binding.cast::<PyList>()?;
    ...
    let bfn: &Bound<'_, PyDict> = bfn_binding.cast::<PyDict>()?;
    let mut bundle_id_for_net = HashMap::new();
    for (key, val) in bfn.iter() {
        let net_idx: usize = key.extract::<usize>()?;
        let bundle_id: usize = val.extract::<usize>()?;
