...
def generate_routing_report(pcb_path: Path, constraints_path: Path) -> str:
    ...
    report.append("The HV section contains the voltage doubler and the half-bridge. Use thick traces and respect creepage.")
    report.append("\n**Key Instructions:**")
    report.append("- **DC_BUS+ / DC_BUS-**: Route with 2.0mm minimum width. Keep these paths as short as possible between C_BUS1, C_BUS2, and the IGBTs (Q1, Q2).")
    report.append("- **SW_NODE**: This is the most electrically noisy net. Keep its area minimal. Connect Q1-Emitter, Q2-Collector, and J_COIL using a wide copper pour if possible.")
    report.append("- **Isolation Barrier**: Maintain a clear 10mm gap between any HighVoltage net and the Signal nets in MCU_ZONE. Do not cross the barrier with any copper except for the isolated gate driver signals.")
    report.append("")
    ...
    for loop in constraints.critical_loops:
        ...
        report.append("**Instruction:** Route the GATE and Return (GND_ISO/SW_NODE) traces as a differential pair or stacked on Top/Bottom layers to cancel inductance.")
    ...
    report.append("- **CGND (Control Ground)**: Use a solid copper plane in the LV_ZONE and MCU_ZONE.")
    report.append("- **Star Point**: Connect PGND and CGND at EXACTLY one point: (50, 40). Use a 0-ohm resistor or a narrow bridge.")
    report.append("- **GND_ISO**: This is the floating ground for the high-side gate driver. It must be isolated from all other grounds. Route it locally around U_GATE and Q1.")
    ...
    try:
        ...
        if u_gate_pos and q1_pos:
            report.append(f"- **High-Side Gate**: Route U_GATE (at {u_gate_pos}) to Q1 (at {q1_pos}). Use a 0.5mm trace. Keep the return path (SW_NODE) directly underneath on the Bottom layer.")
        if u_gate_pos and q2_pos:
            report.append(f"- **Low-Side Gate**: Route U_GATE (at {u_gate_pos}) to Q2 (at {q2_pos}). Use a 0.5mm trace. Keep the return path (PGND) directly underneath.")
    except Exception:
        report.append("- *Component positions could not be extracted for specific advice. Use standard Manhattan routing.*")

    # 7. Finishing Touches
    report.append("\n## 6. Finishing and Optimization")
    report.append("- **Vias**: Minimize vias on HighVoltage and Power paths. Each via adds inductance and resistance.")
    report.append("- **Teardrops**: Use teardrops on all pad-to-trace connections to improve mechanical reliability.")
    report.append("- **Thermal Relief**: Use thermal relief for all pads connected to large copper planes to facilitate soldering.")
